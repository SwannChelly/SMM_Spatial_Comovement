...
\paragraph{The extensive margin.} Equation~\eqref{eq:ek-share} is the probability that a
\emph{single} variety is won by $r'$. With $N_{s}$ varieties the win events are independent
across $\rho$, so the number $K_{r'rs}$ of sector-$s$ varieties that buyer $r$ sources from
$r'$ is binomial and
\begin{equation}
K_{r'rs}\sim\mathrm{Bin}\big(N_{s},\gamma_{r'rs}\big),
\qquad
\Pr\big(K_{r'rs}=0\big)=(1-\gamma_{r'rs})^{N_{s}}>0 .
\label{eq:binomial}
\end{equation}
...
a continuum this probability is zero for every region with $T_{r's}>0$, which is the sense in
which the extensive margin previously had no content. Letting $q_{r's}$ denote the probability
that $r'$ wins a given variety \emph{somewhere} in the downstream industry --- the union over
buyers, which is the event our surveys record, since they report whether a firm supplies the
industry and not which buyer it serves --- the industry-wide count is
$K_{r's}\sim\mathrm{Bin}(N_{s},q_{r's})$.

\paragraph{What the model implies for the extensive-margin regression.} Conditioning on a
firm's own productivity delivers the empirical specification of Equation~(1) in closed form.
For a champion of productivity $z$ located in $r'$, whose area anchor is the downstream buyer
$a$, the probability of \emph{not} supplying that buyer is complementary log-log,
\begin{equation}
...
Three features drive the estimation. The distance coefficient is $+\theta\alpha$, so a cloglog
of the ``does not supply'' indicator on log distance and log own productivity, with an
area$\times$sector fixed effect, recovers $\alpha=\hat\beta_{\ln d}/\theta$ directly. The
variety count is \emph{absent} from \eqref{eq:cloglog}: conditioning on the firm removes it, so
$\alpha$ and $N_{s}$ are identified from separate features of the data. And the coefficient on
own productivity equals $-\theta$, which is a free over-identifying test of a parameter we
otherwise calibrate.
...
\begin{equation}
\mu=\big\{\Omega_{L},\;\{\Omega_{s}\},\;\{T_{as}\},\;\alpha,\;\{A_{r}\},\;\{N_{s}\}\big\}.
\end{equation}
