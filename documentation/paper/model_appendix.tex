...
% app:model-price (the love-of-variety discussion, which lives inside the
% Derivation subsection next to the price indices -- keep the label there).
% =====================================================================
...
\emph{average} rather than a sum is a normalization of the love-of-variety level, discussed
below around \eqref{eq:app-loveofvariety}; it leaves every sourcing share, and hence the entire
extensive margin, unchanged.
...
\end{equation}
The first of these deserves a word, since it is the one place where granularity meets the value
block. Origin $r'$'s champion cost for variety $\rho$ in market $(r,s)$ satisfies
$\Pr(c_{r'\rho s}(r)>x)=\exp(-T_{a(r')s}(w_{r's}\tau_{r'rs})^{-\theta}x^{\theta})$, so by
independence across origins the minimum is Weibull, $\Pr(c^{*}_{\rho}>x)=\exp(-\Phi_{rs}x^{\theta})$,
and $\mathbb{E}[(c^{*}_{\rho})^{k}]=\Phi_{rs}^{-k/\theta}\Gamma(1+k/\theta)$ for $k>-\theta$. With
$N_{s}$ varieties the within-sector index is a \emph{finite} CES aggregate, and we evaluate it at
its certainty-equivalent value $\widetilde P_{rs}=(\mathbb{E}[P_{rs}^{1-\nu_{s}}])^{1/(1-\nu_{s})}$,
which is $p_{rs}$ up to the variety count:
\begin{equation}
\widetilde P_{rs}=N_{s}^{\frac{1}{1-\nu_{s}}}\times
\underbrace{\Gamma\Big(\frac{\theta+1-\nu_{s}}{\theta}\Big)^{\frac{1}{1-\nu_{s}}}\Phi_{rs}^{-1/\theta}}_{N_{s}\text{-free}} .
\label{eq:app-loveofvariety}
\end{equation}
Since $1-\nu_{s}<0$ the leading factor decreases in $N_{s}$: more varieties, a lower sector price
index. Writing the aggregator in \eqref{eq:app-ces} as an \emph{average} rather than a sum sets
that factor to one --- equivalently, rescaling $T_{as}$ by $N_{s}^{\theta/(1-\nu_{s})}$ absorbs
it. This is legitimate because it acts on a gauge the sourcing shares do not see: the share
\eqref{eq:app-share} is a ratio of $T$'s within a sector, so the extensive margin, the count
moment and the within-area contrast are all unchanged, and it is the same gauge the estimator
fixes by normalizing one reference area per sector.

\label{app:model-price}%
Two caveats. First, $P_{rs}$ is a \emph{convex} transform of the average in \eqref{eq:app-ces},
so the realized index exceeds its certainty-equivalent value by Jensen's inequality --- by about
$6\%$ at $N_{s}\approx10^{2}$, falling to $1\%$ at $N_{s}\approx10^{3}$ at our calibration, where
the regularity condition $\theta>2(\nu_{s}-1)$ holds with little slack. We follow
\citet{lmm2023} in having downstream firms optimize against the expected index; most of that gap
is in any case not identifying variation, since a price-level shift is observationally
equivalent to a technology shift and, by Lemma~\ref{lem:qfree} below, none of it touches
$q_{r's}$ or the extensive margin. Second, the assumption must not survive into counterfactuals:
$\partial\ln\widetilde P_{rs}/\partial\ln N_{s}=1/(1-\nu_{s})<0$ is a genuine gain from variety
and a live propagation channel --- a shock that removes suppliers raises $P_{rs}$ downstream
through exactly this elasticity --- so any counterfactual that moves the variety set must map
back to the physical $T$ and use the realized index.
...
\emph{calibrated within the model}: given any candidate value of
$\mu=(\Omega_{L},\{\Omega_{s}\},\alpha,\{A_{r}\})$, each is pinned by the model itself rather
than left free. They are not fixed ahead of the estimation at some external target, and they are
not searched over either --- they are recovered, inside the simulation of the economy that $\mu$
implies, from the moments they exhaust. This is exact rather than approximate: minimizing the
criterion jointly over all parameters and minimizing it over $\mu$ with $T$ and $N$ set to their
within-model values give the same estimate.

We describe first how an economy is simulated at a given $\mu$, then the search over $\mu$.

\paragraph{Simulation algorithm.} Every evaluation of the criterion is solved in the same order.
Given the calibrated elasticities and a candidate parameter vector, we calibrate comparative
advantage, simulate the Ricardian competition to obtain the win-anywhere probabilities, calibrate
the variety count, and assemble the moments:
\begin{equation}
\mu=\big\{\Omega_{L},\;\{\Omega_{s}\},\;\alpha,\;\{A_{r}\}\big\}
\;\longrightarrow\;\{T_{as}\}
\;\longrightarrow\;\{q_{r's}\}
\;\longrightarrow\;\{N_{s}\}
\;\longrightarrow\;\tilde M(\mu) .
\label{eq:app-recursion}
\end{equation}
The arrows run one way only. Comparative advantage is calibrated without knowing the variety
count, and the variety count does not revise comparative advantage, so there is no fixed point
between $T$ and $N$ to solve --- a recursion, not a simultaneous system. We take the steps in
turn.

\paragraph{Step 1: comparative advantage.} We recover the area-level comparative advantage
$T^{*}_{as}(\mu)$ that reproduces the observed area sourcing shares \eqref{eq:app-gamma-area}
exactly. Trade costs are known at this point --- they are $\tau_{r'rs}=d_{r'r}^{\alpha}$ at the
candidate $\alpha$ --- so this is the inversion of a matrix-scaling problem, which we solve by
the damped multiplicative iteration
\begin{equation}
T^{+}_{as}=T_{as}\left(\frac{\hat{\tilde\gamma}_{as}}{\tilde\gamma_{as}(T)}\right)^{\varsigma},
\qquad \varsigma\in(0,1],
\label{eq:app-sinkhorn}
\end{equation}
renormalizing to the reference area of each sector after every pass, and recomputing at every
pass the general-equilibrium expenditure weights that enter \eqref{eq:app-gamma-area}.

Those weights are evaluated at the certainty-equivalent price system
\eqref{eq:app-loveofvariety}, that is, without reference to the variety count. This is exact
rather than approximate: $N_{s}$ enters $\widetilde P_{rs}$ only through a sector-level factor,
and the price index enters the equilibrium only through the combination
$\Omega_{s}p_{rs}^{1-\nu}$, so restoring the variety count here would be the reparametrization
$\Omega_{s}\mapsto\Omega_{s}N_{s}^{(1-\nu)/(1-\nu_{s})}$ --- absorbed by a parameter that is
itself estimated and normalized --- and would leave $T^{*}(\mu)$ numerically unchanged. That is
what makes \eqref{eq:app-recursion} a recursion rather than a simultaneous system.

It is useful to separate the two things iteration \eqref{eq:app-sinkhorn} does, because only the
first is covered by a standard argument.

\emph{The scaling problem at fixed expenditure.} Hold the expenditure weights
...
Recovering $T$ from target shares is then the classical matrix-scaling (Sinkhorn--Knopp) problem,
and \eqref{eq:app-sinkhorn} with $\varsigma=1$ is its standard iteration. Because $B$ has full
support --- every area can serve every buyer at finite cost --- the associated positive linear
map contracts the Hilbert projective metric with a modulus $\kappa_{S}=\tanh(\Delta/4)<1$, where
$\Delta$ is the projective diameter of the image cone (Birkhoff--Hopf). Two consequences: the
iteration converges geometrically from any strictly positive start, and \textbf{the fixed point
is unique up to the per-sector scale}, which is exactly the gauge the reference normalization
fixes. Given trade costs and expenditures, there is one and only one vector of comparative
advantages consistent with the observed area shares.

\emph{The general-equilibrium feedback.} Expenditure is not fixed. A change in $T$ moves the
price indices $p_{rs}$, hence $c_{r}$, hence downstream sales $y_{r}$ and the weights
...
$\kappa_{S}\approx0.005$ and $\|J_{\mathrm{GE}}\|\approx0.015$, so the sufficient condition holds
with a wide margin ($\approx0.02\ll1$); the measured spectral radius of the full map is
$\approx0.012$, and convergence to $10^{-9}$ takes a handful of iterations.%
...
% region-level continuum configuration.
\ We further confirm that the inversion recovers a known $T$ from its own implied shares to
machine precision, and that ten dispersed starting values converge to the same fixed point. This
is a local, numerically verified statement at the estimated parameters, not a global theorem:
\eqref{eq:app-contraction} is sufficient, not necessary, and we do not characterize the region of
the parameter space over which it holds. Should it fail at some candidate $\mu$, the iteration is
not used blindly --- non-convergence is detected and the candidate retains its warm-start $T$, so
the outer optimizer sees a well-defined objective throughout.

\paragraph{Step 2: win probabilities.} We simulate $m$ varieties per sector, draw one Fr\'echet
champion per (region, variety) pair as in Assumption~\ref{as:granular}, and run the Ricardian
competition \eqref{eq:app-champion} for every buyer.\footnote{The uniform draws feeding the
inverse-CDF transform $\varphi_{r'\rho s}=T_{a(r')s}^{1/\theta}(-\log u_{r'\rho s})^{-1/\theta}$
implied by \eqref{eq:app-champion} are taken from a scrambled Sobol sequence rather than from
i.i.d.\ pseudo-random numbers, as a variance-reduction device: every simulated moment is an
average over the $m$ varieties, and a low-discrepancy net makes those averages converge faster
than $1/\sqrt{m}$. The draws carry flat weights $1/m$, which keeps the simulated moments
unbiased for the minimum-coupled objects. Two design choices matter. \emph{Within a sector},
regions are the \emph{dimensions} of the net and varieties its points: we build one net per
sector, of dimension the number of active regions in that sector, and randomize it by an
independent base-2 digital shift in each dimension. The shift makes each region's coordinate
marginally uniform and dissolves the deterministic origin of the sequence while preserving its
equidistribution; the largest pairwise correlation across regions within a sector is of order
$1/\sqrt{m}$, so the draws behave as independent exactly where independence is needed --- the
Ricardian comparison is a minimum across regions of the same sector, and a spurious correlation
there would distort which origin wins. Building one low-dimensional net per sector rather than a
single net over all (sector, region) pairs is also what preserves the net's advantage, since
discrepancy deteriorates with dimension. \emph{Across sectors}, the same underlying sequence is
reused, sectors differing only by their own digital shift. This is innocuous here because no
comparison in the model ever puts a draw from one sector against a draw from another: the
competition that determines every win --- hence $q_{r's}$, the count moment, the extensive
margin and the sourcing shares --- runs within a sector. Sectors meet only in the value blocks,
through $p_{r}$, and there they meet through the sector index $p_{rs}$, which is itself an
average over the $m$ varieties, so what is aggregated across sectors are $m$-averaged quantities
rather than individual draws. We nonetheless revert to i.i.d.\ draws when estimating the
simulation variance $\Sigma_{\mathrm{sim}}$ and the Jacobian, where genuinely independent
designs across replications are what is wanted.} This yields, for each region, the number
$k_{r's}$ of simulated varieties it wins somewhere, and hence an estimate of the win-anywhere
probability $q_{r's}$.

Crucially $m$ is not $N_{s}$: the simulated varieties estimate $q_{r's}$, they do not realize an
economy of the model's size. What licenses that is:
...
upstream wages, since $w$ enters the comparison directly; wages are taken as data. Any $m$ large
enough to estimate $q_{r's}$ precisely will therefore do, whatever the model's own $N_{s}$.

\paragraph{Step 3: the variety count.} Together with Proposition~\ref{prop:firm},
Lemma~\ref{lem:qfree} says that $N_{s}$ reaches the moment vector only through the count moment
and through $\widehat N_{s}$ itself, and that both are functions of $q_{r's}$ alone. The
simulated count moment is then available in closed form, by the unbiased combinatorial estimator
\begin{equation}
\tilde G_{s}(n)=\frac{1}{|\mathcal{R}^{+}_{s}|}\sum_{r'\in\mathcal{R}^{+}_{s}}
\frac{\binom{m-k_{r's}}{n}}{\binom{m}{n}},
\qquad
\mathbb{E}\left[\frac{\binom{m-k}{n}}{\binom{m}{n}}\right]=(1-q)^{n}\ \ \text{for } n\le m,
\label{eq:app-Gclosed}
\end{equation}
the probability that $n$ varieties drawn without replacement from the $m$ simulated ones were all
lost. We use \eqref{eq:app-Gclosed} rather than the plug-in $(1-\hat q_{r's})^{n}$ because the
latter is convex in $\hat q$ and therefore biased upward. Cross-cell dependence --- two regions
in a sector face the same variety draws --- does not affect either expression, since an
expectation of an average is the average of expectations whatever the dependence.

The variety count is the integer that best matches the empirical count moment
$\hat G_{s}(0)$ of \eqref{eq:app-count},
\begin{equation}
\widehat N_{s}=\argmin_{N\in[N^{\mathrm{lo}}_{s},N^{\mathrm{hi}}_{s}]\cap\mathbb{N}}
\;\big(\tilde G_{s}(N)-\hat G_{s}(0)\big)^{2},
\label{eq:app-profile}
\end{equation}
over the bounds \eqref{eq:app-bounds}. Because \eqref{eq:app-Gclosed} is closed form, no
re-simulation is needed as $N$ varies, and since $\tilde G_{s}(n)$ is decreasing in $n$ the
search reduces to a monotone integer bisection: $\widehat N_{s}$ is an integer at every
evaluation, so integrality is never relaxed and never rounded. Clamping at either bound is
informative rather than benign: the upper bound binding means the model cannot generate enough
sparsity even when every variety is sourced from a single origin, which is a rejection signal for
the mechanism rather than a numerical nuisance.

\paragraph{Step 4: the moments.} All six blocks of Section~\ref{app:model-moments} are computed
from the \emph{single} Ricardian solve of step 2: the value blocks 1--3 and 5 from the realized
trade flows, block 4 from the cloglog regression run on the simulated firm population, and block
...
continuum limit $\beta(\mathbb{E}[y])$ rather than $\mathbb{E}[\hat\beta(N_{s})]$. The
finite-sample bias of the auxiliary cloglog --- which an average of $\hat\beta$ over realized
economies of size $N_{s}$ would have cancelled against the same bias in the data --- is therefore
not differenced out. It is a measurable quantity, of the order of $2$--$4\%$ of $\beta$ at
data-consistent group sizes, and we report it alongside $\hat\alpha$. The alternative is not
free: it would treat simulation noise on one block by replication while every other block relies
on the accuracy of the draw design, and it would sit oddly beside the value block, which is
already evaluated at the certainty-equivalent index rather than on a realized economy.
\end{remark}

Substituting the within-model values into the criterion collapses the outer search from several
hundred parameters to a handful, and makes the sourcing-share block just-identified along the
resulting manifold, so that the extensive-margin coefficient is fit by $\mu$ alone. Note that
just-identification here is at the \emph{area} level: within an area, the split of sourcing
across regions remains over-identified by the distance profile, which is the content of
Assumption~\ref{as:geo}.

\paragraph{Estimation algorithm.} The SMM estimator is defined as
\begin{equation}
...
moments; we construct them below. The minimization is carried out by Particle Swarm Optimization
...
parameters set to $1.5$ and $2.5$. The estimation proceeds in four steps.

\begin{enumerate}[topsep=3pt,itemsep=3pt,label=\arabic*.]

\item \textbf{A first fit under equal weights.} Starting values are set as follows: $\Omega_{L}$
and $\{\Omega_{s}\}$ at their empirical counterparts from input--output tables; $\{A_{r}\}$
proportional to $\pi_{r}^{1/\varepsilon}$; and $\alpha$ at the extensive-margin regression
estimate divided by $\theta$. We run the optimizer over $\mu$ with $W$ set to the identity,
refining with an alternating block scheme in which each iteration sequentially optimizes
$\{\Omega_{L},\Omega_{s}\}$, then $\alpha$, then $\{A_{r}\}$, with a search radius progressively
reduced across iterations. This yields $\hat\mu_{1}$.

\item \textbf{The weighting matrix.} At $\hat\mu_{1}$ we estimate the variance--covariance matrix
of the \emph{simulated} moments, $\Sigma_{\mathrm{sim}}$, by repeated re-simulation with
independent draws, and combine it with the variance--covariance matrix of the \emph{data}
moments, $\Sigma_{\mathrm{data}}$, described in the next paragraph:
\begin{equation}
...
errors described below, and the over-identification test.

\end{enumerate}

\paragraph{The variance--covariance matrix of the data moments.} In the SMM framework, noise
arises from two sources: sampling variability in the data and simulation error. Ideally, to
assess the contribution of sampling variability, one would estimate the variance--covariance
matrix of the empirical moments using bootstrap methods. In our setting, we distinguish between
two types of moments: those for which bootstrapping is feasible (regional sourcing shares, count
moments and extensive-margin parameters) and those for which it is not (input--output
coefficients and the downstream sales share distribution). We therefore approximate
$\Sigma_{\mathrm{data}}$ with a block-diagonal matrix that only accounts for within-group
interactions. The block corresponding to non-bootstrappable moments is set to zero. The block
associated with regional sourcing shares and count moments is estimated via bootstrap, resampling
supplier firms over the geography of active areas with the denominator of $\bar G_{s}(0)$ held
fixed --- so that a region with one or two firms can draw zero and flip into the empty set, which
is precisely the variance the count moment needs. The block for extensive-margin parameters is
given by the variance--covariance matrix obtained from the corresponding regression estimates.
% NOTE (implementation): the code loads a SINGLE joint bootstrap covariance over the three
% blocks beta -> gamma -> G (sigma_beta_gamma_filename in tools.jl), i.e. it does carry the
% cross-covariance blocks that the text here describes as set to zero. Either regenerate the
% Sigma file block-diagonally to match this text, or restore the "single joint bootstrap"
% wording of the previous draft. Flagged rather than silently resolved.

The bootstrap uses $15{,}000$ replications. Because some sector--region cells contain few
suppliers, a single firm may generate extreme sourcing shares and inflate the estimated variance
of the corresponding moments. We therefore restrict the bootstrap sample to cells with more than
three suppliers. This restriction removes cells for which the empirical share is dominated by a
single observation and reduces total supplier sales covered by approximately \textbf{X}\%.

We impose a second restriction on the same target, on economic size rather than on the number of
firms: we retain only sector--region cells accounting for more than $1\%$ of total sales in their
sector, which removes a further \textbf{Y}\% of covered supplier sales. This one is not a
variance argument but an identification argument.\footnote{The map from comparative advantage to
sourcing shares is multinomial-logit, so its gradient is a product of the share and one minus the
share, and vanishes with the share itself. Holding trade costs and expenditure fixed,
\eqref{eq:app-share} gives $\partial\ln\gamma_{ars}/\partial\ln T_{as}=1-\gamma_{ars}$ and
$\partial\ln\gamma_{ars}/\partial\ln T_{a's}=-\gamma_{a'rs}$ for $a'\neq a$, hence in levels
$\partial\gamma_{ars}/\partial\ln T_{as}=\gamma_{ars}(1-\gamma_{ars})$. Aggregating to the area
target with the expenditure weights $\omega_{rs}$,
$\partial\tilde\gamma_{as}/\partial\ln T_{as}=\sum_{r}\omega_{rs}\gamma_{ars}(1-\gamma_{ars})
=\tilde\gamma_{as}(1-\tilde\gamma_{as})-\mathrm{Var}_{\omega}(\gamma_{ars})$. As
$\tilde\gamma_{as}\to0$ that response vanishes at rate $\tilde\gamma_{as}$: a nearly empty cell
carries almost no information about its own comparative advantage. The elasticity, however, does
not vanish --- inverting the same expression,
$\partial\ln T_{as}/\partial\ln\tilde\gamma_{as}
=\tilde\gamma_{as}/[\tilde\gamma_{as}(1-\tilde\gamma_{as})-\mathrm{Var}_{\omega}(\gamma_{ars})]\to1$
--- so the \emph{relative} error in a small target passes into $\ln T_{as}$ one for one, and for a
cell resting on a handful of firms that relative error is itself of order one. Small cells are
therefore uninformative and maximally noisy at the same time. The weighting matrix offers no
protection here, because $T$ is calibrated rather than fitted: the inversion
\eqref{eq:app-sinkhorn} reproduces every retained share exactly, whatever variance $W$ attaches
to it. Nor does the damage stay local, since the calibrated $T^{*}(\alpha)$ enters $\Phi_{rs}$ in
\eqref{eq:app-Phi} and hence the extensive-margin moment through which $\alpha$ is identified: a
few wildly mismeasured comparative advantages recovered from near-empty cells blur the very
direction the outer search has to resolve. Excluding those cells from the target is the cleaner
remedy.}

\paragraph{Standard errors.} Let $G=\partial\tilde M/\partial\mu$ be evaluated at $\hat\mu_{2}$
by finite differences on common random numbers, and $\Omega=\Sigma_{\mathrm{data}}+\Sigma_{\mathrm{sim}}$.
For the searched parameters we report the efficient variance $(G'WG)^{-1}$ alongside the sandwich
form $(G'WG)^{-1}G'W\Omega WG(G'WG)^{-1}$, which coincide when $W=\Omega^{-1}$; the ratio is a
diagnostic on the weighting matrix.

Because $T$ and $N$ are calibrated within the model rather than searched, $G$ is taken
\emph{along the manifold they trace}: we perturb $\alpha$ and let $T^{*}(\alpha)$ follow through
\eqref{eq:app-sinkhorn}, so the reported standard error on $\alpha$ is a total derivative. We
deliberately never form the derivative of the moments with respect to $T$ as a free parameter.
The reason is that $T$ reaches the value moments through the Ricardian minimum: the identity of
the winning origin is a step function of $T$ at the level of an individual variety, so a
finite-difference column differences a handful of flipped winners and divides by the step. With
common random numbers the resulting column has a signal-to-noise ratio an order of magnitude
worse than the profiled one, and it does not improve with the number of draws at any usable step
size. Every statement about $T$ and $N$ below therefore travels through the delta method applied
to the calibration maps, which are deterministic and smooth, and never through that column.

For comparative advantage, $T^{*}$ is a function of the estimated $\hat\alpha$ and of the
observed area shares $\hat{\tilde\gamma}$ that \eqref{eq:app-sinkhorn} inverts. Writing
$f_{\alpha}=\partial T^{*}/\partial\alpha$ and $f_{\gamma}=\partial T^{*}/\partial\tilde\gamma$
--- both obtained by finite differences on the inversion itself, which carries no simulation
noise --- the delta method gives
\begin{equation}
\mathrm{Var}(\hat T)=f_{\alpha}\mathrm{Var}(\hat\alpha)f_{\alpha}'
+f_{\gamma}\Sigma_{\tilde\gamma\tilde\gamma}f_{\gamma}'
+f_{\alpha}\mathrm{Cov}(\hat\alpha,\hat{\tilde\gamma})f_{\gamma}'+(\cdot)' ,
\label{eq:app-deltaT}
\end{equation}
with $\Sigma_{\tilde\gamma\tilde\gamma}$ the bootstrap block of $\Sigma_{\mathrm{data}}$ and the
cross-term from the influence representation of $\hat\alpha$. Both channels matter: a
comparative advantage is uncertain because the trade-cost elasticity that positions it is
estimated, and because the share it is inverted from is itself measured with error. We report the
two contributions separately.

For the variety count, differentiating the calibration condition
$\tilde G_{s}(\widehat N_{s})=\hat G_{s}(0)$ gives
\begin{equation}
d\widehat N_{s}=\frac{d\hat G_{s}(0)
-(\partial\tilde G_{s}/\partial\alpha)\,d\hat\alpha
-(\partial\tilde G_{s}/\partial\tilde\gamma)\,d\hat{\tilde\gamma}}
{\partial\tilde G_{s}/\partial n},
\label{eq:app-deltaN}
\end{equation}
so the confidence interval for $\widehat N_{s}$ combines the sampling error in the count moment
itself with the transmission of $\hat\alpha$ and $\hat{\tilde\gamma}$ through $q_{r's}$. The
denominator is negative and available in closed form from \eqref{eq:app-Gclosed}. The share
channel dominates in our estimates, so an interval that ignored it would be more than twice too
narrow. The interval is continuous while $N_{s}$ is an integer, and we report it as such.
