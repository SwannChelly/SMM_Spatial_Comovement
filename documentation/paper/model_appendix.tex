...
\paragraph{Simulation algorithm.} Given the calibrated elasticities and a candidate $\mu$:

\begin{enumerate}[topsep=3pt,itemsep=5pt,label=\arabic*.]

\item \textbf{Calibrate comparative advantage.} We recover the area-level comparative advantage
...
candidate $\alpha$ --- so this is the inversion of a matrix-scaling problem, solved by the damped
...
pass the general-equilibrium expenditure weights that enter \eqref{eq:app-gamma-area}. The
iteration is described, and its convergence established, below.

\item \textbf{Draw varieties and calibrate the variety count.} We simulate $m$ varieties per
sector, draw one Fr\'echet champion per (region, variety) pair as in
Assumption~\ref{as:granular}, and run the Ricardian competition \eqref{eq:app-champion} for every
buyer, which yields for each region the number $k_{r's}$ of simulated varieties it wins somewhere
and hence the win-anywhere probability $q_{r's}$. The variety count is then the integer that best
matches the count moment,
\begin{equation}
...
\end{equation}
over the bounds \eqref{eq:app-bounds}, where $\hat G_{s}(0)$ is the empirical count moment
\eqref{eq:app-count} and $\tilde G_{s}(\cdot)$ its simulated counterpart, given in closed form
below. Crucially $m$ is not $N_{s}$: the simulated varieties are
used to estimate $q_{r's}$, not to realize an economy of the model's size. Why that is legitimate
--- and why \eqref{eq:app-profile} needs no re-simulation at each candidate $N$ --- is the
content of Lemma~\ref{lem:qfree} below.

\item \textbf{Build the moments.} All six blocks of Section~\ref{app:model-moments} are computed
from that \emph{single} Ricardian solve on one set of draws: the value blocks 1--3 and 5 from the
realized trade flows, block 4 from the cloglog regression run on the simulated firm population,
and block 6 in closed form from $\{k_{r's}\}$ and $\widehat N_{s}$.

\end{enumerate}

The three steps form a recursion, not a fixed point:
$\alpha\to T^{*}\to q_{r's}\to\widehat N_{s}$, with no arrow running backwards. Step 1 does not
need to know the variety count and step 2 does not revise comparative advantage. The two
paragraphs that follow establish each half of that claim.

\paragraph{Step 1: uniqueness, and the general-equilibrium feedback.} It is useful to separate
the two things iteration \eqref{eq:app-sinkhorn} does, because only the first is covered by a
standard argument.
...
machine precision, and that ten dispersed starting values converge to the same fixed point.
...
region of the parameter space over which it holds. Should it fail at some candidate $\mu$, the
iteration is not used blindly --- non-convergence is detected and the candidate retains its
warm-start $T$, so the outer optimizer sees a well-defined objective throughout.

\paragraph{Which price system step 1 uses.} The expenditure weights recomputed inside
\eqref{eq:app-sinkhorn} come from the certainty-equivalent price system
\eqref{eq:app-loveofvariety} --- the continuum index --- and not from a price index realized on
$N_{s}$ varieties. This is not an approximation whose error must be assessed, because the
variety count enters $\widetilde P_{rs}$ only through the factor $N_{s}^{1/(1-\nu_{s})}$, which
is common to all origins $a$ and all buyers $r$ within a sector, and because the price index
enters the equilibrium only through the combination $\Omega_{s}p_{rs}^{1-\nu}$ --- in the
aggregator $p_{r}$ and in expenditure $x_{rs}$ alike. Scaling $p_{\cdot s}$ by a sector constant
$\kappa_{s}$ is therefore \emph{exactly} the reparametrization
$\Omega_{s}\mapsto\Omega_{s}\kappa_{s}^{1-\nu}$, absorbed by a parameter that is itself estimated
and normalized. Restoring the variety count in step 1 would leave $T^{*}(\mu)$ numerically
unchanged. The same argument covers the Jensen gap of Appendix~\ref{app:model-price}: the ratio
of the realized index to its certainty-equivalent value is scale-free, depending on
$(N_{s},\theta,\nu_{s})$ alone, hence again a sector-level factor.

Two remarks. First, step 1 and step 3 use the same price system: the CES aggregate in the
simulation is taken as an average over the simulated varieties, as in \eqref{eq:app-ces}, which
is the certainty-equivalent object. There is no inconsistency between the price system used to
calibrate $T$ and the one used to evaluate the value moments. Second, this is what makes the
recursion of the previous paragraph triangular rather than simultaneous. Were the price system
used in step 1 to depend on $N_{s}$ in a way that was \emph{not} a sector-level scale, $T$ and
$N$ would have to be solved jointly. As Appendix~\ref{app:model-price} insists, that is exactly
the situation in counterfactuals that move the variety set, which must use the realized index;
it is not the situation at estimation.

\paragraph{Step 2: why no economy of size $N_{s}$ is ever drawn.} The variety count never enters
the simulation. It is confined to two objects, both closed form, and that confinement is the
content of Proposition~\ref{prop:firm} together with:
...
alone. Any $m$ large enough to estimate $q_{r's}$ precisely will do, whatever the model's own
$N_{s}$. Writing $k_{r's}$ for the number of the $m$ simulated varieties won somewhere by region
$r'$, the simulated count moment is evaluated by the unbiased combinatorial estimator
\begin{equation}
...
\end{equation}
which is the probability that $n$ varieties drawn without replacement from the $m$ simulated ones
were all lost. We use \eqref{eq:app-Gclosed} rather than the plug-in $(1-\hat q_{r's})^{n}$
...
an expectation of an average is the average of expectations whatever the dependence. So
\eqref{eq:app-Gclosed} carries no simulation noise of its own beyond that already in $\hat q$, no
economy of size $N_{s}$ is ever drawn, and no replication is needed.

Two consequences for \eqref{eq:app-profile}. It requires no re-simulation as $N$ varies, and
since $\tilde G_{s}(n)$ is decreasing in $n$ it reduces to a monotone integer bisection, so
$\widehat N_{s}$ is an integer at every evaluation --- integrality is never relaxed and never
rounded. Clamping at either bound is informative rather than benign: the upper bound binding
...
on the accuracy of the draw design, and it would sit oddly beside
Appendix~\ref{app:model-price}, where the value block is already evaluated at the
certainty-equivalent index rather than on a realized economy.
\end{remark}
...
resulting manifold, so that the extensive-margin coefficient is fit by
$(\alpha,\Omega_{L},\{\Omega_{s}\},\{A_{r}\})$ alone. Note that just-identification here is at
the \emph{area} level: within an area, the split of sourcing across regions remains
over-identified by the distance profile, which is the content of Assumption~\ref{as:geo}.
