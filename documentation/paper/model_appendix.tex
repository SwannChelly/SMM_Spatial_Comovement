...
\label{prop:firm}
Let a firm be a (region, variety) champion with productivity $z$ located in $r'$, and let $a$ be
its area's downstream anchor. Writing
...
\item \textbf{The empirical specification is the model's own auxiliary model.} A cloglog of the
``does not supply'' indicator on log distance and log own productivity, with an
area$\times$sector fixed effect, is exactly \eqref{eq:app-firm-cloglog} for a single downstream
market, with distance coefficient $+\theta\alpha$ and productivity coefficient $-\theta$. The
empirical outcome is the union over buyers rather than a single market (Remark~\ref{rem:dependence}),
so \eqref{eq:app-firm-cloglog} is exact in the single-destination limit and is otherwise an
auxiliary model. We therefore estimate the \emph{same} regression on simulated and observed data
and match the coefficients, which is consistent for $\alpha$ whether or not the link is exact.
...
\paragraph{Concentrating out comparative advantage.} Given $(\alpha,\Omega_{L},\{\Omega_{s}\},\{A_{r}\})$,
the area-level comparative advantage that reproduces the observed sourcing shares
\eqref{eq:app-gamma-area} exactly is the solution of a matrix-scaling problem. We solve it by
...
\end{equation}
recomputing the general-equilibrium expenditure weights at each pass and renormalizing to the
reference area per sector. The map is a contraction in the relevant region of the parameter
space, and the fixed point $T^{*}(\alpha,\Omega,A)$ is unique. Substituting it into
\eqref{eq:app-smm} collapses the outer search from several hundred parameters to a handful, and
makes the sourcing-share block just-identified along the profiled manifold, so that the
extensive-margin coefficient is fit by $(\alpha,\Omega,A)$ alone.
