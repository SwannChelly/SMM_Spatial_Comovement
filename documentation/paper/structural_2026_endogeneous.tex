...
Evaluating the two granular cells needs one object, and the cleanest route to it is the case in which it is a count. Write $\omega_{r'rs}=\sum_{j=1}^{N_{s}}v_{jrs}I_{jr's}$, where $v_{jrs}$ is the share of buyer $r$'s spending on sector $s$ going to variety $j$ and $I_{jr's}=\mathbf 1\{r'\text{ wins variety }j\text{ for }r\}$. Suppose first that varieties carried equal expenditure weights, $v_{jrs}=1/N_{s}$. Then $N_{s}\omega_{r'rs}$ is simply the number of varieties region $r'$ wins, which is $\operatorname{Binomial}(N_{s},\gamma_{r'rs})$, and
\begin{equation}
\mathbb{E}\big[H_{rs}\big]=H(\gamma_{rs})+\frac{1}{N_{s}}\big[1-H(\gamma_{rs})\big].
\label{eq:granular_binomial}
\end{equation}
The mechanism is visible in one line: a finite $N_{s}$ makes winner counts random, the variance of a count is mechanically positive, and a variance of shares is exactly what a Herfindahl adds up.\footnote{For $K\sim\operatorname{Bin}(N,\gamma)$, $\mathbb{E}[K^{2}]=N\gamma(1-\gamma)+N^{2}\gamma^{2}$, so $\mathbb{E}[(K/N)^{2}]=\gamma^{2}+\gamma(1-\gamma)/N$. Summing over $r'$ and using $\sum_{r'}\gamma_{r'rs}=1$ gives \eqref{eq:granular_binomial}.} What granularity adds is proportional to $1-H(\gamma_{rs})$: it concentrates whatever dispersion the expected network has left to concentrate, which is why it matters least where comparative advantage has already done the work.

Under CES the weights are not equal --- a cheaper winner takes a larger share --- so $N_{s}\omega_{r'rs}$ is a \emph{weighted} sum of winner indicators rather than a count, and \eqref{eq:granular_binomial} is not literally a binomial second moment. The argument survives intact with $1/N_{s}$ replaced by the \emph{effective number of varieties} $\mathcal{V}_{rs}=\mathbb{E}[\sum_{j}v_{jrs}^{2}]$, and extends to any pair of buyers:
\begin{equation}
...
\end{equation}
with $\mathcal{V}_{rr''s}=\mathbb{E}[\sum_{j}v_{jrs}v_{jr''s}]$, $Q_{rr''s}$ the probability that the two buyers win the same variety in the same region, and $\Gamma_{rr''s}$ the probability that they land in the same region through different varieties --- the overlap the sourcing shares of \eqref{eq:gamma_rrs} already imply. Setting $r''=r$ returns $\mathbb{E}[H_{rs}]=\mathcal{V}_{rs}+(1-\mathcal{V}_{rs})H(\gamma_{rs})$, which is \eqref{eq:granular_binomial} with the effective count in place of the nominal one.\footnote{Under Fr\'echet the identity of a variety's winner is independent of the price at which it is won, so expenditure weights and win locations are independent; winners are independent across varieties, so for $j\neq k$ the two buyers land in the same region with probability $\gamma_{rs}\!\cdot\!\gamma_{r''s}$. Expanding $\omega_{rs}\!\cdot\!\omega_{r''s}$ and separating $j=k$ from $j\neq k$ gives \eqref{eq:variety_kernel}. Equal expenditure shares give $\mathcal V_{rs}=N_s\cdot N_s^{-2}=1/N_s$, recovering \eqref{eq:granular_binomial}; by Cauchy--Schwarz $\mathcal{V}_{rs}\ge 1/N_{s}$ always, so reading granularity as a count \emph{understates} it.}

The model predicts $\mathcal{V}_{rs}$ to be the same for every buyer --- the Fr\'echet scale cancels in an expenditure share --- and that is a testable restriction rather than an assumption; measured across buyers its range does not exceed $0.013$ in either industry. Granularity then contributes $\mathcal{V}_{s}\big[1-\mathbb{E}_{r}H(\gamma_{rs})\big]$ in total, and the only thing left to determine is how that total divides between the two granular cells, which is what the cross-buyer kernel supplies.
