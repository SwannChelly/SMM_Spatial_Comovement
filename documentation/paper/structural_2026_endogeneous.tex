...
Figure~\ref{fig:counterfactual_distance} reports the distribution of $\bar d_r$ across buyers under each regime. Trade costs shorten the diffusion in both industries, by 23~km (6.6\%) in motor vehicles and 34~km (9.6\%) in aerospace, and they do so for \emph{every} buyer in both --- all 22 motor-vehicle and all 18 aerospace buyers source further once $\alpha$ is set to zero, as \eqref{eq:distance_rate} requires. Comparative advantage is where the two industries part, and Figure~\ref{fig:alignment_correlation} shows why. In motor vehicles, the covariance is negative for essentially every buyer: comparative advantage is co-located with demand. Equalising $\hat T$ therefore lengthens the diffusion for 20 of the 22 buyers, by 65~km on average, close to three times the effect of trade costs. In aerospace, the covariance changes sign across buyers, and the effect of equalising $\hat T$ is close to zero at the centre of the distribution: the average sourcing distance moves by 2~km and the median by 3. Figure~\ref{fig:counterfactual_distance_region} shows that this near-zero average hides large offsetting movements. Exactly half the aerospace buyers source further and half source closer, and 89\% of them move by more than 5~km --- against a motor-vehicle counterfactual in which no buyer moves by less than that and 91\% move the same way. Buyers in Toulouse lose the advantage of their local suppliers and source further. Buyers in the Paris region, whose purchases were drawn towards distant hubs such as Toulouse, source more locally: in Roissy -- Sud Picardie the average euro travels 377~km under the estimated allocation and 332~km once comparative advantage is equalised, a fall of 12\%.
