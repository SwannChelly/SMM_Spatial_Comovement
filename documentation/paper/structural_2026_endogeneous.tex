...
$$ 
A productivity gap $\Delta$ is exactly offset by a distance ratio $\exp(\Delta/\alpha\theta)$. This gives a common unit --- kilometers --- to compare productivity and distance.

meaning that a gap $\Delta$ in productivity between two region is compensated by a gap $\exp(\frac{\Delta}{\alpha\theta})$ in the bilateral distances. We report in Figure~\ref{} for each sector the distance gap that separate the comparative advantage of the most productive region with the median comparative advantage. 

Motor vehicles and Aerospace differ sharply. Motor vehicles spans six orders of magnitude in equivalent distance, against fewer than three for Aerospace: comparative advantage is far more extreme in the automotive sector. Three automotive sectors --- C33Z (Machinery \& Equipment Repair), G45Z (Motor Vehicle \& Motorcycle Trade), and G46Z (Wholesale Trade, excl. motor vehicles) --- have equivalent distances of $10^3$ to $10^6$ km, far beyond any distance observed in France: here comparative advantage alone drives concentration, regardless of trade costs. These are trade, repair and headquarter-type activities rather than heavy manufacturing, and they cluster overwhelmingly in Île-de-France. The remaining automotive sectors are heavy industrial inputs (Steel Manufacturing, Forging \& Metal Machining, Automotive Equipment Manufacturing) and sit below the benchmark, where trade costs dominate instead. This split is consistent with the geography of the industry: downstream car plants are relatively dispersed across France, so heavy, costly-to-ship inputs are naturally supplied by the nearest capable plant, leaving little room for comparative advantage to override proximity. Trade and headquarter functions face no such constraint --- they can serve the whole country from one location --- so comparative advantage, not distance, governs their concentration. Aerospace shows no such split: most sectors cluster near the benchmark, indicating that comparative advantage and trade costs are of comparable magnitude across the industry, consistent with a downstream industry itself concentrated in a few regions, which weakens the disciplining role of proximity on sourcing.
...
The influence of both forces in the propagation of the shock is very different for the two industries. For the Motor Vehicles industry, equalising $T$ across areas lowers the local share, from 7\% to 6\% on average, and sends the median euro 65 km further (347 to 412 km): productive suppliers are located where the buyers are, so comparative advantage and proximity reinforce one another. In aerospace, the same operation raises the local share, from 9\% to 10\%, and leaves the average sourcing distance unchanged (353 to 354 km). For this industry, we observe that the median region source more locally when we shut down comparative advantage than when we take both forces into account. This is a sign of a strong concentration of upstream sectors in the Aerospace industry in few hubs confirmed by the decomposition of the two forces for each downstream region in the bottom panel of Figure~\ref{fig:diffusion_counterfactual}. Shutting down the comparative advantage force considerably raises the local sourcing of downstream location far away from the two hubs. For example, around 7\% of a shock originating from Valence travels within a 100 km radius. This share nearly triple when we shut down the comparative advantage channel. 
...
\caption{Diffusion with one force switched off\label{fig:diffusion_counterfactual}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: $L_{r}(d)$ against $d$ for the median shocked region, under the estimated allocation and under three counterfactual regimes. \emph{Distance only} equalises comparative advantage across attraction areas; \emph{Comparative advantage only} sets $\alpha=0$; \emph{Neither} allocates sourcing uniformly across the sector's modelled cells. The realised curve is the finite-variety draw of the estimated economy, whose closed-form expectation is \emph{Both forces}; the gap between them is granularity. }
\end{figure}
