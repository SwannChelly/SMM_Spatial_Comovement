...
The decomposition yields two main conclusions. First, comparative advantage is by far the dominant source of dispersion in sourcing attractiveness in both industries: it accounts for 87\% of the variance in motor vehicles and 86\% in aerospace. The industries therefore do not differ primarily in the amount of comparative-advantage heterogeneity. Second, the role of geography depends importantly on its spatial alignment with comparative advantage. The pure distance component accounts for only 2\% of the variance in motor vehicles and 7\% in aerospace, consistent with the substantially larger trade-cost elasticity in aerospace. But in motor vehicles, the co-location component contributes an additional 11\%, more than five times the contribution of distance alone. This reflects a strong negative correlation between comparative advantage and distance: at the median sector, $\operatorname{Corr}(\log\hat T,\log d)=-0.47$. Productive suppliers tend to be located close to buyers, so proximity amplifies the sourcing advantage of high-$T$ locations. However, the geography of buyers being itself not homogeneous, one can not tell wether each buyers its own pool of productive suppliers or if all buyers are spatially clustered around one supplier hub. We provide evidences for the second mechanism in the next section.  

In aerospace, by contrast, the co-location component is 9\%, only modestly larger than the 7\% contribution of distance alone, and the corresponding correlation is much weaker, at $-0.17$. Comparative advantage and proximity are therefore substantially less aligned. The relevant distinction across industries is thus not the dispersion of comparative advantage itself, nor simply the strength of distance costs, but the extent to which productive capacity is spatially concentrated near demand. In motor vehicles, comparative advantage and proximity largely reinforce one another; in aerospace, they operate more nearly as separate sources of sourcing variation.
...
The level is the same in the two industries: the euro released by a shock travels 347~km upstream in motor vehicles and 354~km in aerospace. The level on its own says nothing about \emph{which} force is holding the euro at that distance, nor whether the two industries hold it there for the same reason. The counterfactuals do.
...
Reporting the distribution rather than its mean is not presentational, and the reason is an exact property of the Ricardian allocation. Dialling comparative advantage down from its estimate towards a common value along $\rho_{lr}(t)\propto T_{a(l)}^{\,t}\,d_{lr}^{-\alpha\theta}$ makes the sourcing probabilities an exponential family in $t$ with sufficient statistic $\log T$, so
\begin{equation}
\frac{d}{dt}\,\mathbb{E}_{\rho_{r}(t)}\!\left[d_{lr}\right] \;=\; \operatorname{Cov}_{\rho_{r}(t)}\!\left(\log T_{a(l)},\,d_{lr}\right).
\label{eq:alignment}
\end{equation}
The distance a buyer's euro travels moves at the rate of the \emph{covariance} between capability and distance evaluated \emph{at that buyer} --- the co-location term of Table~\ref{tab:variance_decomposition}, in kilometres and buyer by buyer. Two consequences follow. A $\hat T$ that varies widely but is uncorrelated with the geography of demand reallocates sourcing across regions without sending the euro one kilometre further, so dispersion only bounds the effect, $|\operatorname{Cov}|\le\operatorname{sd}(\log T)\operatorname{sd}(d)$. And because the covariance is buyer-specific, a curve that shifts as a block says it is common across buyers, while a curve that bends only in its tail says it is concentrated in a few of them --- a distinction the mean cannot make.
...
This is Table~\ref{tab:variance_decomposition}'s co-location term arriving on the euros, and the shape of the curves is what confirms that reading rather than merely illustrating it. In motor vehicles the covariance in \eqref{eq:alignment} is negative in \emph{every} sector --- between $-6$ and $-116$~km, median $-54$~km --- so no buyer faces a geography in which capability and proximity pull apart, and the 65~km should be a displacement of the whole cross-section rather than a movement of its tail. In aerospace the median is only $-14$~km and the sign is not even common across sectors: other transport equipment, which alone carries 56\% of the industry's modelled intermediate purchases, has a \emph{positive} covariance of $+44$~km, so equalising $\hat T$ pulls its buyers \emph{closer} while pulling everyone else's further away. Buyers therefore differ in the direction in which equalising $\hat T$ moves them, and the 2~km aggregate is a near-cancellation of movements in both directions rather than an absence of movement. Figure~\ref{fig:counterfactual_distance} separates the two cases: a parallel displacement of the whole curve in motor vehicles against a rotation in aerospace, where the curves cross. \swann{A verifier sur la figure une fois produite, et donner un chiffre: de combien se deplace le p10 et le p90 dans chaque industrie (la fonction `distance\_distribution\_report` du notebook imprime exactement ces quantiles et leurs decalages contre `Both forces`).}
