...
\paragraph{Diffusion.} Amplification says how large the upstream response is, not where it lands, and it is the second object that is specific to a model with endogenous linkages. We summarise where a shock lands by two statistics. The first is the \emph{average sourcing distance}: writing $X_{r'r}$ for the upstream sales that a demand shock in downstream region $r$ generates in upstream region $r'$,
$$d_{r}=\frac{\sum_{r'} X_{r'r}\,d_{r'r}}{\sum_{r'} X_{r'r}},\qquad\text{and}\qquad L_{r}(d)=\frac{\sum_{r':\,d_{r'r}\le d} X_{r'r}}{\sum_{r'} X_{r'r}},$$
so that $d_{r}$ is the distance travelled by the average euro of upstream response and $L_{r}(d)$ is the share of that response accruing within $d$ kilometres of the origin of the shock. The first is the natural headline: it is a single number in kilometres, it aggregates the whole geography of the response rather than one radius, and it is exactly the object that identity~\eqref{eq:alignment} governs, so the counterfactuals below can be read against Section~\ref{sec:estimation} directly. The second is not redundant, and we keep it for a reason that the estimates make concrete: an average distance is blind to how mass is split between the immediate neighbourhood of the buyer and an intermediate range, and it is the local share that a region cares about. Figure~\ref{fig:local_share_map} maps $L_{r}(100)$ on a common colour scale.
...
The two industries source from equally far away on average: the euro released by a shock travels 347~km upstream in motor vehicles and 354~km in aerospace, and the median shocked region keeps about 6\% of its response within 100~km in both. What separates them is not the level of $d_{r}$ but the dispersion of the incidence behind it. In motor vehicles the local share ranges from 2.4\% to 18.0\%, and at 200~km no region falls below 15.5\%: shocks diffuse widely, but they diffuse in the same way wherever they start. In aerospace the range is 0.6\% to 39.2\%, and at 200~km the least local region retains just 3.1\%. The higher aerospace mean (8.8\% against 7.4\%) is carried by a single commuting zone, Toulouse, which keeps close to two fifths of its upstream response within 100~km, while a shock to most other aerospace regions is transmitted almost entirely elsewhere. Diffusion in motor vehicles is broad and homogeneous; in aerospace it is organised around a few hubs.

\paragraph{Which force produces this.} Because the sourcing probabilities are closed-form, the same shock can be propagated with one of the two allocative forces switched off, holding fixed how much of the euro leaves for upstream and how it splits across sectors\footnote{Switching off one of the two forces does not change the amplification coefficient: $\sum_{l}\rho_{lrs}=1$ in every regime, so $D_{r}$ is identical throughout and the regimes differ only in where the shock lands, not in how large it is.}. \emph{Distance only} equalises comparative advantage across attraction areas; \emph{Comparative advantage only} sets $\alpha=0$. We read the reallocation on $d_{r}$ first, and turn to $L_{r}(100)$ where the two statistics disagree. Figure~\ref{fig:diffusion_counterfactual} reports both: the top panel the distribution of $d_{r}$ across shocked regions under each regime, the bottom panel $L_{r}(100)$ region by region. Appendix Figure~\ref{fig:diffusion_profile} gives the whole profile $L_{r}(d)$ for the median region.


The two forces divide the work in opposite ways across the two industries, and the average sourcing distance states it in one number. In motor vehicles it is \emph{comparative advantage} that keeps the euro close: equalising $\hat T$ across areas sends it from 348 to 413~km, a 19\% increase, while switching off distance costs only 23~km (348 to 371~km). Productive suppliers are located where the buyers are, so removing capability dispersion destroys more proximity than removing the trade cost itself does --- which is exactly the co-location term of \eqref{eq:psi_decomposition} and the $-54$~km sourcing-weighted covariance behind \eqref{eq:alignment}. In aerospace the assignment is reversed: equalising $\hat T$ moves the euro by 2~km (355 to 358~km), essentially nothing, whereas switching off distance sends it 34~km further (355 to 389~km). There, capability is concentrated in hubs that are not where most of the demand is, the covariance is $-14$~km, and proximity is doing the localising on its own. \swann{Une phrase a ecrire une fois la figure produite: la moyenne bouge de 65 km en auto et de 2 km en aero, mais il faut regarder si la distribution se deplace en bloc ou seulement dans la queue.}

The local share confirms this in motor vehicles and, in aerospace, adds what the average cannot see. In motor vehicles both switches lower it --- to 6.2\% from 7.3\% when $\hat T$ is equalised, and to 5.1\% when $\alpha=0$: the two forces reinforce one another, so removing either one disperses the shock. In aerospace, equalising $\hat T$ \emph{raises} the local share from 8.9\% to 10.2\% while leaving $d_{r}$ flat. The two statistics are not in conflict --- they measure different things. The mass taken off Toulouse and Ile-de-France lands partly next door to each buyer, which the local share records, and partly at an intermediate distance, which offsets it in the mean. A section written on $d_{r}$ alone would have concluded that comparative advantage does nothing to aerospace incidence, which is false: it does nothing to how far the euro travels, and a great deal to whether the region that received the shock retains any of it. Region by region, the bottom panel shows that the gain is concentrated in the downstream locations far from the two hubs. Around 7\% of a shock originating in Valence travels within a 100~km radius, and that share nearly triples once the comparative-advantage channel is shut down.

This is the efficiency--stability trade-off of the introduction, made quantitative, and the two statistics say which side of it each industry sits on. The concentration of high-productivity aerospace suppliers around Toulouse and Paris raises the competitiveness of the downstream industry, but it also means that a shock originating anywhere in the industry converges on those same two areas: it does not travel further than it would otherwise --- $d_{r}$ is unchanged --- it travels to the same place, whoever is hit, and the regions in which it originates retain almost none of it. Motor vehicles, whose upstream capability is co-located with a more dispersed set of downstream producers, buys a 65~km shorter supply chain with that co-location and leaves no region without a local response.

\begin{figure}[H]
    \centering
\caption{Diffusion with one force switched off\label{fig:diffusion_counterfactual}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Top row: the cumulative distribution across shocked downstream commuting zones of the average sourcing distance $d_{r}$ --- the distance travelled by the average euro of upstream response --- one curve per regime, with the median of each distribution marked. Bottom row: $L_{r}(100)$ under the same regimes, one group of bars per shocked zone, ordered by the realised local share. \emph{Distance only} equalises comparative advantage across attraction areas; \emph{Comparative advantage only} sets $\alpha=0$. The realised curve is the finite-variety draw of the estimated economy, whose closed-form expectation is \emph{Both forces}; the gap between them is granularity, not a force. $D_{r}$ is identical in every regime by construction, so the regimes differ only in where the shock lands, not in how large it is.}
\end{figure}
