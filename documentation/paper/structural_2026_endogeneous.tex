...
The natural summary of ``where'' is the barycentre of a buyer's sourcing, and it cannot make that distinction. A barycentre is the \emph{first moment} of the destination distribution, and a dispersed central pool and a pair of distant hubs have nearly the same first moment: both sit near the national centre of mass, since with four fifths of the euros spread nationally the mean is pinned there however concentrated the remaining fifth is. In aerospace this is not a hypothetical --- every displacement points south-east, including from buyers as remote as Brest, while the arrowheads land in the centre of the country, some 250~km short of Toulouse. The map records a \emph{tilt}, not a destination. The same degeneracy afflicts any scalar built on that first moment: Appendix~\ref{apsec:diffusion_extra} reports the regression of the barycentre on the buyer's own position, and the shrinkage coefficient is below $0.12$ in both industries and does not separate them.
...
\swann{This paragraph carries no numbers yet. They come from \texttt{incidence\_concentration\_overlap} / \texttt{incidence\_summary} and are drawn by \texttt{plot\_destination\_composition} and \texttt{plot\_incidence\_plane}, all in the amplification section of \texttt{analysis\_granular.ipynb}. The run pushed on 2026-09-09 was executed before those functions existed, so the section needs a re-run of the amplification cells; the figure files are \texttt{destination\_composition\_\{auto,aero\}\_mu*.pdf} and \texttt{incidence\_plane\_mu*.pdf}. To fill: the median effective number of destinations and its ratio to the uniform allocation per industry, the median $\Delta_{r}$, and whether the predicted corner is where aerospace lands.}
