...
A great deal, and through one channel rather than two. At the estimates the median buyer's portfolio carries $\mathbb{E}_{\Omega}[H_{r}]=0.118$ in motor vehicles and $0.207$ in aerospace, against $H(\gamma_{r})=0.017$ and $0.055$ for the same buyer's expected network: with finitely many varieties the supplier portfolio is $6.9$ times as concentrated in motor vehicles and $3.7$ times as concentrated in aerospace as the allocation the model would deliver in the continuum. In effective numbers of supplier commuting zones, $58$ and $18$ become $8.5$ and $4.8$. Two distinct statements have to be kept apart here, and only one of them is about $\mathcal{V}_{s}$ alone. The \emph{level} of the granular term is: by \eqref{eq:granular_buyer} it is $\mathcal{V}_{s}\,[1-H(\gamma_{rs})]$, the second factor is bounded above by one, and the measured slack $1-H(\gamma_{r})$ is $0.983$ and $0.945$ --- within $1.7$ and $5.5$ percent of that ceiling --- so comparative advantage has concentrated the expected network far too little to attenuate chance, and the granular term is very nearly $\mathcal{V}_{s}$ itself ($0.101$ against $0.103$; $0.152$ against $0.157$). The \emph{ratio} above is not: dividing \eqref{eq:granular_buyer} by $H(\gamma_{rs})$ gives $1+\mathcal{V}_{s}[1-H(\gamma_{rs})]/H(\gamma_{rs})$, where the dispersion of the expected network enters through a \emph{denominator} and does real work. It is, up to the slack, the ratio of the effective number of regions in the expected network to the effective number of varieties, $58.3/9.7=6.0$ and $18.0/6.4=2.8$; had comparative advantage concentrated the expected network onto $H(\gamma_{r})=0.10$ at the same $\mathcal{V}_{s}$, the multiple would be $1.9$ and $2.4$ rather than $6.9$ and $3.7$. Granularity multiplies concentration by so much here \emph{because} the expected network is diffuse, not in spite of it. What survives as a statement about $\mathcal{V}_{s}$ alone is the level: the measured effective number of varieties, $1/\mathcal{V}_{s}=9.7$ and $6.4$, is almost the whole of the realised statistic. Precisely: \eqref{eq:granular_buyer} splits $\mathbb{E}_{\Omega}[H_{r}]$ into the two terms $H(\gamma_{r})$ and $\mathcal{V}_{s}[1-H(\gamma_{r})]$, and the second is $86$ percent of the total in motor vehicles and $73$ percent in aerospace --- the granular share of Table~\ref{tab:incidence}, reached here from the buyer-level decomposition rather than from the industry aggregate. Since the slack is within a few percent of one, that share is very nearly $\mathcal{V}_{s}/\mathbb{E}_{\Omega}[H_{r}]$ itself ($87$ and $76$ percent), which is the sense in which the effective variety count is the statistic. What remains for the geography is the last seventh or quarter. The reading is that with $\hat N_{s}$ between $12$ and $24$ and CES handing the cheapest winner a disproportionate share --- the effective count is about $0.6\,N_{s}$ --- a buyer buys a sector's inputs from of order ten distinct varieties, each won by one region, so its Herfindahl is close to a count of varieties wearing a region's label. That is a property of the object and not of the calibration: a Herfindahl weights regions by euros, and with finitely many varieties the euros pile onto whichever few varieties came out cheap. It is also the caution that governs everything below, including Figure~\ref{fig:buyer_concentration_region}: no intervention on $\hat T$ or $\alpha$ can move a statistic more than the tenth or quarter of it those forces own.
