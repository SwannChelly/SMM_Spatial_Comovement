...
\paragraph{Does sourcing track the buyer?} The counterfactual says capability sits near demand in motor vehicles. It does not say whether that capability is one mass placed inside the demand distribution or many local pockets, and those are different economies: the first sends every buyer to the same suppliers, the second gives each buyer its own. The distinction is directly measurable. Let $b_{r}=\sum_{r'}X_{r'r}x_{r'}/\sum_{r'}X_{r'r}$ be the barycentre of the sourcing a shock in $r$ generates, with $x_{r'}$ the location of upstream region $r'$, and regress it on the buyer's own location, $b_{r}=(1-\lambda)c+\lambda x_{r}+\varepsilon_{r}$. Then $\lambda=1$ means every buyer sources around itself, $\lambda=0$ means the allocation is buyer-independent and all shocks converge on one national barycentre, and intermediate values measure how much of its own position a buyer's sourcing inherits.

The model predicts the ordering before the number is computed, and it is the opposite of what the alignment alone would suggest. The only buyer-specific term in $\rho_{lrs}$ is $d_{lr}^{-\alpha\theta}$, so $\lambda$ is governed by $\alpha\hat\theta$ and must be \emph{smaller} in motor vehicles, the industry with the stronger alignment; it must also fall to zero under \emph{Comparative advantage only}, where $\alpha=0$ removes the buyer-specific term outright. Both predictions hold. The estimated shrinkage is $\hat\lambda=0.065$ in motor vehicles against $0.080$ in aerospace --- smaller in the industry with the stronger alignment, as $\alpha\hat\theta$ requires --- and it collapses to $-0.003$ and $-0.023$ respectively once $\alpha=0$, the control the mechanism demands. Both numbers are small and precisely so ($R^{2}=0.96$ and $0.85$): in \emph{neither} industry does a buyer's sourcing follow the buyer. What separates them is what happens when capability is equalised. In aerospace $\hat\lambda$ then rises by half, to $0.121$: it is the hubs that were holding sourcing away from each buyer's own neighbourhood, and removing them returns buyers to it. In motor vehicles equalising $\hat T$ leaves $\hat\lambda$ essentially unchanged (0.065 to 0.069), because nothing there was holding it down but the low trade-cost elasticity itself.

Appendix Figure~\ref{fig:sourcing_barycentre} draws the underlying displacements. This is the hub question answered. Motor-vehicle capability is one broadly central mass sitting inside the demand distribution: buyers do not source locally, they source from the same central pool, which is exactly how a negative covariance can hold at \emph{every} buyer simultaneously. Aerospace capability is concentrated away from most of its demand, and its buyers are pulled towards Toulouse and Île-de-France rather than towards themselves --- which is why switching that concentration off is the one intervention that makes aerospace sourcing more local.
...
be the share of the upstream response accruing within $d$ kilometres of the origin of the shock, mapped in Figure~\ref{fig:local_share_map}.
...
\caption{Where a downstream shock lands\label{fig:local_share_map}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/local_share_map_auto_mu1.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/local_share_map_aero_mu1.pdf}
    \end{tabular}
...
The median shocked region keeps about 6\% of its response within 100~km in both industries; the dispersion behind that median is what differs. In motor vehicles the local share ranges from 2.4\% to 18.0\% and no region falls below 15.5\% at 200~km: shocks diffuse widely, but in the same way wherever they start. In aerospace the range is 0.6\% to 39.2\% and the least local region retains 3.1\% at 200~km; the higher mean (8.8\% against 7.5\%) is carried by Toulouse alone, which keeps close to two fifths of its response within 100~km while a shock to most other aerospace regions is transmitted almost entirely elsewhere.

The counterfactual then produces the one sign reversal in this section, and it is the reason both statistics are needed. In motor vehicles both switches lower the local share --- to 6.2\% from 7.3\% when $\hat T$ is equalised, and to 5.1\% when $\alpha=0$ --- so the two forces reinforce one another and removing either disperses the shock. In aerospace, equalising $\hat T$ \emph{raises} the local share from 8.9\% to 10.2\% while leaving $d_{r}$ flat. The two statistics are not in conflict: the mass taken off Toulouse and Île-de-France lands partly next door to each buyer, which the local share records, and partly at an intermediate distance, which offsets it in the mean. ``How far'' and ``who keeps it'' are separate questions, and in aerospace comparative advantage answers only the second --- with the gain concentrated in the downstream locations farthest from the two hubs. Around 7\% of a shock originating in Valence stays within 100~km, and that share nearly triples once the comparative-advantage channel is shut down.

This is the efficiency--stability trade-off of the introduction, made quantitative, and the two industries sit on opposite sides of it. The concentration of high-productivity aerospace suppliers around Toulouse and Paris raises the competitiveness of the downstream industry, but it also means a shock originating anywhere in the industry converges on those same two areas: it does not travel further than it otherwise would --- $d_{r}$ is unchanged --- it travels to the same place, whoever is hit, and the regions in which it originates retain almost none of it. Motor vehicles, whose upstream capability is co-located with a more dispersed set of downstream producers, buys a 65~km shorter supply chain with that co-location and leaves no region without a local response.
