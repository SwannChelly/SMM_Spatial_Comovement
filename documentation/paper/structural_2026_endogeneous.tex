...
\paragraph{Trade cost and comparative advantages.} The estimated model attributes the geography of sourcing to two forces. For a downstream buyer in $r$, the probability that a given variety is supplied from upstream region $l$ is
$$
\rho_{lrs} \;\propto\; T_{a(l)s}\, d_{lr}^{-\alpha\theta},
$$
so on a log scale comparative advantage and proximity are additive and directly commensurable. Figure~\ref{fig:emp_sim_gamma_coef_G0} establishes that this allocation reproduces the observed regional sourcing structure; the estimated $\hat T_{as}$ and $\alpha\hat\theta$ now let us ask which of the two forces does the work, and where.

Two regions have equal odds of supplying a buyer when
$$
\log T_{a(r')}-\log T_{a(l)} \;=\; \alpha\theta \left(\log d_{r'r} - \log d_{lr}\right),
$$
so a productivity gap $\Delta$ is exactly offset by a distance ratio $\exp(\Delta/\alpha\theta)$: a comparative advantage can be quoted as the amount of additional distance it compensates. Two things enter that equivalent distance, and they are easily conflated. The size of the productivity gap is of the same order in both industries --- the top area exceeds the median one by 1.6 log points in motor vehicles and 2.2 in aerospace, with a comparable dispersion across the cells competing for a buyer, $\operatorname{sd}(\log \hat T) = 0.91$ and $0.96$. What differs is the price of distance: $\alpha\hat\theta = 0.23$ in motor vehicles against $0.46$ in aerospace. The same productivity gap therefore converts into twice as many decades of equivalent distance in motor vehicles, and comparative advantage is correspondingly more able to overwhelm proximity there. Three automotive sectors --- C33Z (Machinery \& Equipment Repair), G45Z (Motor Vehicle \& Motorcycle Trade), and G46Z (Wholesale Trade, excl. motor vehicles) --- have equivalent distances of $10^3$ to $10^6$ km, far beyond any distance observed in France: their concentration is driven by comparative advantage alone, whatever the trade costs. These are trade, repair and headquarter-type activities rather than heavy manufacturing, and they cluster overwhelmingly in Île-de-France. The remaining automotive sectors are heavy industrial inputs (Steel Manufacturing, Forging \& Metal Machining, Automotive Equipment Manufacturing) and sit below the benchmark, where trade costs dominate instead. This split is consistent with the geography of the industry: downstream car plants are relatively dispersed across France, so heavy, costly-to-ship inputs are naturally supplied by the nearest capable plant, leaving little room for comparative advantage to override proximity, while trade and headquarter functions can serve the whole country from one location. Aerospace shows no such split: most sectors cluster near the benchmark, with comparative advantage and trade costs of comparable magnitude across the industry.
...
\paragraph{Diffusion.} Amplification says how large the upstream response is, not where it lands, and it is the second object that is specific to a model with endogenous linkages. For each shocked downstream region $r$ we compute
$$L_{r}(d)=\frac{\sum_{r':\,d_{r'r}\le d} X_{r'r}}{\sum_{r'} X_{r'r}},$$
the share of the upstream sales generated by the shock that accrues within $d$ kilometres of its origin. Figure~\ref{fig:local_share_map} maps $L_{r}(100)$ on a common colour scale.
...
For both industries, the median shocked region keeps 6\% of its upstream response within 100 km. However, the distribution of this measure across downstream locations differ widely between the Motor Vehicle and Aerospace industries. In the first, the share ranges from 2.4\% to 18.0\%, and at 200 km no region falls below 15.5\%: shocks diffuse widely, but they diffuse in the same way wherever they start. In aerospace the range is 0.6\% to 39.2\%, and at 200 km the least local region retains just 3.1\%. The higher aerospace mean (8.8\% against 7.4\%) is carried by a single commuting zone, Toulouse, which keeps close to two fifths of its upstream response within 100 km, while a shock to most other aerospace regions is transmitted almost entirely elsewhere. Diffusion in motor vehicles is broad and homogeneous; in aerospace it is highly centered around few hubs. 

\paragraph{Which force produces this.} Because the sourcing probabilities are closed-form, the same shock can be propagated with one of the two allocative forces switched off, holding fixed how much of the euro leaves for upstream and how it splits across sectors\footnote{Switching one of two forces doesn't change the amplification coefficient.}. Figure~\ref{fig:diffusion_counterfactual} reports the two readings of the resulting reallocation. The top panel gives the distribution across shocked regions of the average distance the upstream euro travels --- the quantity identity~\eqref{eq:alignment} governs --- and the bottom panel the share of the response retained within 100~km, region by region. The two need not move together: mass taken off a distant hub can land partly next door and partly at an intermediate distance, raising the local share while leaving the average distance almost unchanged. Appendix Figure~\ref{fig:diffusion_profile} reports the whole profile $L_{r}(d)$ for the median region under each regime.


The influence of both forces in the propagation of the shock is very different for the two industries. For the Motor Vehicles industry, equalising $T$ across areas lowers the local share, from 7\% to 6\% on average, and sends the average euro 65 km further (348 to 413 km): productive suppliers are located where the buyers are, so comparative advantage and proximity reinforce one another. In aerospace, the same operation raises the local share, from 9\% to 10\%, and leaves the average sourcing distance essentially unchanged (355 to 358 km). The top panel of Figure~\ref{fig:diffusion_counterfactual} puts those two averages back into the cross-section, and says whether each is a common shift of the whole distribution or a movement of its tail alone.\swann{Une phrase à écrire une fois la figure produite: la moyenne bouge de 65 km en auto et de 3 km en aéro, mais il faut regarder si la distribution se déplace en bloc ou seulement dans la queue.} For this industry, we observe that the median region source more locally when we shut down comparative advantage than when we take both forces into account. This is the alignment margin of equation~\eqref{eq:alignment} at work: comparative advantage is of similar magnitude in the two industries, but only in motor vehicles does it sit on top of the buyers, and it is that covariance rather than the dispersion of $\hat T$ that the sourcing distance responds to. In aerospace, upstream capability is concentrated in a few hubs away from most of the demand, as confirmed by the decomposition of the two forces for each downstream region in the bottom panel of Figure~\ref{fig:diffusion_counterfactual}. Shutting down the comparative advantage force considerably raises the local sourcing of downstream location far away from the two hubs. For example, around 7\% of a shock originating from Valence travels within a 100 km radius. This share nearly triple when we shut down the comparative advantage channel. 

This is the efficiency--stability trade-off of the introduction, made quantitative. The concentration of high-productivity aerospace suppliers around Toulouse and Paris raises the competitiveness of the downstream industry, but it also means that a shock originating anywhere in the industry converges on those same two areas, while the regions in which it originates retain almost none of it. Motor vehicles, whose upstream capability is co-located with a more dispersed set of downstream producers, spreads shocks more evenly and leaves no region without a local response.
