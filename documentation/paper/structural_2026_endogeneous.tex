...
\paragraph{Trade cost and comparative advantages.} The estimated model attributes the geography of sourcing to two forces: comparative advantage across upstream regions and the cost of trading across distance. Together, they determine which regions supply downstream buyers and hence shape the production network. For a downstream buyer in $r$, the probability that a given variety is supplied from upstream region $l$ is $\rho_{lrs} \propto T_{a(l)s}\,d_{lr}^{-\alpha\theta}$ so comparative advantage and proximity are additive and directly commensurable in logs. Figure~\ref{fig:emp_sim_gamma_coef_G0} shows that this allocation reproduces the observed regional sourcing structure.

Disentangling the two forces from sourcing patterns alone is difficult: a region may be a large supplier because it is particularly productive or because it is close to demand. The two are observationally similar unless the trade-cost elasticity $\alpha\theta$ is identified separately. This is precisely what the estimation delivers, allowing us to assess which force drives sourcing.

A first comparison is the dispersion of comparative advantage. Averaged across sectors, the within-sector standard deviation of $\log \hat T_{as}$ is nearly identical across the two industries: 0.91 in motor vehicles and 0.96 in aerospace. By this measure, comparative advantage is equally dispersed.

The key difference lies instead in how industries price distance. The estimated trade-cost elasticity is $\alpha\hat\theta=0.23$ in motor vehicles versus $0.46$ in aerospace, twice as large. A comparative-advantage gap $\Delta$ is offset by a distance ratio $\exp(\Delta/\alpha\theta)$: when $\alpha\theta$ is smaller, larger distances are required to offset a given productivity advantage. Thus, an identical comparative-advantage gap shifts sourcing roughly twice as much in motor vehicles as in aerospace. Comparative advantage is therefore of similar strength across industries, but operates against substantially stronger trade-cost frictions in aerospace. Trade costs consequently constrain sourcing more tightly there, whereas comparative advantage has greater scope to override proximity in motor vehicles.

This comparison, however, treats the two forces as if they were laid out independently in space, so that only their relative strength mattered. They are not. Selection among the cells competing to supply a buyer operates on $\log\psi_{lrs}=\log T_{a(l)s}-\alpha\theta\log d_{lr}$, whose dispersion decomposes exactly into three terms,
\begin{equation}
\operatorname{Var}(\log\psi)=\operatorname{Var}(\log T)+(\alpha\theta)^{2}\operatorname{Var}(\log d)-2\alpha\theta\operatorname{Cov}(\log T,\log d).
\label{eq:psi_decomposition}
\end{equation}
The third term is the co-location of capability and demand: it is positive precisely when productive suppliers sit close to the buyers, in which case the two forces do not compete but reinforce one another, and a given dispersion of $\hat T$ produces a sharper selection than either force could deliver alone.

Measured at the estimates, this term is where the two industries part company, and it qualifies the reading of the previous paragraph. Comparative advantage accounts for a similar share of $\operatorname{Var}(\log\psi)$ in both industries (87\% in motor vehicles, 86\% in aerospace, at the sector median). Geography on its own accounts for little in either, and for three times more in aerospace (7\% against 2\%), as the larger $\alpha\theta$ requires. The co-location term contributes 11\% in motor vehicles against 9\% in aerospace, and the informative comparison is with the pure-geography term beside it: in motor vehicles co-location shapes the allocation \emph{five times} more than geography does on its own, whereas in aerospace the two are of the same order. Equivalently, the correlation between $\log \hat T$ and log distance to the buyer is $-0.47$ at the median motor-vehicle sector against $-0.17$ in aerospace. In motor vehicles capability and proximity are close to being the same variable; in aerospace they are close to orthogonal. The geography of the production network is therefore not settled by the tension between the two forces alone --- it is settled by where capability happens to sit relative to demand, and that is what we measure next.


\paragraph{Where comparative advantage pulls sourcing.} The comparison above measures the \emph{strength} of comparative advantage relative to geography. It says nothing about its \emph{direction}, and it is the direction that governs the geography of shock propagation: what matters is not how unequal comparative advantage is, but where it is located relative to downstream demand. Write $d_{rs}=\sum_{l}\rho_{lrs}\,d_{lr}$ for the average distance an euro spent by a buyer in $r$ on product $s$ travels upstream. Dialling comparative advantage down from its estimate towards a common value along $\rho_{lr}(t)\propto T_{a(l)}^{\,t}\,d_{lr}^{-\alpha\theta}$ moves that distance at the rate
\begin{equation}
\frac{d}{dt}\,\mathbb{E}_{\rho_{r}(t)}\!\left[d_{lr}\right] \;=\; \operatorname{Cov}_{\rho_{r}(t)}\!\left(\log T_{a(l)},\,d_{lr}\right),
\label{eq:alignment}
\end{equation}
an exact property of the Ricardian allocation: along that path the sourcing probabilities form an exponential family in $t$ whose sufficient statistic is $\log T$. What moves sourcing in space is therefore the \emph{covariance} between capability and distance to the buyer, measured in kilometres, and not the dispersion of capability: a $\hat T$ that varies widely but is uncorrelated with the geography of demand reallocates sourcing across regions without sending the euro one kilometre further. Dispersion only bounds that covariance, $|\operatorname{Cov}|\le \operatorname{sd}(\log T)\,\operatorname{sd}(d)$.

Equation~\eqref{eq:alignment} also fixes how the covariance must be measured. It holds for one sector and one buyer at a time, so the estimand is a within-(sector $\times$ downstream region) object, and it is taken under the sourcing probabilities rather than over regions counted equally --- what matters is where the buyer's euro comes from, not which places could in principle have supplied it. Equivalently, we run
$$
d_{lr} \;=\; a_{sr} \;+\; b\,\log \hat T_{a(l)s} \;+\; \varepsilon_{lrs},
\qquad \text{weights } \rho_{lrs},
$$
a weighted least squares with a sector $\times$ downstream-region fixed effect. Its slope is in kilometres per log point of comparative advantage: how much closer to the buyer a supplier sits, per unit of the capability that wins it the sale. The regression is a way of displaying the covariance in \eqref{eq:alignment}, not an estimate of an unknown: $\hat T$ and $d$ are model objects rather than a sample, so the dispersion around the line carries no sampling interpretation and we read the sign and the implied movement in $d_{rs}$ rather than a confidence interval.

Figure~\ref{fig:spatial_alignment} shows the cloud behind \eqref{eq:alignment} for a representative buyer in each industry: one dot per competing upstream commuting zone, its comparative advantage against its distance to that buyer, with the area of the dot given by the sourcing probability $\rho_{lr}$. The two industries are opposed, and they differ in kind rather than in degree. In motor vehicles the fitted line slopes downward in \emph{every} sector: the productive suppliers are the ones close to the assembly plants, so comparative advantage and proximity pull in the same direction, and integrating \eqref{eq:alignment} to a common $\hat T$ sends the average euro 65~km further --- a movement in $d_{rs}$ of the same order as the width of a French region. In aerospace the same operation moves it by 32~km at the median sector, and one sector has a line that slopes the \emph{wrong} way: capability there is concentrated in a small number of hubs --- Toulouse and Île-de-France --- that are not close to most of the demand, so the productivity premium of those hubs is paid for in kilometres by every buyer except the ones next to them, and equalising $\hat T$ brings sourcing no closer. The aerospace median overstates what happens to the industry as a whole, and for a reason that is itself the point: the sector carrying 56\% of its intermediate purchases --- other transport equipment --- is exactly the misaligned one, with a distance effect of 3~km, so the spend-weighted aggregate of the next section sits far below the median sector. In motor vehicles no sector plays that role and the two coincide.\footnote{$\hat T$ is the Sinkhorn image of the observed sourcing shares, so this covariance is model-implied rather than read directly off the data. Writing $\log \hat T = \log \hat\gamma^{\,emp} - \log M(\alpha)$, it splits into the gradient of the observed shares and minus that of market access. In aerospace the two contribute about equally. In motor vehicles the observed shares are, if anything, slightly increasing in distance, and the whole of the negative alignment comes from the market-access term: areas far from a given buyer tend to be centrally located overall, so the sales they are observed to make require less comparative advantage to rationalise.}

This distinction is what carries into the propagation of shocks. Where productive suppliers sit close to downstream demand, comparative advantage reinforces the local incidence that trade costs already generate; where they are concentrated in a few hubs, comparative advantage instead redirects shocks towards those hubs and away from the regions in which they originate. The next section quantifies that incidence by propagating an actual shock with each force switched off in turn.

\begin{figure}[H]
    \centering
\caption{Where comparative advantage pulls sourcing\label{fig:spatial_alignment}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/spatial_alignment_auto_mu2.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/spatial_alignment_aero_mu2.pdf}
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Each dot is an upstream commuting zone competing to supply one representative downstream buyer --- the buyer whose average sourcing distance is of median length. The horizontal axis is its estimated comparative advantage $\log \hat T_{a(l)s}$ and the vertical axis its distance to that buyer in kilometres, both measured as deviations from their sourcing-weighted sector mean, since $\hat T$ is identified only up to a per-sector scale. The area of each dot is the sourcing probability $\rho_{lr}$, and the line is the sourcing-weighted fit with a sector $\times$ buyer fixed effect, whose slope --- annotated on each panel --- is in kilometres of sourcing distance per log point of comparative advantage. The most heavily sourced areas are named. A downward line means the buyer's euro comes from places that are both productive and close, so equalising comparative advantage would push sourcing outward; a flat line means comparative advantage is orthogonal to the buyer's geography and equalising it would move the euro nowhere.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{Distance equivalent of comparative advantage\label{fig:ca_distance_equivalence}}
    \begin{tabular}{cc}
    Motor vehicles & Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/ca_distance_equivalence_auto_mu2.pdf}&\includegraphics[width = 0.5\linewidth]{figures/ca_distance_equivalence_aero_mu2.pdf}\\ \vspace{1em}\\
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: For each sector, the bar reports the distance (in km) that would generate the same sourcing-probability gap as the observed comparative-advantage gap between the most productive region and the median region, holding all else equal. The dotted line represent the median bilateral distance observed in the data between upstream and downstream regions. 
    }
\end{figure}
...
\end{equation}
the distance travelled by the average euro of upstream response. It is a single number in kilometres, it aggregates the whole geography of the response rather than one radius, and it is exactly the object identity~\eqref{eq:alignment} governs --- so what Section~\ref{sec:estimation} measured on the sourcing probabilities can be read here directly on the euros.

The level is the same in the two industries: the euro released by a shock travels 347~km upstream in motor vehicles and 354~km in aerospace. That similarity is not informative on its own, since it is close to what the geography of France would give a buyer sourcing at random, and it says nothing about \emph{which} force is holding the euro at that distance. The counterfactuals do.

\paragraph{Which force keeps the euro close.} Because the sourcing probabilities are closed-form, the same shock can be propagated with one of the two allocative forces switched off, holding fixed how much of the euro leaves for upstream and how it splits across sectors.\footnote{Switching off one of the two forces does not change the amplification coefficient: $\sum_{l}\rho_{lrs}=1$ in every regime, so $D_{r}$ is identical throughout and the regimes differ only in where the shock lands, not in how large it is.} \emph{Distance only} equalises comparative advantage across attraction areas; \emph{Comparative advantage only} sets $\alpha=0$. Figure~\ref{fig:counterfactual_distance} reports the resulting distribution of $d_{r}$ across shocked regions, one cumulative curve per regime. Reporting the distribution rather than its mean is not presentational: by~\eqref{eq:alignment} the movement of $d_{r}$ \emph{is} the alignment covariance evaluated at that buyer, so a curve that shifts as a block says the covariance is common across buyers, while a curve that only bends in its tail says it is concentrated in a few of them.
...
The two forces divide the work in opposite ways across the two industries, and one number states it. In motor vehicles it is \emph{comparative advantage} that keeps the euro close: equalising $\hat T$ across areas sends it from 348 to 413~km, a 19\% increase, while switching off distance costs only 23~km (348 to 371~km). Productive suppliers sit where the buyers are, so removing the dispersion of capability destroys more proximity than removing the trade cost itself does. In aerospace the assignment is reversed: equalising $\hat T$ moves the euro by 2~km (355 to 358~km), essentially nothing, whereas switching off distance sends it 34~km further (355 to 389~km). There, capability is concentrated in hubs that are not where most of the demand is, and proximity is doing the localising on its own.

This is the co-location term of \eqref{eq:psi_decomposition} and the alignment covariance of \eqref{eq:alignment} arriving on the euros, and the shape of the curves is what confirms that reading rather than merely illustrating it. In motor vehicles \emph{every} sector has a negative covariance --- $-6$ to $-116$~km, median $-54$~km --- so no buyer faces a geography in which capability and proximity pull apart, and the 65~km is predicted to be a shift of the whole cross-section rather than a movement of its tail. In aerospace the median covariance is only $-14$~km and the sign is not even common across sectors: other transport equipment, which alone carries 56\% of the industry's modelled intermediate purchases, has a \emph{positive} covariance of $+44$~km. Buyers therefore differ in the direction in which equalising $\hat T$ moves them, and the 2~km aggregate is a near-cancellation of movements in both directions rather than an absence of movement. Figure~\ref{fig:counterfactual_distance} separates the two cases: a parallel displacement of the whole curve in motor vehicles against a rotation in aerospace, where the curves cross. \swann{A verifier sur la figure une fois produite, et donner un chiffre: de combien se deplace le p10 et le p90 dans chaque industrie (la fonction `distance\_distribution\_report` du notebook imprime exactement ces quantiles et leurs decalages contre `Both forces`).}

\paragraph{What the average distance hides.} How far the euro travels is not the same question as how much of it the shocked region keeps, and in aerospace the two answers differ. Let
$$L_{r}(d)=\frac{\sum_{r':\,d_{r'r}\le d} X_{r'r}}{\sum_{r'} X_{r'r}}$$
be the share of the upstream response accruing within $d$ kilometres of the origin of the shock. Figure~\ref{fig:local_share_map} maps $L_{r}(100)$ and Figure~\ref{fig:counterfactual_local_share} reports it region by region under the same three regimes.
...
\caption{Where a downstream shock lands\label{fig:local_share_map}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/local_share_map_auto_mu2.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/local_share_map_aero_mu2.pdf}
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Each shocked downstream commuting zone is coloured by $L_{r}(100)$, the share of the upstream sales generated by a demand shock in that zone that accrues within 100 km of it. Commuting zones hosting no downstream producer are shaded in grey: they originate no shock. Both panels use the same colour scale.}
\end{figure}

For both industries, the median shocked region keeps about 6\% of its response within 100~km, but the dispersion behind that median differs sharply. In motor vehicles the local share ranges from 2.4\% to 18.0\%, and at 200~km no region falls below 15.5\%: shocks diffuse widely, but they diffuse in the same way wherever they start. In aerospace the range is 0.6\% to 39.2\%, and at 200~km the least local region retains just 3.1\%. The higher aerospace mean (8.8\% against 7.4\%) is carried by a single commuting zone, Toulouse, which keeps close to two fifths of its upstream response within 100~km, while a shock to most other aerospace regions is transmitted almost entirely elsewhere. Diffusion in motor vehicles is broad and homogeneous; in aerospace it is organised around a few hubs.

The counterfactual confirms the reading above in motor vehicles and adds to it in aerospace. In motor vehicles both switches lower the local share --- to 6.2\% from 7.3\% when $\hat T$ is equalised, and to 5.1\% when $\alpha=0$: the two forces reinforce one another, so removing either disperses the shock. In aerospace, equalising $\hat T$ \emph{raises} the local share from 8.9\% to 10.2\% while leaving $d_{r}$ flat. The two statistics are not in conflict; they measure different things. The mass taken off Toulouse and Île-de-France lands partly next door to each buyer, which the local share records, and partly at an intermediate distance, which offsets it in the mean. Comparative advantage does nothing to how far the aerospace euro travels, and a great deal to whether the region that received the shock retains any of it --- and the gain is concentrated in the downstream locations far from the two hubs. Around 7\% of a shock originating in Valence stays within 100~km, and that share nearly triples once the comparative-advantage channel is shut down.
...
\caption{Local incidence with one force switched off\label{fig:counterfactual_local_share}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_local_share_auto_mu2.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_local_share_aero_mu2.pdf}
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: $L_{r}(100)$, the share of the upstream response accruing within 100 km of the origin of the shock, one group of bars per shocked commuting zone, ordered by the realised local share. Regimes as in Figure~\ref{fig:counterfactual_distance}. Appendix Figure~\ref{fig:diffusion_profile} reports the whole profile $L_{r}(d)$ for the median region under each regime.}
\end{figure}

This is the efficiency--stability trade-off of the introduction, made quantitative, and the two statistics say which side of it each industry sits on. The concentration of high-productivity aerospace suppliers around Toulouse and Paris raises the competitiveness of the downstream industry, but it also means that a shock originating anywhere in the industry converges on those same two areas: it does not travel further than it otherwise would --- $d_{r}$ is unchanged --- it travels to the same place, whoever is hit, and the regions in which it originates retain almost none of it. Motor vehicles, whose upstream capability is co-located with a more dispersed set of downstream producers, buys a 65~km shorter supply chain with that co-location and leaves no region without a local response.
