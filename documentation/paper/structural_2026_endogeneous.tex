...
Equation~\eqref{eq:granular_buyer} has three inputs and each is a force we can switch off: comparative advantage and trade costs both enter through $\gamma_{rs}$, and the variety count enters through $\mathcal{V}_{rs}$ alone. Before reading the counterfactuals, however, one property of $\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ has to be stated, because it decides what the statistic can be asked. At the measured $\mathcal{V}_{s}N_{s}\simeq1.8$ and $N_{s}$ between 12 and 24, the first term of \eqref{eq:granular_buyer} carries \textbf{83 to 91 per cent} of $\mathbb{E}_{\Omega}[H_{rs}(\Omega)]$. The reciprocal of a realised Herfindahl is therefore an effective number of \emph{varieties} with a region label attached to it, and the two forces --- which enter through $\gamma_{rs}$ and nothing else --- can move only the remaining tenth.

That is a property of the object and not of this calibration: a Herfindahl weights regions by the euros they collect, and with finitely many varieties those euros are concentrated on whichever few varieties happen to be cheap. The count of \emph{places} is a different statistic and carries no expenditure weights at all. Region $r'$ loses every one of the $N_{s}$ varieties of sector $s$ with probability $(1-\gamma_{r'rs})^{N_{s}}$, and variety draws are independent across sectors, so the expected number of distinct upstream commuting zones supplying buyer $r$ is
\begin{equation}
...
\end{equation}
exact under the same independence that gives \eqref{eq:granular_binomial}. Its $N_{s}\to\infty$ limit is the \emph{support}: the number of zones the buyer could reach at all. We report \eqref{eq:region_reach} beside the effective number of the \emph{expected} network, $1/H(\gamma_{r})$, which is euro-weighted and equally free of $\mathcal{V}$. The two answer different questions and neither replaces the other: \eqref{eq:region_reach} counts a zone winning one variety the same as one winning forty, so it says where a shock \emph{arrives}; $1/H(\gamma_{r})$ says how concentrated the euros are among the zones it arrives at.

\paragraph{The buyer-level portfolio in the data.}
Figure~\ref{fig:buyer_reach_region} reports \eqref{eq:region_reach} for every shocked commuting zone under the same two counterfactuals as Figure~\ref{fig:counterfactual_distance_region}, with the reachable support printed beside each row; Figure~\ref{fig:buyer_concentration_region} reports $1/\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ in levels on a logarithmic scale, with the $N_{s}\to\infty$ limit $1/\sum_{s}(X_{rs}/X_{r})H(\gamma_{rs})$ drawn beside it.\footnote{Levels on a log scale rather than percentage deviations, because the infinite-variety limit sits about seven times above the realised economy --- $+611\%$ against $+11\%$ and $+1\%$ for the two counterfactuals --- so on a percentage axis one series would crush the other three to invisibility. That the counterfactuals are that small is the substantive point of the paragraph above, not a defect of the figure.}

\swann{Les chiffres ci-dessous doivent \^etre remplis avec le nouveau run. Les fonctions sont dans \texttt{diffusion.ipynb} (section Test 5 bis) : \texttt{buyer\_region\_reach} / \texttt{plot\_buyer\_region\_reach} \'ecrivent \texttt{buyer\_reach\_region\_\{auto,aero\}\_mu2.pdf}, et \texttt{plot\_buyer\_granular\_concentration(units="level", kind="point", logx=True)} r\'e\'ecrit \texttt{buyer\_concentration\_region\_\{auto,aero\}\_mu2.pdf}. \`A reporter : la m\'ediane et l'\'etendue de la port\'ee, le ratio port\'ee/support, et le sens des deux contrefactuels sur la port\'ee.}

Three readings follow. \emph{First}, the level: a shock to the median motor-vehicle buyer reaches \swann{XX} of the \swann{XX} commuting zones its sectors could supply from, and a shock to the median aerospace buyer \swann{XX} of \swann{XX}. The gap between the two is granularity measured in places rather than in euros, and it is the statement that three quarters of the continuum's geographic reach is an artefact of assuming a continuum. \emph{Second}, the two counterfactuals move the reach in the direction \eqref{eq:region_reach} requires and the Herfindahl could not show: anything that flattens $\gamma_{rs}$ spreads the win probabilities over more zones, so both equalising $\hat T$ and setting $\alpha=0$ \swann{raise/lower} it, by \swann{XX} and \swann{XX} per cent. \emph{Third}, the euro-weighted count tells the opposite story about which force matters --- equalising $\hat T$ moves $1/H(\gamma_{r})$ enormously while $\alpha=0$ barely moves it --- and the two readings are consistent rather than in conflict: comparative advantage decides how the euros divide among the zones a buyer reaches, and the variety count decides how many zones that is.
...
\caption{How many upstream commuting zones a shock to each buyer reaches\label{fig:buyer_reach_region}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Equation~\eqref{eq:region_reach}, the expected number of distinct upstream commuting zones supplying each shocked buyer, under each regime; the \emph{reachable} support --- the $N_{s}\to\infty$ limit, i.e.\ the number of zones with $\gamma_{r'rs}>0$ in at least one of the buyer's sectors --- is printed beside each row, and \emph{Infinite varieties} is exactly that support. A zone supplying in several sectors is counted once. Unlike Figure~\ref{fig:buyer_concentration_region} this statistic carries \emph{no} expenditure weights, which is why it is reported: at the estimated variety counts the realised Herfindahl is 83 to 91 per cent an effective number of varieties. The price of that purity is that a zone winning one variety counts as much as one winning forty, so the reach says how many places a shock arrives in and not how much lands in each; the euro-weighted region count is $1/H(\gamma_{r})$, reported in the companion figure. Levels rather than percentage deviations, since the regimes sit within a factor of about two of one another. Regions are ordered by their baseline reach.}
\end{figure}
...
\caption{The concentration of each buyer's supplier portfolio under each scenario\label{fig:buyer_concentration_region}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: The effective number of supplier commuting zones $1/\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ of each shocked commuting zone, from \eqref{eq:granular_buyer}, in levels on a \emph{logarithmic} axis. \emph{Infinite varieties} holds the estimated network $\gamma_{rs}$ fixed and sends $N_{s}\to\infty$, so the distance between that series and \emph{Both forces} is granularity, buyer by buyer, and every other series moves $\gamma_{rs}$ itself. A log axis, and marks rather than bars, because that limit sits about seven times above the realised economy while the two counterfactuals move the same statistic by one and eleven per cent: on a linear or percentage axis the three would be indistinguishable from the baseline, and on a log axis a bar's length would encode the axis limits rather than the number. The compression of the counterfactuals is the finding of the paragraph, not an artefact --- at these variety counts the statistic is 83 to 91 per cent an effective number of varieties, and the two forces enter only through $\gamma_{rs}$. Figure~\ref{fig:buyer_reach_region} reports the count of \emph{places}, which carries no variety weighting. Regions are ordered by their baseline effective number. The counterfactual rows are drawn from the model's own primitives, with one productivity draw per (variety, region) shared across buyers, since it is the same physical variety.}
\end{figure}
