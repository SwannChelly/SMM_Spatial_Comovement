...
\paragraph{Uncertainty.} The counterfactual quantities above are reported as point estimates. They are functions of $\hat\alpha\hat\theta$ and $\hat T$, so they inherit the sampling uncertainty of the estimation, and the standard errors reported in Appendix~\ref{apsec:diffusion_extra} are not that: they measure the cross-sectional agreement of buyers on a common value, holding the parameters fixed at their estimates. \swann{A parametric bootstrap over $(\hat\alpha,\hat T)$, redrawing from the estimated sampling distribution and recomputing each counterfactual, is the standard way to attach an interval to these numbers and is not yet run.}
