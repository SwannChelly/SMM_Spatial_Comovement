...
the share of the response to a shock in $r$ that lands in each upstream region. The first is \textbf{concentration}: the Herfindahl $H_{r}=\sum_{l}\omega_{lr}^{2}$, reported through its reciprocal $1/H_{r}$, the effective number of destination commuting zones. Because its level reflects how many cells a sector has and how the country is shaped before either force acts, we report it \emph{relative} to the same buyer's value under the uniform allocation, so that the number is the concentration the two forces add rather than the concentration the geography supplies. Both statistics are computed on the estimated sourcing probabilities $\rho$, which carry no variety count, so both describe the \emph{average} network rather than any realised one --- and whether that matters depends on the order of the statistic. The average sourcing distance is \emph{linear} in $\omega$, so averaging is exact and the realised distance is the one reported. Concentration is not: $H_{r}$ is a quadratic, hence convex, function of $\omega$, so by Jensen a realised network is strictly more concentrated than the average one, by $\sum_{l}\operatorname{Var}(\omega_{lr})$. The bound is not a matter of degree. In one draw each of a sector's $\hat N_{s}$ varieties is won by exactly one region, so the realised incidence has at most $\sum_{s}\hat N_{s}$ non-zero entries --- of order a hundred cells, which is the same order as the effective destination counts reported below. $\Delta_{r}$ is convex too, in the opposite direction, so the buyer-specificity reported here is a lower bound. \emph{The counterfactual comparisons are unaffected}: every regime is computed with the same variety-free machinery, so granularity enters each of them identically and cancels from the ratios and displacements this section reads. What the caveat qualifies is the \emph{level} of $1/H_{r}$, which describes the average network and overstates the reach of any single one. The second is \textbf{commonality}: how much of a buyer's incidence is specific to that buyer, measured against the common incidence $\bar\omega=\operatorname{mean}_{r}\omega_{r}$ by
