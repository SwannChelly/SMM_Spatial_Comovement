...
\label{eq:alignment}
\end{equation}
an exact property of the Ricardian allocation. What moves sourcing in space is therefore the \emph{covariance} between capability and distance to the buyer, weighted by the sourcing the buyer actually does, and not the dispersion of capability: a $\hat T$ that varies widely but is uncorrelated with the geography of demand reallocates sourcing across regions without sending it any further away. Dispersion only bounds that covariance, $|\operatorname{Cov}|\le \operatorname{sd}(\log T)\operatorname{sd}(\log d)$.

Figure~\ref{fig:spatial_alignment} shows the cloud behind \eqref{eq:alignment} for a representative buyer in each industry: one dot per competing upstream commuting zone, its distance from the buyer against its comparative advantage, with the area of the dot given by the sourcing probability $\rho_{lr}$. The two industries are opposed. In motor vehicles the fitted relationship is steeply downward, $\operatorname{corr}(\log \hat T,\log d) = -0.47$ at the sector median: the productive suppliers are the ones close to the assembly plants, so comparative advantage and proximity pull in the same direction. In aerospace it is nearly flat, $-0.17$: capability is concentrated in a small number of hubs --- Toulouse and Île-de-France --- that are not close to most of the demand, and the productivity premium of those hubs is paid for in kilometres by every buyer except the ones next to them.\footnote{$\hat T$ is the Sinkhorn image of the observed sourcing shares, so this covariance is model-implied rather than read directly off the data. Writing $\log \hat T = \log \hat\gamma^{\,emp} - \log M(\alpha)$, it splits into the gradient of the observed shares and minus that of market access. In aerospace the two contribute about equally. In motor vehicles the observed shares are, if anything, slightly increasing in distance (a covariance of $+0.07$), and the whole of the negative alignment comes from the market-access term: areas far from a given buyer tend to be centrally located overall, so the sales they are observed to make require less comparative advantage to rationalise.}
