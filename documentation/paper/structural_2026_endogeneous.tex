...
The two industries are indistinguishable in the centre of the distribution and opposite in its shape. The median shocked region keeps 5.9\% of its upstream response within 100 km in motor vehicles and 5.8\% in aerospace: effectively the same. What differs is the dispersion around it. In motor vehicles the share ranges from 2.4\% to 18.0\%, and at 200 km no region falls below 15.5\%: shocks diffuse widely, but they diffuse in the same way wherever they start. In aerospace the range is 0.6\% to 39.2\%, and at 200 km the least local region retains just 3.1\%. The higher aerospace mean (8.8\% against 7.4\%) is carried by a single commuting zone, Toulouse, which keeps close to two fifths of its upstream response within 100 km, while a shock to most other aerospace regions is transmitted almost entirely elsewhere. Diffusion in motor vehicles is broad and homogeneous; in aerospace it is hub-and-spoke.

\paragraph{Which force produces this.} Because the sourcing probabilities are closed-form, the same shock can be propagated with one of the two allocative forces switched off, holding fixed how much of the euro leaves for upstream and how it splits across sectors.\footnote{Sourcing shares sum to one within a sector, so switching off one of the two forces does not change the amplification coefficient: it moves a shock in space without changing its size.} The top panel of Figure~\ref{fig:diffusion_counterfactual} reports $L_{r}(d)$ for the median region under each regime; the bottom panel reports the same two switches region by region, at 100~km.

Switching off proximity lowers the local share in both industries, as it must: from 7.3\% to 5.1\% in motor vehicles and from 8.9\% to 4.7\% in aerospace. Comparative advantage is the force whose effect differs across the two industries. In motor vehicles, equalising $T$ across areas \emph{lowers} the local share as well, from 7.3\% to 6.2\%, and sends the average euro 65~km further (348 to 413~km): productive suppliers are located where the buyers are, so comparative advantage and proximity reinforce one another. In aerospace the same operation \emph{raises} the local share, from 8.9\% to 10.2\%, and leaves the average sourcing distance essentially unchanged (355 to 358~km): the median region sources more locally with comparative advantage switched off than with both forces at work. This is the signature of a strong concentration of upstream capability in a few hubs, and the bottom panel confirms it --- shutting comparative advantage down raises the local share of the downstream regions far from the two hubs, and lowers it only for the hubs themselves. A shock originating in Valence, for instance, keeps around 7\% of its upstream response within 100~km, and that share nearly triples once comparative advantage is switched off.

What separates the two industries is not how unequal comparative advantage is, but where it sits. What moves the average sourcing distance when $T$ is equalised is the covariance across competing cells between capability and distance to the buyer, not the dispersion of capability itself: a $T$ that varies widely but is uncorrelated with geography reallocates sourcing without sending it further away. In motor vehicles that covariance is strongly negative --- capability is co-located with the assembly plants --- so the two forces reinforce one another for essentially every buyer. In aerospace the productive areas are remote from most buyers, so $T$ localises sourcing for Toulouse and Paris and disperses it for everyone else; the two nearly cancel in the average distance while still moving the median region's local share by more than a percentage point.
...
\paragraph{Granularity.} The finite number of varieties is not a technical detail of the estimation: it is what makes the previous paragraphs statements about a \emph{realised} network rather than about an average one. Because the identity of a variety's winner is independent of its realised price under Fr\'echet, the \emph{expected} local share is invariant to $N_{s}$ --- the numbers above would be unchanged in the continuum limit, and we verify that they are, to within 0.4 percentage points. Two things are not invariant, and both matter for reading the figures.

First, the support. A downstream firm sources $\sum_{s}\widehat N_{s}$ varieties --- 185 in motor vehicles, 99 in aerospace --- so a shock can reach at most that many distinct origins. It reaches 113 commuting zones in motor vehicles and 62 in aerospace, against 273 and 262 in the continuum limit of the same economy. Three quarters of the geographic reach of the continuum model is an artefact of assuming a continuum, and the loss is larger in aerospace precisely because comparative advantage concentrates sourcing there. The count of regions a shock reaches, unlike the share it delivers, is a granular object.

Second, the dispersion. $L_{r}(100)$ is a random variable whose standard deviation across realisations of the variety lottery is 3.0 percentage points in motor vehicles and 4.7 in aerospace --- of the same order as its mean. The cross-region contrast the map reports is a contrast of expectations and is not affected; but a statement about what one region receives in one year is not, and small shares are the fragile ones, since the relative standard deviation of a share behaves as $\sqrt{(1-p)/(pN)}$. Granularity also raises the amplification coefficient slightly, by 0.010 in motor vehicles and 0.012 in aerospace, because the within-sector price index is convex in the productivity draws and a finite basket is on average dearer; with $\lambda<1$ a dearer input bundle raises the intermediate share. We report all three magnitudes in Appendix~\ref{apsec:granularity}.
...
\caption{Diffusion with one force switched off\label{fig:diffusion_counterfactual}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Top row: $L_{r}(d)$ against $d$ for the median shocked region, under the estimated allocation and under two counterfactual regimes. Bottom row: $L_{r}(100)$ under the same three regimes, one group of bars per shocked downstream commuting zone, ordered by the realised local share. \emph{Distance only} equalises comparative advantage across attraction areas; \emph{Comparative advantage only} sets $\alpha=0$. The realised curve is one draw of the finite-variety economy, whose expectation is \emph{Both forces}; the gap between them is granularity. $D_{r}$ is identical in every regime by construction, so the regimes differ only in where the shock lands, not in how large it is.}
\end{figure}
