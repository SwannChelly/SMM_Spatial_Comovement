...
\paragraph{Trade cost and comparative advantages:} The distribution of regional sourcing share in Figure~\ref{fig:emp_sim_gamma_coef_G0} reveals that for some sectors sourcing is strongly concentrated. Two forces can explain it: the dispersion of comparative advantages or or high trade costs combined with concentrated downstream demand. The model allows us to separate them since in the Ricardian model, two upstream regions $r'$ and $l$ have equal odds of supplying downstream region $r$ when
$$
\log T_{a(r')}-\log T_{a(l)} = \alpha\theta \left(\log d_{r'r} - \log d_{lr}\right).
$$ 
A productivity gap $\Delta$ is exactly offset by a distance ratio $\exp(\Delta/\alpha\theta)$. This gives a common unit --- kilometers --- to compare productivity and distance. We report in Figure~\ref{} for each sector the distance gap that separates the comparative advantage of the most productive area from the median one. 

Two things enter that equivalent distance and are easily conflated: the size of the productivity gap, and the price of distance. The two industries differ mainly in the second. In raw productivity units the gap between the top area and the median one is of the same order in both --- a median of 1.6 log points in motor vehicles against 2.2 in aerospace, with a comparable dispersion across competing cells, $\operatorname{sd}(\log \hat T) = 0.91$ and $0.96$ --- while $\alpha\theta$ is estimated at $0.23$ in motor vehicles and $0.46$ in aerospace. Distance is twice as expensive in aerospace, so the same productivity gap converts into twice as many decades of equivalent distance in motor vehicles. Motor vehicles spans six orders of magnitude in equivalent distance and aerospace fewer than three, then, not because comparative advantage is more extreme in the automotive sector, but because proximity is a weaker constraint on it there. Three automotive sectors --- C33Z (Machinery \& Equipment Repair), G45Z (Motor Vehicle \& Motorcycle Trade), and G46Z (Wholesale Trade, excl. motor vehicles) --- have equivalent distances of $10^3$ to $10^6$ km, far beyond any distance observed in France: here comparative advantage alone drives concentration, regardless of trade costs. These are trade, repair and headquarter-type activities rather than heavy manufacturing, and they cluster overwhelmingly in Île-de-France. The remaining automotive sectors are heavy industrial inputs (Steel Manufacturing, Forging \& Metal Machining, Automotive Equipment Manufacturing) and sit below the benchmark, where trade costs dominate instead. This split is consistent with the geography of the industry: downstream car plants are relatively dispersed across France, so heavy, costly-to-ship inputs are naturally supplied by the nearest capable plant, leaving little room for comparative advantage to override proximity. Trade and headquarter functions face no such constraint --- they can serve the whole country from one location --- so comparative advantage, not distance, governs their concentration. Aerospace shows no such split: most sectors cluster near the benchmark, indicating that comparative advantage and trade costs are of comparable magnitude across the industry, consistent with a downstream industry itself concentrated in a few regions, which weakens the disciplining role of proximity on sourcing.

\paragraph{Where comparative advantage sits:} How large the productivity gaps are says how much of sourcing is decided by capability rather than by proximity. It does not say in which direction capability pulls a shock, and that is a separate statistic. Since $\rho_{lr}\propto T_{a(l)}\,d_{lr}^{-\alpha\theta}$, dialling comparative advantage down from its estimate towards a common value along $\rho_{lr}(t)\propto T_{a(l)}^{\,t}\,d_{lr}^{-\alpha\theta}$ moves the average sourcing distance at the rate
$$
\frac{d}{dt}\,\mathbb{E}_{\rho(t)}\!\left[\log d\right] \;=\; \operatorname{Cov}_{\rho(t)}\!\left(\log T,\log d\right),
$$
an exact property of the Ricardian allocation. What moves sourcing in space is therefore the \emph{covariance} between capability and distance to the buyer, not the dispersion of capability: a $\hat T$ that varies widely but is uncorrelated with the geography of demand reallocates sourcing across regions without sending it any further away. Dispersion only bounds the covariance, $|\operatorname{Cov}|\le \operatorname{sd}(\log T)\operatorname{sd}(\log d)$; alignment is what makes it bite.

The two industries are opposed on this margin. In motor vehicles $\operatorname{corr}(\log \hat T,\log d) = -0.47$ at the sector median: the productive areas are the ones close to the assembly plants, so comparative advantage and proximity pull in the same direction. In aerospace it is $-0.17$: capability is close to orthogonal to the geography of demand, and the productivity premium of Toulouse is paid for in kilometres by every buyer except those next to it.\footnote{$\hat T$ is the Sinkhorn image of the observed sourcing shares, so this covariance is model-implied rather than read directly off the data. Writing $\log \hat T = \log \hat\gamma^{\,emp} - \log M(\alpha)$, it splits into the gradient of the observed shares and minus that of market access. In aerospace the two contribute about equally. In motor vehicles the observed shares are, if anything, slightly increasing in distance (a covariance of $+0.07$), and the whole of the negative alignment comes from the market-access term: areas far from a given buyer tend to be centrally located overall, so the sales they are observed to make require less comparative advantage to rationalise.} Section~\ref{subsec:diffusion} shows that this, and not the dispersion of $\hat T$, is what separates the two industries when a shock is propagated with one force switched off.
...
\caption{Diffusion with one force switched off\label{fig:diffusion_counterfactual}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Top row: $L_{r}(d)$ against $d$ for the median shocked region, under the estimated allocation and under the two counterfactual regimes. Bottom row: $L_{r}(100)$ under the same regimes, one group of bars per shocked downstream commuting zone, ordered by the realised local share. \emph{Distance only} equalises comparative advantage across attraction areas; \emph{Comparative advantage only} sets $\alpha=0$. The realised curve is the finite-variety draw of the estimated economy, whose closed-form expectation is \emph{Both forces}; the gap between them is granularity. }
\end{figure}
