...
The average sourcing distance says how far a shock travels, not where it lands. A buyer sourcing at 350~km may be drawing on a ring of neighbours or on a single distant hub, and for the stability of the industry the two are not the same economy. What distinguishes them are two questions, and they are the same object read from its two sides. From the buyer's side: \emph{how concentrated is a buyer's portfolio of suppliers?} From the supplier's side: \emph{how concentrated is a supplier region's portfolio of customers?} If sourcing is buyer-specific, each region sells to its own set of buyers and its fortunes track theirs; if it is not, one region sells to every buyer in the industry, which maximises its exposure to shocks common to the industry and minimises its exposure to shocks hitting any one buyer. We answer the first question buyer by buyer, and the second by comparing buyers.

\paragraph{The portfolio of a single buyer.}
We start from the object a single shock acts on, and we keep the realisation of the network explicit throughout, because with a finite number of varieties the network is drawn and not solved. Let $\Omega$ denote one realisation --- one assignment of each of the $N_{s}$ varieties of every sector to the region that wins it, for every buyer. The portfolio of sector-$s$ suppliers of buyer $r$ in that realisation is the vector of shares
\begin{equation}
\omega_{rs}(\Omega)=\left(\frac{X_{r'rs}(\Omega)}{\sum_{r'}X_{r'rs}(\Omega)}\right)_{r'},
\label{eq:incidence_vector}
\end{equation}
which is also the incidence vector of a shock to that buyer: the share of the upstream response landing in each supplier region. Its concentration is the Herfindahl index
$$H_{rs}(\Omega)=\sum_{r'}\omega_{r'rs}^{2}(\Omega)=\big\|\omega_{rs}(\Omega)\big\|^{2},$$
whose inverse is the \emph{effective number} of supplier regions the shock reaches; a buyer's overall figure aggregates its sectors with its own purchase shares, $H_{r}(\Omega)=\sum_{s}(X_{rs}/X_{r})H_{rs}(\Omega)$. We take it sector by sector before aggregating because a buyer's shock hits every one of its input sectors and each sector has its own supplier geography.

This answers the first question, buyer by buyer, in the same currency as the average sourcing distance and the local share. But unlike those two, $H$ is a \emph{quadratic} statistic of the network, and that is what makes granularity a term of it rather than a caveat on it.

\paragraph{What a finite number of varieties adds.}
Write $\mathbb{E}_{\Omega}$ for the expectation over realisations, so that $\mathbb{E}_{\Omega}[\omega_{rs}(\Omega)]=\gamma_{rs}$ is the expected portfolio --- the regular Ricardian allocation of \eqref{eq:gamma_rrs}, with no $\Omega$ in it. Completing the square coordinate by coordinate gives
\begin{equation}
...
\end{equation}
the concentration of the expected network plus the dispersion of a realisation around it. The second term is strictly positive whenever $N_{s}$ is finite: in any realisation each variety is won by exactly one region, so $\omega_{rs}(\Omega)$ has at most $N_{s}$ non-zero entries however diffuse $\gamma_{rs}$ is. The $N_{s}\to\infty$ limit of the same buyer's portfolio is $\|\gamma_{rs}\|^{2}$, so the gap between the two is granularity measured in the units of $H$ itself.

The mechanism is clearest when varieties carry equal expenditure weights. Writing $\omega_{r'rs}(\Omega)=\sum_{j=1}^{N_{s}}v_{jrs}(\Omega)\,I_{jr's}(\Omega)$, with $v_{jrs}$ the share of buyer $r$'s sector-$s$ spending going to variety $j$ and $I_{jr's}=\mathbf 1\{r'\text{ wins }j\text{ for }r\}$, suppose first that $v_{jrs}=1/N_{s}$. Then $N_{s}\omega_{r'rs}(\Omega)$ is simply the number of varieties region $r'$ wins, which is $\operatorname{Binomial}(N_{s},\gamma_{r'rs})$, and
\begin{equation}
\mathbb{E}_{\Omega}\big[H_{rs}(\Omega)\big]=H(\gamma_{rs})+\frac{1}{N_{s}}\big[1-H(\gamma_{rs})\big].
\label{eq:granular_binomial}
\end{equation}
A finite $N_{s}$ makes winner counts random, the variance of a count is mechanically positive, and a variance of shares is exactly what a Herfindahl adds up.\footnote{For $K\sim\operatorname{Bin}(N,\gamma)$, $\mathbb{E}[K^{2}]=N\gamma(1-\gamma)+N^{2}\gamma^{2}$, so $\mathbb{E}[(K/N)^{2}]=\gamma^{2}+\gamma(1-\gamma)/N$. Summing over $r'$ and using $\sum_{r'}\gamma_{r'rs}=1$ gives \eqref{eq:granular_binomial}.} What granularity adds is proportional to $1-H(\gamma_{rs})$: it concentrates whatever dispersion the expected network has left to concentrate. Granularity and comparative advantage are therefore \textbf{substitutes} as sources of concentration, and the prediction is buyer-level and testable --- the granular term must be smallest exactly where the structural one is largest.

Under CES a cheaper winner takes a larger share, so $N_{s}\omega_{r'rs}(\Omega)$ is a \emph{weighted} sum of winner indicators rather than a count and \eqref{eq:granular_binomial} is not literally a binomial second moment. The argument survives intact with $1/N_{s}$ replaced by the \emph{effective number of varieties} $\mathcal{V}_{rs}=\mathbb{E}_{\Omega}[\sum_{j}v_{jrs}^{2}(\Omega)]$:
\begin{equation}
\mathbb{E}_{\Omega}\big[H_{rs}(\Omega)\big]=\mathcal{V}_{rs}+\big(1-\mathcal{V}_{rs}\big)H(\gamma_{rs}),
\qquad
\mathbb{E}_{\Omega}\big[H_{r}(\Omega)\big]=\sum_{s}\frac{X_{rs}}{X_{r}}\,\mathbb{E}_{\Omega}\big[H_{rs}(\Omega)\big].
\label{eq:granular_buyer}
\end{equation}
Under Fr\'echet the identity of a variety's winner is independent of the price at which it is won, so expenditure weights and win locations are independent and \eqref{eq:granular_buyer} is exact rather than an approximation.\footnote{Equal expenditure shares give $\mathcal V_{rs}=N_s\cdot N_s^{-2}=1/N_s$, recovering \eqref{eq:granular_binomial}; by Cauchy--Schwarz $\mathcal{V}_{rs}\ge 1/N_{s}$ always, so reading granularity as a count \emph{understates} it. The model predicts $\mathcal{V}_{rs}$ to be the same for every buyer --- the Fr\'echet scale cancels in an expenditure share --- and that is a testable restriction rather than an assumption; measured across buyers its range does not exceed $0.013$ in either industry. The closed form for $\mathcal V$ has no content at this calibration: with Fr\'echet productivity and CES demand the variety weights are Pareto with tail index $\kappa_{s}=\theta/(\nu_{s}-1)$, and $\mathcal V$ would be $\Xi(\kappa_{s})/N_{s}$ with $\Xi(\kappa)=\Gamma(1-2/\kappa)/\Gamma(1-1/\kappa)^{2}$; at $\theta=1$ and $\nu_{s}=1.5$ this gives $\kappa_{s}=2$ \emph{exactly}, the boundary at which $\Xi$ diverges, so the variance of the variety weights is not finite and a single variety can carry a non-vanishing share of a buyer's sector spending. We measure $\mathcal V$ from the draws instead.}

\paragraph{How many regions, and how concentrated among them.}
Equation~\eqref{eq:granular_buyer} has three inputs and each is a force we can switch off: comparative advantage and trade costs both enter through $\gamma_{rs}$, and the variety count enters through $\mathcal{V}_{rs}$ alone. Before any counterfactual is read, however, one property of $\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ has to be stated, because it decides what the statistic can be asked. Measured at the estimate, the first term of \eqref{eq:granular_buyer} carries \textbf{87 per cent} of $\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ in motor vehicles and \textbf{76 per cent} in aerospace. The reciprocal of a realised Herfindahl is therefore an effective number of \emph{varieties} with a region label attached to it --- $1/\mathcal{V}_{s}$ is $9.7$ and $6.4$, against realised effective numbers of $8.5$ and $4.8$ --- and the two forces, which enter through $\gamma_{rs}$ and nothing else, can move only the remaining quarter or tenth.

That is a property of the object rather than of this calibration: a Herfindahl weights regions by the euros they collect, and with finitely many varieties those euros concentrate on whichever few varieties happen to be cheap. The count of \emph{places} is a different statistic and carries no expenditure weights at all. Region $r'$ loses every one of the $N_{s}$ varieties of sector $s$ with probability $(1-\gamma_{r'rs})^{N_{s}}$, and variety draws are independent across sectors, so the expected number of distinct upstream commuting zones supplying buyer $r$ is
\begin{equation}
\mathbb{E}_{\Omega}\Big[\#\big\{r':\,r'\text{ supplies }r\big\}\Big]
=\sum_{r'}\Big[1-\prod_{s}\big(1-\gamma_{r'rs}\big)^{N_{s}}\Big],
\label{eq:region_reach}
\end{equation}
exact under the same independence that gives \eqref{eq:granular_binomial}, and bounded above by the total number of varieties the buyer purchases, $\sum_{s}N_{s}$. We report it beside the effective number of the \emph{expected} network, $1/H(\gamma_{r})$, which is euro-weighted and equally free of $\mathcal{V}$. The two answer different questions and neither replaces the other: \eqref{eq:region_reach} counts a zone winning one variety the same as one winning forty, so it says where a shock \emph{arrives}; $1/H(\gamma_{r})$ says how concentrated the euros are among the zones it arrives at.

\paragraph{The buyer-level portfolio in the data.}
Figure~\ref{fig:buyer_reach_region} reports \eqref{eq:region_reach} for every shocked commuting zone under the same two counterfactuals as Figure~\ref{fig:counterfactual_distance_region}, and Figure~\ref{fig:buyer_concentration_region} reports $1/\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ beside its $N_{s}\to\infty$ limit. Three readings follow.

\textbf{A shock arrives in about two fifths of the country in motor vehicles and a quarter of it in aerospace, and the binding constraint is the variety count rather than the geography.} The median motor-vehicle buyer reaches \textbf{112} of the 275 modelled commuting zones and the median aerospace buyer \textbf{63}, so a continuum of varieties would spread the same shock over $2.4$ and $4.4$ times as many places.\footnote{This is the object the earlier realisation-based count estimated by taking the union of origins in a simulated economy, which gave 113 and 62; \eqref{eq:region_reach} is a closed form and agrees with it to within a zone, which is a check of two routes sharing no code. The ceiling is not the support but $\sum_{s}N_{s}$: a motor-vehicle buyer purchases $185$ varieties in total and an aerospace buyer $99$, so the reach can never approach 275 at finite $N_{s}$ however diffuse $\gamma_{rs}$ becomes, and the realised reaches sit at $61\%$ and $63\%$ of their own ceilings. That is why the \emph{Infinite varieties} series of Figure~\ref{fig:buyer_reach_region}, which is the support, is not on the same footing as the three regimes beside it: it is a limit no finite-variety economy can approach, and the reachable-zone count printed beside each row should be read as that limit rather than as a target.}

\textbf{On places, trade costs do almost all of the work.} Both counterfactuals flatten $\gamma_{rs}$ and therefore spread the win probabilities over more zones, so both raise the reach, as \eqref{eq:region_reach} requires. But equalising $\hat T$ raises it by $14\%$ in motor vehicles and $23\%$ in aerospace, while setting $\alpha=0$ raises it by only $2.0\%$ and $1.1\%$.\footnote{Under $\alpha=0$ the reach is \emph{identical} for every buyer to machine precision ($114.32$ and $63.47$ in the two industries, with zero cross-buyer spread), and likewise under the unselective benchmark. That is the section's internal control rather than a rendering artefact: the distance factor is the only buyer-specific term in $\gamma_{r'rs}$, so with equal distances every buyer faces the same sourcing shares and hence the same reach.}

\textbf{The euro-weighted count agrees on the ranking and disagrees on the magnitude, and that disagreement is the economics.} Equalising $\hat T$ multiplies $1/H(\gamma_{r})$ by $2.6$ (motor vehicles) and $3.1$ (aerospace) while multiplying the reach by only $1.14$ and $1.23$. Comparative advantage therefore governs how the euros \emph{divide} among the zones a buyer reaches and barely governs how many zones there are; what sets that number is the variety count, through the ceiling of the previous footnote. The realised effective number inherits the same contamination and should not be read as a statement about the network at all: switching off comparative advantage moves it by $-1.8\%$ in motor vehicles and $+1.8\%$ in aerospace --- opposite signs, and in motor vehicles the opposite sign from the structural count it is supposed to track.\footnote{The reversal is $\mathcal{V}$, not the network: $\alpha=0$ lowers $H(\gamma_{rs})$, which should raise the effective number, but it also makes every buyer face the same ranking of varieties and so raises $\mathcal{V}$ slightly ($0.103\to0.106$ in motor vehicles), and at $87\%$ weight the second channel wins. Any reading of the realised effective number across regimes should be taken from the structural column beside it.}

One further feature of Figure~\ref{fig:buyer_concentration_region} is worth naming because it is a result and not a defect of the drawing: \textbf{granularity makes buyers look alike}. The realised effective number ranges over $2\%$ of its median across motor-vehicle buyers and $14\%$ across aerospace ones, against $16\%$ and $37\%$ for the structural number, so the cross-buyer heterogeneity the expected network carries is largely washed out in any one year. The mechanism is the one the model predicts: $\mathcal{V}_{s}$ is common across buyers --- its measured cross-buyer range does not exceed $0.0052$ --- and it is most of what the realised statistic is made of.
\paragraph{Why the buyer-level index is not enough.}
Two buyers can each source narrowly and still describe opposite economies. They may source narrowly from \emph{different} regions, in which case each supplier region holds its own clientele and shocks to different buyers offset one another. Or they may pile onto the \emph{same} regions, in which case those regions hold undiversified clienteles and buyer shocks accumulate. $H_{rs}(\Omega)$ is identical in the two cases, and so is $\mathbb{E}_{\Omega}[H_{r}(\Omega)]$. Separating them is the second of the two questions --- the supplier's portfolio of customers --- and it requires comparing buyers rather than describing them one at a time.

\paragraph{Aggregating across buyers.}
The concentration faced by the industry as a whole is $\bar H_{s}(\Omega)=\mathbb{E}_{r}[H_{rs}(\Omega)]$, where $\mathbb{E}_{r}$ weights each buyer by its share $X_{rs}/X_{\cdot s}$ of the sector's purchases, with $X_{\cdot s}=\sum_{k}X_{ks}$. The weights are not a smoothing device: a euro of demand is more likely to hit a large buyer than a small one, so weighting by purchases is the statement that the shock is proportional to size, and it is also the only aggregator under which the identity below holds exactly.\footnote{$H$ is averaged before being inverted into an effective number, since $1/H$ is convex; the buyer-level effective numbers of Figure~\ref{fig:buyer_concentration_region} therefore do not average to the aggregate figure. The cross-buyer distributions behind these means are in Appendix~\ref{app:incidence_buyer}.}

\emph{Across buyers: common versus buyer-specific exposure.} Fix a realisation $\Omega$ and let $\bar\omega_{s}(\Omega)=\mathbb{E}_{r}[\omega_{rs}(\Omega)]$ be the incidence of a shock hitting the industry as a whole. Then
\begin{equation}
\bar H_{s}(\Omega)\;=\;\underbrace{\big\|\bar\omega_{s}(\Omega)\big\|^{2}}_{\text{common}}
\;+\;\underbrace{\mathbb{E}_{r}\big\|\omega_{rs}(\Omega)-\bar\omega_{s}(\Omega)\big\|^{2}}_{\text{buyer-specific}},
\label{eq:incidence_identity}
\end{equation}
the variance decomposition applied coordinate by coordinate. A shock to a single buyer lands with concentration $\bar H_{s}(\Omega)$ on average; a shock to the whole industry lands with $\|\bar\omega_{s}(\Omega)\|^{2}$, which is smaller exactly to the extent that buyers send their euros to different places. The first term is the part of a buyer's exposure that every other buyer shares, the second the part that is its own. This is the split the buyer-level index could not deliver.

\emph{The two splits commute.} Equation~\eqref{eq:incidence_identity} is an average over buyers at a fixed realisation and \eqref{eq:variety_split} an expectation over realisations at a fixed buyer, so applying both gives four additive terms, each a squared norm:
\begin{equation}
\mathbb{E}_{\Omega}\big[\bar H_{s}(\Omega)\big]
...
+\underbrace{\mathbb{E}_{\Omega}\big\|\bar\omega_{s}(\Omega)-\bar\gamma_{s}\big\|^{2}}_{\text{(iii) granular, common}}
+\underbrace{\Big[\mathbb{E}_{\Omega}\,\mathbb{E}_{r}\big\|\omega_{rs}(\Omega)-\gamma_{rs}\big\|^{2}-\text{(iii)}\Big]}_{\text{(iv) granular, specific}} .
\label{eq:four_cells}
\end{equation}
Each term reads as a variance, and that is what makes the split economic rather than accounting. Cells (i) and (ii) sum to the buyer-level structural term of \eqref{eq:variety_split}, averaged; cells (iii) and (iv) sum to its granular term, so Figure~\ref{fig:buyer_concentration_region} is the vertical of this table and the horizontal is new. Cell (iii) is the variance of the \emph{industry-level} incidence: the part of chance that survives aggregation over buyers, because the same varieties happen to be won for everyone. Cell (iv) is the average variance minus the variance of the average --- the part that diversifies away when the industry is aggregated --- and is non-negative by convexity, with equality only if every buyer's realised deviation is the same vector.\footnote{Both granular terms are evaluated from the variety kernel \eqref{eq:variety_kernel} below. Cell (iii) $=\sum_{r,r''}\tfrac{X_{rs}}{X_{\cdot s}}\tfrac{X_{r''s}}{X_{\cdot s}}\operatorname{Cov}_{\Omega}(\omega_{rs},\omega_{r''s})$, and $\operatorname{Cov}_{\Omega}(\omega_{rs},\omega_{r''s})=\mathbb{E}_{\Omega}[\omega_{rs}\!\cdot\!\omega_{r''s}]-\gamma_{rs}\!\cdot\!\gamma_{r''s}=\mathcal V_{rr''s}(Q_{rr''s}-\Gamma_{rr''s})$. The total granular term is the same expression restricted to $r''=r$ and averaged, $\mathbb{E}_r[\mathcal V_{rs}(1-H(\gamma_{rs}))]$, using $Q_{rr}=1$, which is \eqref{eq:granular_buyer}; cell (iv) is the difference. Non-negativity of (iv) is $\mathbb{E}_r\|x_r\|^2\ge\|\mathbb{E}_r x_r\|^2$ at $x_r=\omega_{rs}(\Omega)-\gamma_{rs}$.}

\paragraph{Two objects, and no others.}
The buyer-level formula \eqref{eq:granular_buyer} fixes the \emph{total} granular term, $\mathcal{V}_{s}[1-\mathbb{E}_{r}H(\gamma_{rs})]$. How that total divides between cells (iii) and (iv) is a statement about \emph{pairs} of buyers, and it needs one more object: whether a realisation gives two buyers the \emph{same} variety in the same region --- call that probability $Q_{rr''s}$ --- rather than landing them in the same region through different varieties, which the sourcing shares of \eqref{eq:gamma_rrs} already imply at rate $\Gamma_{rr''s}=\gamma_{rs}\!\cdot\!\gamma_{r''s}$. The same argument that gave \eqref{eq:granular_buyer} extends to any pair,
\begin{equation}
\mathbb{E}_{\Omega}\big[\omega_{rs}(\Omega)\!\cdot\!\omega_{r''s}(\Omega)\big]
...
\end{equation}
with $\mathcal{V}_{rr''s}=\mathbb{E}_{\Omega}[\sum_{j}v_{jrs}v_{jr''s}]$; setting $r''=r$ returns \eqref{eq:granular_buyer}.\footnote{Winners are independent across varieties, so for $j\neq k$ the two buyers land in the same region with probability $\gamma_{rs}\!\cdot\!\gamma_{r''s}$. Expanding $\omega_{rs}\!\cdot\!\omega_{r''s}$ and separating $j=k$ from $j\neq k$ gives \eqref{eq:variety_kernel}.}

The finite number of varieties therefore enters through \textbf{two objects and no others}. $\mathcal{V}$ governs \emph{how much} a realisation concentrates a shock --- that is the whole of the buyer-level paragraph --- and the excess co-sourcing $Q-\Gamma$ governs \emph{whether that concentration is shared} across buyers. Were win events independent across buyers, $Q=\Gamma$, every cross-buyer covariance would vanish and cell (iii) with it: granularity would be purely idiosyncratic and would diversify the industry's exposure. $Q>\Gamma$ is positive dependence, and it compounds it instead.

Two ratios summarise \eqref{eq:four_cells}. $C^{\text{str}}_{s}=\text{(i)}/[\text{(i)}+\text{(ii)}]$ is the share of the \emph{structural} term that aggregating across buyers cannot diversify away, and $C^{\text{gran}}_{s}=\text{(iii)}/[\text{(iii)}+\text{(iv)}]$ the same share of the \emph{granular} term.

\paragraph{What the data could have delivered on its own.}
Only one aggregate of this object is observed. The surveys record whether a region supplies the downstream industry, not which buyer it serves and not how much, so the geography of supplier \emph{presence} at the industry level is close to cell (i) and everything else is a model prediction. Two gaps separate even those two: the incidence vector is value-weighted while the survey is a count, and under CES the two differ systematically. The split of a buyer's exposure into common and specific parts is a statement about pairs of buyers, which no moment in the estimation looks at, and the supplier's customer portfolio is unobserved altogether, since we have no supplier-side breakdown of sales by buyer. This is what the model buys, and equally where it is exposed.
...
\textbf{The granular cells dominate the total.} In a single year a motor-vehicle shock reaches an effective \textbf{8.2} supplier regions and an aerospace shock \textbf{4.9}, against 58 and 18 in expectation: cells (iii) and (iv) carry \textbf{86\%} and \textbf{72\%} of realised concentration. Two measured inputs set that magnitude. Expenditure is far from evenly spread across varieties --- $\mathcal{V}_{s}N_{s}$ has a median of $1.81$ in motor vehicles and $1.63$ in aerospace, so the effective number of varieties is about $0.6N_{s}$ and the multinomial reading of \eqref{eq:granular_binomial} would understate realised concentration by 40 to 45 per cent. And the $1-H(\gamma_{rs})$ factor makes granularity and comparative advantage substitutes, which the cross-sector spread confirms: in aerospace granularity accounts for 96\% of realised concentration in C33Z, whose expected network spreads over an effective hundred regions on five varieties, against 57\% in C30C, the sector with the most concentrated comparative advantage.
...
\begin{figure}[H]
    \centering
\caption{How many upstream commuting zones a shock to each buyer reaches\label{fig:buyer_reach_region}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/buyer_reach_region_auto_mu2.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/buyer_reach_region_aero_mu2.pdf}
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Equation~\eqref{eq:region_reach}, the expected number of distinct upstream commuting zones supplying each shocked buyer, under each regime; the \emph{reachable} support --- the $N_{s}\to\infty$ limit, i.e.\ the number of zones with $\gamma_{r'rs}>0$ in at least one of the buyer's sectors --- is printed beside each row, and \emph{Infinite varieties} is exactly that support. A zone supplying in several sectors is counted once. Unlike Figure~\ref{fig:buyer_concentration_region} this statistic carries \emph{no} expenditure weights, which is why it is reported: at the estimated variety counts the realised Herfindahl is 87 per cent (motor vehicles) and 76 per cent (aerospace) an effective number of varieties. The price of that purity is that a zone winning one variety counts as much as one winning forty, so the reach says how many places a shock arrives in and not how much lands in each; the euro-weighted region count is $1/H(\gamma_{r})$, reported in the companion figure. Levels rather than percentage deviations, since the three finite-variety regimes sit within a quarter of one another. Regions are ordered by their baseline reach. \emph{Two features to read correctly.} The binding ceiling is $\sum_{s}N_{s}$ --- 185 varieties in motor vehicles and 99 in aerospace --- and not the 275-zone support, so \emph{Infinite varieties} is a limit no finite-variety regime can approach and the annotation should be read as that limit rather than as a target; against their own ceilings the realised reaches are $61\%$ and $63\%$. And the two regimes with $\alpha=0$ are flat across buyers to machine precision, because the distance factor is the only buyer-specific term in $\gamma_{r'rs}$; that is the internal control, not a failure to vary.}
\end{figure}

\begin{figure}[H]
    \centering
\caption{The concentration of each buyer's supplier portfolio under each scenario\label{fig:buyer_concentration_region}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: The effective number of supplier commuting zones $1/\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ of each shocked commuting zone, from \eqref{eq:granular_buyer}, in levels on a \emph{logarithmic} axis. \emph{Infinite varieties} holds the estimated network $\gamma_{rs}$ fixed and sends $N_{s}\to\infty$, so the distance between that series and \emph{Both forces} is granularity, buyer by buyer, and every other series moves $\gamma_{rs}$ itself. A log axis, and marks rather than bars, because that limit sits $6.9$ times (motor vehicles) and $3.7$ times (aerospace) above the realised economy while the two counterfactuals move the same statistic by $+8.2\%$ and $-1.8\%$ (motor vehicles) and $+21.5\%$ and $+1.8\%$ (aerospace): on a linear or percentage axis the three would be indistinguishable from the baseline, and on a log axis a bar's length would encode the axis limits rather than the number. The compression is the finding of the paragraph, not an artefact --- at these variety counts the statistic is 87 and 76 per cent an effective number of varieties, and the two forces enter only through $\gamma_{rs}$. For the same reason the \emph{Comparative advantage only} series can move in the opposite direction from the structural count it tracks, as it does in motor vehicles; the sign there is $\mathcal{V}$, not the network. Figure~\ref{fig:buyer_reach_region} reports the count of \emph{places}, which carries no variety weighting. Regions are ordered by their baseline effective number. The counterfactual rows are drawn from the model's own primitives, with one productivity draw per (variety, region) shared across buyers, since it is the same physical variety.}
\end{figure}
