...
The two forces divide the work in opposite ways, and one number states it. In motor vehicles it is \emph{comparative advantage} that keeps the euro close: equalising $\hat T$ across areas sends it from 348 to 413~km, a 19\% increase, while switching off distance costs only 23~km (348 to 371~km). Removing the dispersion of capability destroys more proximity than removing the trade cost itself does. In aerospace the assignment is reversed: equalising $\hat T$ moves the euro by 2~km (355 to 358~km), essentially nothing, whereas switching off distance sends it 34~km further (355 to 389~km). There, proximity is doing the localising on its own.
...
be the share of the upstream response accruing within $d$ kilometres of the origin of the shock. We report it at $d=200$~km, the radius at which the reduced-form evidence of the first part of the paper is measured, and map it in Figure~\ref{fig:local_share_map}.
