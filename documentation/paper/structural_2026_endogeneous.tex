...
The average sourcing distance says how far a shock travels, not where it lands. A buyer sourcing at 350~km may be drawing on a ring of neighbours or on a single distant hub, and for the stability of the industry the two are not the same economy. What distinguishes them is a pair of questions. \emph{How few places does one buyer's shock reach?} And \emph{are they the same places for every buyer?} A supplier region serving every buyer aggregates the industry's shocks onto itself; distinct local pockets diversify them.
...
Concentration is measured by the Herfindahl of the incidence vector, $H_{rs}=\sum_{l}\omega_{lrs}^{2}$, whose inverse $1/H_{rs}$ is the \emph{effective number} of supplier regions a shock reaches; a buyer's overall figure aggregates its sectors with its own input mix, $H_{r}=\sum_{s}\theta_{rs}H_{rs}$. Because the level of $H$ depends on how many regions the sector has and on the shape of the country before any economic force acts, we report it relative to the same statistic under an unselective allocation, exactly as we normalised the sourcing distance.
...
Table~\ref{tab:incidence} places the industries on both coordinates. A shock to a motor-vehicle buyer spreads over an effective \textbf{58} supplier commuting zones, \textbf{38\%} of what an unselective allocation would give it; a shock to an aerospace buyer over \textbf{18}, only \textbf{28\%}. On the second coordinate both industries are overwhelmingly common: $C=0.985$ in motor vehicles and $0.954$ in aerospace, so between 95 and 99 per cent of a buyer's exposure is exposure that every other buyer in the industry shares. Aerospace is concentrated \emph{and} common --- the hub-and-spoke corner --- while motor vehicles is a dispersed common pool. Neither industry is made of local pockets, and that is a statement about risk: shocks to different buyers in the same industry pile up on the same suppliers rather than diversifying each other, and they do so three times more tightly in aerospace.
...
Everything so far describes the expected network. With a finite number of varieties per sector the realised network is far more concentrated, and by an amount the model predicts: since each variety is won by exactly one region,
\begin{equation}
\mathbb{E}\big[H_{rs}\big]\;\simeq\;H^{E}_{rs}+\frac{1-H^{E}_{rs}}{N_{s}},
\label{eq:granular_H}
\end{equation}
where $H^{E}_{rs}$ is the concentration of the expected network.\footnote{Exact when varieties carry equal expenditure shares: with $\omega_{l}=\sum_{\rho}w_{\rho}\mathbf{1}\{l\text{ wins }\rho\}$ and independent draws, $\mathbb{E}[H]=H^{E}+(1-H^{E})\sum_{\rho}w_{\rho}^{2}$, which is \eqref{eq:granular_H} at $w_\rho=1/N_s$. Under CES the cheaper winners take larger expenditure shares, so $\sum_\rho w_\rho^2$ exceeds $1/N_s$ --- on the estimates by a factor of about $1.7$, i.e. an effective variety count of roughly $0.6N_{s}$ --- and \eqref{eq:granular_H} understates the realised concentration by 40 to 47 per cent. The table reports the realised quantity, not the approximation.}
Taking expectations of \eqref{eq:incidence_identity} splits a realised shock's concentration into four additive parts: common and buyer-specific, each structural or granular. Figure~\ref{fig:concentration_decomposition} reports the split sector by sector; these are predictions rather than fitted quantities, since bilateral links are not observed.

The magnitudes are large. In a single year a motor-vehicle shock reaches an effective \textbf{8.5} supplier regions and an aerospace shock \textbf{4.7}, against 58 and 18 in expectation: \textbf{85\% and 73\% of realised concentration is granular}, not structural. Two regularities organise the sector-level picture, and both are implications of \eqref{eq:granular_H}. Granularity and comparative advantage are \textbf{substitutes}: the term granularity adds is proportional to $1-H^{E}$, so it matters most where the expected network is diffuse and least where a hub already holds the mass --- in aerospace it accounts for 96\% of realised concentration in C33Z, whose expected network is spread and which carries only five varieties, against 58\% in C30C, the sector with the most concentrated capability. Granularity and distance are \textbf{complements}: chance winners differ across buyers only in so far as buyers are far enough apart to rank regions differently, so the granular share of buyer-specificity follows the same $(\alpha\theta)^{2}$ law as the structural one, $0.024$ in motor vehicles against $0.064$ in aerospace.

The economic reading is that granularity reinforces the industry's exposure rather than diversifying it. A realised network is much more concentrated than the expected one, but what it concentrates on is still, overwhelmingly, common across buyers: $C$ is $0.978$ and $0.941$ on realised networks, barely below its structural value. Where trade costs are low, the varieties that happen to be won are won by the same regions for everybody.
...
\begin{table}[H]
    ...
    \begin{tabular}{llccc}
    \hline\hline
    & & \multicolumn{2}{c}{Concentration} & Commonality \\
    \cline{3-4}\cline{5-5}
    Industry & Regime & $1/\bar H$ & rel.\ unselective & $C$ \\
    \hline
    \multicolumn{5}{l}{\emph{Panel A. Expected network}}\\
    Motor vehicles & Both forces                & $58.3$  & $0.38$ & $0.985$ \\
                   & Distance only              & $149.8$ & $0.98$ & $0.983$ \\
                   & Comparative advantage only & $61.9$  & $0.40$ & $1.000$ \\
                   & Unselective benchmark      & $153.4$ & $1.00$ & $1.000$ \\[0.3em]
    Aerospace      & Both forces                & $17.7$  & $0.28$ & $0.954$ \\
                   & Distance only              & $54.7$  & $0.88$ & $0.884$ \\
                   & Comparative advantage only & $17.9$  & $0.29$ & $1.000$ \\
                   & Unselective benchmark      & $62.4$  & $1.00$ & $1.000$ \\
    \hline
    \multicolumn{5}{l}{\emph{Panel B. Realised network} (finite varieties, mean over 50 draws)}\\
    Motor vehicles & Both forces                & $8.5$  & --- & $0.978$ \\
    Aerospace      & Both forces                & $4.7$  & --- & $0.941$ \\
    \hline\hline
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: $\bar H=\sum_{s}\theta_{rs}\bar H_{s}$ is the concentration of the incidence vector, averaged over buyers with their share of each sector's purchases and over sectors with the buyer's input mix, and inverted afterwards; $1/\bar H$ is the effective number of supplier commuting zones one shock reaches. \emph{rel.\ unselective} divides it by the same statistic under an allocation in which neither force discriminates among regions ($\alpha=0$ and $\hat T$ equalised); it is a geometric reference, not a counterfactual economy. $C=1-M/\bar H$ is the share of a buyer's concentration that a shock to the whole industry would also carry, from the decomposition \eqref{eq:incidence_identity}. \emph{Comparative advantage only} sets $\alpha=0$ and is the internal control: with equal distances, every buyer's incidence is the same vector and $C=1$ holds to machine precision. Panel B recomputes the same statistics on each realised finite-variety network and averages across realisations; the gap to Panel A is granularity, decomposed in Figure~\ref{fig:concentration_decomposition}.}
\end{table}
