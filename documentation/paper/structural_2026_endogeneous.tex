...
Since the average sourcing distance is a linear statistic of the network, only comparative advantage and trade costs influence it. We switch them off one at a time: equalising comparative advantage across areas (Distance only) or setting $\alpha=0$ (Comparative advantage only). In both counterfactuals, sectoral spending $X_{rs}$ is held fixed and only its allocation across regions changes. Writting:
...
The two expressions have very different implications. Since $d$ and $\log d$ always move together, the covariance in \eqref{eq:distance_rate} is positive for every buyer and sector: trade costs always shorten the diffusion, and removing them lengthens it everywhere. Comparative advantage has no such sign restriction. By \eqref{eq:alignment}, $\hat T$ affects the sourcing distance only through its alignment with the buyer's geography. Where productive suppliers sit close to the buyer ($\operatorname{Cov}<0$), comparative advantage shortens the diffusion, and equalising it sends the euro further. Where they sit far away ($\operatorname{Cov}>0$), comparative advantage lengthens it. The dispersion of $\hat T$ matters only through this covariance: a highly unequal $\hat T$ that is unrelated to the buyer's geography reallocates sourcing across regions without changing how far the euro travels.

Figure~\ref{fig:alignment_covariance} reports the distribution of the covariance in \eqref{eq:alignment} across sector $\times$ buyer pairs, in kilometres per log point of comparative advantage. Trade costs shorten the diffusion, by 23~km (6.6\%) in motor vehicles and 34~km (9.6\%) in aerospace, and they do so for every buyer in both --- all 22 motor-vehicle and all 18 aerospace buyers source further once $\alpha$ is set to zero, as \eqref{eq:distance_rate} requires.

Comparative advantage is where the two industries are different. Both distributions sit on the negative side --- a median of $-48$~km per log point in motor vehicles and $-11$ in aerospace --- but they differ in how much of the mass lies beyond zero, and in how far it lies. Only 19\% of motor-vehicle sector $\times$ buyer pairs carry a positive covariance, against 42\% of aerospace ones, and the aerospace support is both wider and nearly symmetric: a tenth-to-ninetieth-percentile range of $[-92,\,+96]$~km against $[-126,\,+18]$ in motor vehicles. In motor vehicles almost every pair pulls sourcing towards the buyer. In aerospace two fifths pull it away, and by as much as the rest pull it in.

Figure~\ref{fig:counterfactual_distance_region} report the counterfactuals for each buyer as a percentage deviation from the baseline scenario. In motor vehicles 20 of the 22 buyers source further when $\hat T$ is equalised, and the average sourcing distance rises by 65~km, or 19\%, close to three times the 6.6\% that trade costs are worth. In aerospace the two directions very nearly cancel in the aggregate --- the average moves by 2~km, under one per cent --- while exactly half the buyers source further and half source closer: the average buyer's sourcing distance moves by 8.8\% in absolute value, and the individual deviations run from $+30\%$ to $-14\%$. Buyers in Toulouse lose the advantage of their local suppliers, and their sourcing distance rises by 30\%. Buyers in Ile-de-France, whose purchases were drawn towards distant hubs such as Toulouse, source more locally, and their sourcing distance falls by 11\%. Trade costs admit no such split: with $\alpha$ set to zero every one of the 18 buyers sources further, by 10\% on average and by at most 25\%.
...
\begin{figure}[H]
    \centering
\caption{Distribution of covariance between comparative advantage and distances\label{fig:alignment_covariance}}
    \includegraphics[width = 0.65\linewidth]{figures/alignment_covariance_mu2.pdf}
    \caption*{\footnotesize \emph{Notes}: Distribution across sector $\times$ downstream buyer pairs of $\operatorname{Cov}_{\tilde\gamma_{\cdot rs}}(\log \hat T_{a(l)s},\,d_{lr})$, in kilometres per log point of comparative advantage. The moments are taken across the supplier cells $l$ competing for each buyer $r$ and weighted by the sourcing probabilities. A kernel density is drawn over the histogram. Mass to the left of zero is comparative advantage sitting near demand.}
\end{figure}
...
\caption{Average sourcing share per buyer under each scenario, \label{fig:counterfactual_distance_region}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: The average sourcing distance $d_{r}$ of each shocked commuting zone under each counterfactual regime, expressed as a percentage deviation from the baseline scenario; the level it is taken against is printed in kilometres beside each row.}
\end{figure}
...
\paragraph{How concentrated is a buyer's portfolio of suppliers?}
...
\end{equation}
which is also the incidence vector of a shock to that buyer: the share of the upstream response landing in each supplier region. We are going to use the sector $\times$ buyer ($H_rs$) and buyer specific measure of portfolio concentration ($H_r$):
...
\end{equation}
The term $\mathcal{V}_{s}$ is the concentration of a buyer's sector-$s$ input bill across varieties, so that $1/\mathcal{V}_{s}$ is the effective number of varieties purchased; it depends only on $(\theta,\nu_{s},N_{s})$, not on the buyer's location.\footnote{Under Fr\'echet the price at which a variety is won is independent of the identity of its winner, $G_{r'rs}(p)=G_{rs}(p)$, so the $N_{s}$ champion prices are i.i.d.\ draws from a common distribution whose scale $\Phi_{rs}^{-1/\theta}$ alone carries $T$ and $\tau$. Expenditure shares $v_{\rho rs}=p_{\rho rs}^{1-\nu_{s}}/\sum_{j}p_{jrs}^{1-\nu_{s}}$ are homogeneous of degree zero in prices, so that scale cancels: geography shifts the level of the champion-price distribution, common to all of a buyer's varieties, and leaves its relative dispersion untouched. Both results rely on firms sourcing on the expected price index rather than the realised CES aggregate, following \citet{lenoir2023search}.} Geography and granularity therefore enter \eqref{eq:granular_buyer} through separate terms, and every counterfactual below moves the first alone.

The granular term has a simple reading. Granularity forces the system to produce exactly $N_{s}$ winners: each variety is won by one region, drawn with probability $\gamma_{r'rs}$, so a region's realised share is a weighted count of wins and the variance of that count is what the Herfindahl adds up. Under equal weights the variance is $\gamma_{r'rs}(1-\gamma_{r'rs})/N_{s}$; under CES the wins are weighted by expenditure and $\mathcal{V}_{s}$ replaces $1/N_{s}$, but the shape is unchanged.\footnote{\emph{Appendix :} the mechanism is clearest when varieties carry equal expenditure weights, $v_{jrs}=1/N_{s}$. Then $N_{s}\omega_{r'rs}(\Omega)$ is literally the number of varieties region $r'$ wins, which is $\operatorname{Binomial}(N_{s},\gamma_{r'rs})$, and since $\mathbb{E}[K^{2}]=N\gamma(1-\gamma)+N^{2}\gamma^{2}$ for $K\sim\operatorname{Bin}(N,\gamma)$, summing over $r'$ and using $\sum_{r'}\gamma_{r'rs}=1$ gives $\mathbb{E}_{\Omega}[H_{rs}(\Omega)]=H(\gamma_{rs})+N_{s}^{-1}[1-H(\gamma_{rs})]$. Under CES a cheaper winner takes a larger share, so $N_{s}\omega_{r'rs}$ is a \emph{weighted} sum of winner indicators rather than a count and that expression is not literally a binomial second moment; \eqref{eq:granular_buyer} is the general case, with $\mathcal{V}_{s}=1/N_{s}$ recovered under equal shares and $\mathcal{V}_{s}\ge 1/N_{s}$ always by Cauchy--Schwarz, so reading granularity as a count \emph{understates} it. } Each region's contribution vanishes at $\gamma_{r'rs}=0$ and $\gamma_{r'rs}=1$ and peaks in between, so granularity bears on the regions a buyer sources from occasionally, not on those it always or never uses. It is a large force when comparative advantage leaves the sourcing probabilities spread out, and none at all when comparative advantage has already settled the winner.

To assess the strength of each forces in shaping the concentration of the portfolio, we compute for each buyer $\mathbb{E}_\Omega(H_r)$. As for the sector $\times$ region concentration index, this buyer specific index can be decomposed into a structural and granular part. The median buyer's portfolio concentration is $0.118$ in motor vehicles and $0.207$ in aerospace. In the standard Ricardian case, the portfolio concentration would have been $H(\gamma_{r})=0.017$ and $0.055$: with finitely many varieties the supplier portfolio is $6.9$ times as concentrated in motor vehicles and $3.7$ times as concentrated in aerospace as the allocation the model would deliver in the continuum.\footnote{The inverse of the Herfindhal index is the effective number of supplier of the portfolio. In the continuum case, we would have expected this number to be $58$ for motor vehicles and $18$ for the aerospace against $8.5$ and $4.8$ when taking granularity into account. }. Following the decomposition of Equation~\ref{eq:granular_buyer}, we can write $\mathbb{E}_\Omega(H_r) = \sum_s (X_{rs}/X_r)||\gamma_{rs}||^2 + \sum_s(X_{rs}/X_r)\mathcal{V}_s(1-||\gamma_{rs}||^2)$. Concentration in the infinite benchmark can be approximated by a weighted sum of  $||\gamma_{rs}||^2$ which order of magnitude are about $10^{-2}$. Consequently, $1-||\gamma_{rs}||^2) \simeq 1$ meaning that the granularity term is high because $\mathcal{V}_s$ is high: the model predicts a very small number of varieties making the concentration of the portfolio several order of magnitude more important than in the infinite benchmark. \swann{D'un point de vue très "physique" qu'est ce que ça veut dire qu'on ait 5 variétés dans un même secteur ? On a l'impression que les fournisseurs de l'aéronautique produisent un grand nombre de variétés...} 


As for the average sourcing distance, we are interested in the impact of comparative advantage and trade costs on the concentration of shock propagation. Figure~\ref{fig:buyer_concentration_region} runs the same exercise as Figure~\ref{fig:buyer_reach_region}, now on the buyer-specific concentration $\mathbb{E}_{\Omega}[H_{r}]$, expressed as percentage deviations from the estimated economy. Equalising comparative advantage across space lowers concentration by $7.6$ percent in motor vehicles and $17.7$ percent in aerospace, with little variation across buyers.\footnote{The near-uniformity of the bars across buyers is a prediction of the model rather than a defect of the figure. The realised effective number ranges from $8.4$ to $8.5$ in motor vehicles and from $4.4$ to $5.0$ in aerospace, a relative spread of $2$ and $14$ percent, against $16$ and $37$ percent for the structural number underneath. Since $\mathcal{V}_{s}$ is a sector object common to every buyer and carries $87$ and $76$ percent of the realised statistic, only the remaining $(1-\mathcal{V}_{s})\|\gamma_{rs}\|^{2}$ can differentiate them. Granularity makes buyers look alike, and this bounds what any cross-section of realised linkages could identify about differences across buyers.} Shutting down trade costs does much less and does not even do it in a common direction: $+1.9$ percent in motor vehicles and $-1.8$ percent in aerospace. The uniform benchmark, in which productivities are equalised across attraction areas and $\alpha=0$ so that nothing selects among cells, sits at $-7.9$ and $-18.8$ percent. Equalising $\hat T$ alone therefore already delivers $95$ and $94$ percent of the joint effect: the two forces are substitutes in producing concentration rather than complements. 

Concentration responds to a force when that force favours the regions that already hold large shares. The sourcing shares \eqref{eq:sourcing_share} form an exponential family in $\log T_{a(r')s}$ and $\log d_{r'r}$, so each force moves the structural term at a rate given by a covariance under the sourcing probabilities themselves. Tilting the whole productivity vector, $T_{a(r')s}\mapsto T_{a(r')s}^{\,t}$, and evaluating at the estimate $t=1$,
$$\frac{\partial \|\gamma_{rs}\|^{2}}{\partial t}=2\,\mathrm{Cov}_{\gamma_{\cdot rs}}\big(\log T_{a(r')s},\,\gamma_{r'rs}\big),
\qquad
\frac{\partial \|\gamma_{rs}\|^{2}}{\partial (\alpha\theta)}=-2\,\mathrm{Cov}_{\gamma_{\cdot rs}}\big(\log d_{r'r},\,\gamma_{r'rs}\big),$$
where $t=0$ is the uniform benchmark. The first covariance is positive by construction, since $\gamma$ is increasing in $T$ at a given distance: comparative advantage reinforces itself. The second carries no sign restriction, and it is in any case the weaker instrument. Over the whole span of French distances, a log range of about $3.8$, the estimated $\alpha\hat\theta$ of $0.228$ and $0.458$ downweights the most remote supplier by a factor of $2.4$ and $5.7$; spread over the hundred-odd cells competing for a buyer, a gradient that shallow can tilt sourcing but cannot concentrate it. Estimated comparative advantage differs by orders of magnitude across attraction areas and can leave a sector's sourcing to a handful of them. The structural statistic records this: equalising $\hat T$ takes the median $\|\gamma_{r}\|^{2}$ from $0.017$ to $0.007$ in motor vehicles and from $0.055$ to $0.018$ in aerospace, falls of $61$ and $68$ percent, essentially all the way to the uniform benchmark.\footnote{At the buyer level, $\|\gamma_{r}\|^{2}=\sum_{s}(X_{rs}/X_{r})\|\gamma_{rs}\|^{2}$, the spending-weighted average of the within-sector structural terms, in keeping with the definition of $H_{r}$.}

The realised statistic moves by an eighth of that, and \eqref{eq:granular_buyer} says why. Granularity absorbs the difference twice over. A unit fall in $\|\gamma_{rs}\|^{2}$ passes into $\mathbb{E}_{\Omega}[H_{rs}]$ damped by $1-\mathcal{V}_{s}$, and as $\|\gamma_{rs}\|^{2}$ falls the slack $1-\|\gamma_{rs}\|^{2}$ rises, so the granular term $\mathcal{V}_{s}\big[1-\|\gamma_{rs}\|^{2}\big]$ rises with it --- from $0.101$ to $0.102$ and from $0.148$ to $0.152$ --- and offsets most of what the structural term gives up. Chance fills in the dispersion that comparative advantage vacates. The economic content is that dispersing comparative advantage buys a buyer no diversification in any given year: it changes which few regions the draw lands on, not how few. The realised Herfindahl therefore pools two objects with different dynamics. Structural concentration is persistent, so the same regions absorb the shock every year and exposure accumulates on them; granular concentration is redrawn, so a year looks concentrated but the identity of the concentrated region is not a property of the economy.

Trade costs act on a different margin, and this is where the heterogeneity across buyers comes from. The distance term $d_{r'r}^{-\alpha\theta}$ is the only component of $\gamma_{r'rs}$ that varies with the identity of the buyer; $T_{a(r')s}$ is common to all of them. All cross-buyer heterogeneity in sourcing therefore runs through trade costs, and the $\alpha=0$ counterfactual is not a shift but a collapse: with equal distances every buyer faces the same sourcing shares, and concentration converges to one common value.\footnote{This is the section's internal control and an identity rather than a fitted result; the same identity drives the reach of Figure~\ref{fig:buyer_reach_region} to $114.32$ and $63.47$ for every buyer. It also means the $\alpha=0$ column of Figure~\ref{fig:buyer_concentration_region} should not be read as an economic effect on the realised statistic. Setting $\alpha=0$ moves the structural term by $-0.0010$ and $+0.0005$ in levels, while the accompanying movement in $\mathcal{V}_{s}$, from $0.103$ to $0.106$ in motor vehicles and from $0.157$ to $0.154$ in aerospace, moves the granular term by $+0.0032$ and $-0.0029$. The second channel is three to six times the first and sets the sign, and those $\mathcal{V}_{s}$ movements are of the order of the simulation error at $50$ replications, which is why the column changes sign between the industries. We therefore read the distance counterfactual on $\|\gamma_{r}\|^{2}$.} Whether a given buyer gains or loses concentration then depends on where it sits relative to that common value, which is what the covariance above measures: trade costs raise a buyer's concentration only when its largest suppliers are also its nearest.

Toulouse is the extreme case and the informative one. It is co-located with the aerospace comparative-advantage hub, so proximity and productivity point at the same cells and the covariance is strongly negative: distance is \emph{adding} concentration there, and removing it spreads Toulouse's structural portfolio toward the common allocation. Peripheral aerospace buyers are in the opposite position. Their largest suppliers are the distant hubs, so trade costs were pulling spending away from them, and equalising distances leaves those buyers unchanged or slightly more concentrated. Motor vehicles has no buyer in Toulouse's position: comparative advantage sits near demand at four fifths of the (sector, buyer) pairs, but it is spread across several hubs rather than one, so the effect is small and common in sign.
\begin{figure}[H]
    \centering
...
We analyzed the concentration at the buyer level and find that most of it was due to granularity. However, this exercise doesn't tell whether or not the portfolio of two assembly plants is similar. If it were the case, this would imply that, from the supplier point of view, its portfolio of customer is itself concentrated and suppliers of both industries are equally exposed to industry wide shocks or customer specific ones. 


\paragraph{How concentrated is a supplier region's portfolio of customers?}

Consider a one-euro demand shock hitting industry $s$. That euro is more likely to land on a large buyer than on a small one. Its expected incidence is therefore the purchase-weighted average of buyer-level incidence:
\begin{equation*}
\bar H_{s}(\Omega)=\mathbb{E}_{r}[H_{rs}(\Omega)],
\end{equation*}
where $\mathbb{E}_{r}$ weights buyer $r$ by its share $X_{rs}/X_{\cdot s}$ of sector purchases, with $X_{\cdot s}=\sum_{k}X_{ks}$. Following the same decomposition made earlier we have 
...
\end{equation}
where $\Bar{\omega}_s(\Omega) = \mathbb{E}_r[\omega_{rs}(\Omega)]$ the incidence of an industry-wide shock. Taking the expectation over network realisations yields: 
...
Since we aggregate across buyers and network realization we are looking at two dimensions for shock propagation. First, the structural vs granular dimension (i and ii vs iii and iv) then the common vs buyer specific dimension (i and iii vs ii and iv). The first term determines how a shock propagates among the common pool of suppliers in the continuum model and the third term measure how much how such common diffusion is affected by granularity. The second term looks at the buyer specific concentration in the continuum network and the fourth term is the granular effect. If all buyers sources from the same portfolio of suppliers both in the continuum and granular version of the model, ii and iv would be null. \\

Figure~\ref{fig:concentration_decomposition} reports, for each sector, the decomposition of $\mathbb{E}_{\Omega}[\bar H_{s}(\Omega)]$ into its four terms. Granularity accounts for most of realised concentration. The structural part is smaller but it is almost entirely common across buyers. A shock to a single buyer therefore lands on nearly the same supplier regions as a shock to the whole industry. The reason is that buyers rank suppliers in almost the same order. Comparative advantage is the same for every buyer, so only distance can make portfolios buyer-specific. Yet distance weighs little against comparative advantage: $\operatorname{sd}(\log\hat T)/\big(\alpha\hat\theta\,\operatorname{sd}(\log d)\big)$ is $6.5$ in motor vehicles and $3.5$ in aerospace at the median sector.\footnote{This ratio is a lower bound: most of the dispersion carried by distance is absolute remoteness ($72\%$ and $59\%$), which penalises a supplier equally for every buyer. } Both a low elasticity and the short span of French distances explain this. Comparative advantage only reinforces this pattern in aerospace, where it concentrates sourcing on a few hubs that are roughly equally distant from most buyers.\footnote{When comparative advantage is equalised across areas (\emph{Distance only}), the common share within structural terms remains at $0.98$ in motor vehicles and falls only to $0.88$ in aerospace, against $0.985$ and $0.954$ in the estimated economy.}
