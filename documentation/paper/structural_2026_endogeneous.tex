...
\emph{Across buyers.} Let $\bar\omega_{s}=\sum_{r}(X_{rs}/X_{\cdot s})\,\omega_{rs}$ be the incidence of a shock to the industry as a whole. Averaging $H$ over buyers and completing the square gives
\begin{equation}
\bar H_{s}\;=\;H(\bar\omega_{s})\;+\;M_{s},
\qquad
M_{s}=\sum_{r'}\operatorname{Var}\!\big(\omega_{r'rs}\big)\;\ge 0,
\label{eq:incidence_identity}
\end{equation}

\swann{I prefer the definition with the norm explicited. Also it should be buyer specific and there is not r index here. }
the variance being taken across buyers with the same weights.\footnote{$\bar H_s=\sum_r (X_{rs}/X_{\cdot s})\sum_{r'}\omega_{r'rs}^2=\sum_{r'}[\operatorname{Var}(\omega_{r'rs})+\bar\omega_{r's}^2]$. The identity holds for the weighted mean that defines $\bar\omega_s$ and for no other aggregator, so every average in this section is purchase-weighted, and $H$ is averaged before being inverted into an effective number, since $1/H$ is convex. The cross-buyer distributions behind these means are in Appendix~\ref{app:incidence_buyer}.} A shock to one buyer lands with concentration $\bar H_{s}$; a shock to the whole industry lands with $H(\bar\omega_{s})$, which is smaller exactly to the extent that buyers send their euros to different places. The first term is therefore the \emph{common} part of a buyer's exposure and $M_{s}$ the \emph{buyer-specific} part.

\emph{Across varieties.} \swann{I have already introduced in the begining of section 5, the idea of realised vs average network. You can shorten the paragraph here. }A realised network is not the expected one. Each of a sector's $N_{s}$ varieties is won by exactly one region, so a realised incidence vector has at most $N_{s}$ non-zero entries however diffuse the expected network is. Write $\omega_{r'rs}=\sum_{j}v_{jrs}\mathbf{1}\{r'\text{ wins }j\}$, with $v_{jrs}$ the share of buyer $r$'s spending on sector $s$ going to variety $j$. Under Fr\'echet the identity of a variety's winner is independent of the price at which it is won, so expenditure weights and win locations are independent and, for \emph{any} pair of buyers,
\begin{equation}
...
\qquad
\Gamma_{rr''s}=\sum_{r'}\gamma_{r'rs}\,\gamma_{r'r''s}.
\label{eq:variety_kernel}
\end{equation}
Here $\mathcal{V}_{rr''s}=\mathbb{E}[\sum_{j}v_{jrs}v_{jr''s}]$ is the \emph{effective number of varieties}, $Q_{rr''s}$ the probability that the two buyers win the same variety in the same region, and $\Gamma_{rr''s}$ the probability that they land in the same region through different varieties --- the overlap that the sourcing shares $\gamma_{r'rs}$ of \eqref{eq:gamma_rrs} already imply. Setting $r''=r$ gives $\mathbb{E}[H_{rs}]=\mathcal{V}_{rs}+(1-\mathcal{V}_{rs})H(\gamma_{rs})$: chance concentrates a shock, and $\mathcal{V}$ says by how much.\footnote{Squaring $\omega_{r'rs}$ and taking expectations, $\mathbb{E}[H]=\mathbb{E}\sum_j v_j^2+\mathbb{E}\sum_{j\neq j'}v_jv_{j'}\mathbf{1}\{\text{same winner}\}$, and under the independence above the second term is $(1-\mathcal V)H(\gamma)$. The familiar $1/N_s$ is the special case of equal expenditure shares across varieties; by Cauchy--Schwarz $\mathcal{V}_{rs}\ge 1/N_{s}$ always, so reading granularity as a multinomial count \emph{understates} it.}

Substituting \eqref{eq:variety_kernel} into \eqref{eq:incidence_identity} \swann{I think it is more interesting to speak using conditional variance here. Please add the demonstration in a footnote. } splits the expected concentration of a realised shock into four additive terms:
\begin{equation}
\mathbb{E}\big[\bar H_{s}\big]
=\underbrace{H(\bar\gamma_{s})}_{\text{(i) structural, common}}
+\underbrace{\textstyle\sum_{r'}\operatorname{Var}(\gamma_{r'rs})}_{\text{(ii) structural, buyer-specific}}
+\underbrace{\textstyle\sum_{r,r''}\tfrac{X_{rs}}{X_{\cdot s}}\tfrac{X_{r''s}}{X_{\cdot s}}\mathcal{V}_{rr''s}\big(Q_{rr''s}-\Gamma_{rr''s}\big)}_{\text{(iii) granular, common}}
+\underbrace{\big[\,\textstyle\sum_{r}\tfrac{X_{rs}}{X_{\cdot s}}\mathcal{V}_{rs}\big(1-H(\gamma_{rs})\big)-\text{(iii)}\,\big]}_{\text{(iv) granular, buyer-specific}}.
\label{eq:four_cells}
\end{equation}
The decomposition is exact, and it isolates where the finite number of varieties enters: through \textbf{two objects and no others}. $\mathcal{V}$ governs \emph{how much} chance concentrates a shock, and the excess co-sourcing $Q-\Gamma$ governs \emph{whether that concentration is shared} across buyers. Independent win events across buyers would set $Q=\Gamma$ and leave cell (iii) at zero, so that granularity would be purely idiosyncratic and would diversify the industry's exposure; $Q>\Gamma$ is positive dependence, and it compounds it instead.
