...
Figure~\ref{fig:alignment_covariance} reports the distribution of the covariance in \eqref{eq:alignment} across (sector, buyer) pairs, and Figure~\ref{fig:counterfactual_distance_region} the displacement it produces, buyer by buyer. Trade costs shorten the diffusion in both industries, by 23~km (6.6\%) in motor vehicles and 34~km (9.6\%) in aerospace, and they do so for \emph{every} buyer in both --- all 22 motor-vehicle and all 18 aerospace buyers source further once $\alpha$ is set to zero, as \eqref{eq:distance_rate} requires.

Comparative advantage is where the two industries part, and it parts them on the sign. In motor vehicles the covariance is negative for 20 of the 22 buyers, at a median of $-63$~km per log point: productive suppliers sit near demand almost everywhere, and only 19\% of (sector, buyer) pairs point the other way. Equalising $\hat T$ therefore lengthens the diffusion for 20 of the 22 buyers --- the same count, as \eqref{eq:alignment} requires, since a buyer's euro travels further under the counterfactual exactly when its covariance is negative --- by 65~km on average, close to three times the effect of trade costs. In aerospace the covariance is \emph{positive} for two thirds of the buyers, at a median of $+7$~km and over a range from $-27$ to $+73$: capability sits far from most buyers and close to a few, which is the hub configuration stated as a rate. The two directions nearly cancel, so the effect of equalising $\hat T$ is close to zero at the centre of the distribution --- the average sourcing distance moves by 2~km and the median by 3 --- while exactly half the buyers source further and half source closer and 89\% of them move by more than 5~km. In motor vehicles no buyer moves by less than that and 91\% move the same way.\footnote{The covariance is evaluated at the estimate whereas the displacement integrates it along the whole path from $t=1$ to $t=0$, so the two need not agree buyer by buyer where the alignment is weak: in aerospace twelve buyers have a positive covariance at the estimate but nine end up sourcing closer. In motor vehicles, where the alignment is strong and one-sided, the counts coincide.} Buyers in Toulouse lose the advantage of their local suppliers and source further. Buyers in the Paris region, whose purchases were drawn towards distant hubs such as Toulouse, source more locally: in Roissy -- Sud Picardie the average euro travels 377~km under the estimated allocation and 332~km once comparative advantage is equalised, a fall of 12\%.
