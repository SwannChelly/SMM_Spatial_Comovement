...
Equation~\eqref{eq:granular_buyer} has three inputs and each is a force we can switch off: comparative advantage and trade costs both enter through $\gamma_{rs}$, and the variety count enters through $\mathcal{V}_{rs}$ alone. Figure~\ref{fig:buyer_concentration_region} reports $\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ for every shocked commuting zone as an effective number of supplier regions, under the same two counterfactuals as Figure~\ref{fig:counterfactual_distance_region} and, beside them, the $N_{s}\to\infty$ limit $\sum_{s}(X_{rs}/X_{r})H(\gamma_{rs})$ that isolates granularity at the estimated network.

\swann{Les trois lectures ci-dessous doivent \^etre remplies avec le nouveau run : la fonction \texttt{buyer\_granular\_concentration} et la figure \texttt{plot\_buyer\_granular\_concentration} sont dans \texttt{diffusion.ipynb} (section Test 5 bis) et \'ecrivent \texttt{buyer\_concentration\_region\_\{auto,aero\}\_mu2.pdf}. Les trois chiffres \`a reporter sont (a) la m\'ediane et l'\'etendue de $1/\mathbb{E}_\Omega[H_r]$ par industrie, (b) l'\'ecart entre la s\'erie \emph{Infinite varieties} et z\'ero, qui est la granularit\'e acheteur par acheteur, et (c) le signe et la taille des deux contrefactuels.}

Three readings follow, and each is a buyer-level version of a statement the industry aggregate can only make on average. \emph{First}, the level: a shock to the median motor-vehicle buyer reaches an effective \swann{XX} supplier zones and a shock to the median aerospace buyer \swann{XX}, over ranges of \swann{XX} and \swann{XX}. \emph{Second}, granularity is the distance between the \emph{Infinite varieties} series and the origin, and it is large for every buyer: the same shock would reach \swann{XX} times as many zones if varieties were a continuum. \emph{Third}, the two counterfactuals are lopsided in the same direction for every buyer, and in the opposite way to the sourcing distance --- equalising $\hat T$ disperses the portfolio enormously while setting $\alpha=0$ barely moves it. That asymmetry is the substitution of \eqref{eq:granular_binomial} seen from the other side, and we return to it once the industry aggregate is in place.
...
\caption{The concentration of each buyer's supplier portfolio under each scenario\label{fig:buyer_concentration_region}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: The effective number of supplier commuting zones $1/\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ of each shocked commuting zone, from \eqref{eq:granular_buyer}, expressed as a percentage deviation from \emph{Both forces}, the estimated finite-variety economy; the level it is taken against is printed beside each row. \emph{Infinite varieties} holds the estimated network $\gamma_{rs}$ fixed and sends $N_{s}\to\infty$, so the distance between that series and the zero rule is granularity, buyer by buyer, and every other series moves $\gamma_{rs}$ itself. Percentages rather than levels because the levels differ across buyers by an order of magnitude, so on a level axis the cross-buyer spread would crowd out the counterfactual. Regions are ordered by their baseline effective number. The counterfactual rows are drawn from the model's own primitives, with one productivity draw per (variety, region) shared across buyers, since it is the same physical variety.}
\end{figure}
