...
and we take it sector by sector, because a buyer's shock hits every one of its input sectors and each sector has its own supplier geography. Pooling them lets a nearly common input mix impose a similarity across buyers that says nothing about where inside a sector the euro goes.\footnote{Pooling is not innocuous. Buyers in the same industry share their technology, so their sectoral spending shares $\theta_{rs}$ are nearly identical, and the pooled incidence vectors are forced to look alike whatever happens within sectors. Measured on pooled vectors, the cosine similarity between any two buyers' incidence exceeds $0.97$ in both industries and separates nothing.}

The concentration of the buyer's supplier portfolio is measured by the Herfindahl of the incidence vector, $H_{rs}=\sum_{l}\omega_{lrs}^{2}$, whose inverse $1/H_{rs}$ is the \emph{effective number} of supplier regions a shock reaches; a buyer's overall figure aggregates its sectors with its own input mix, $H_{r}=\sum_{s}\theta_{rs}H_{rs}$. Because the level of $H$ depends on how many regions the sector has and on the shape of the country before any economic force acts, we report it relative to the same statistic under an unselective allocation, exactly as we normalised the sourcing distance.

Commonality does not need a separate statistic: it is already inside $H$. Let $\pi_{rs}$ be buyer $r$'s share of sector $s$'s purchases and $\bar\omega_{s}=\sum_{r}\pi_{rs}\omega_{rs}$ the incidence of a shock to the industry as a whole. Then
\begin{equation}
