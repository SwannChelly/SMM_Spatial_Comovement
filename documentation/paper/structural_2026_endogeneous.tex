...
Figure~\ref{fig:counterfactual_distance} reports the distribution of $\bar d_r$ across buyers under each regime. Trade costs shorten the diffusion in both industries, by \textbf{X}~km (\textbf{X}\%) in motor vehicles and \textbf{X}~km (\textbf{X}\%) in aerospace, as \eqref{eq:distance_rate} requires. Comparative advantage is where the two industries part, and Figure~\ref{fig:alignment_correlation} shows why. In motor vehicles, the covariance is negative for essentially every buyer: comparative advantage is co-located with demand. Equalising $\hat T$ therefore lengthens the diffusion for every buyer, by \textbf{X}~km, about twice the effect of trade costs. In aerospace, the covariance changes sign across buyers, and the median effect of equalising $\hat T$ is close to zero. Figure~\ref{fig:counterfactual_distance_region} shows that this near-zero median hides large offsetting movements. Buyers in Toulouse lose the advantage of their local suppliers, and their sourcing distance rises by 30\%. Buyers in \^Ile-de-France, whose purchases were drawn towards distant hubs such as Toulouse, source more locally, and their sourcing distance falls by \textbf{X}\%. The absolute reallocation amounts to \textbf{X}~km, \textbf{X} times the effect of trade costs.
