...
The two industries are indistinguishable in the centre of the distribution and opposite in its shape. The median shocked region keeps 5.7\% of its upstream response within 100 km in motor vehicles and 5.6\% in aerospace: effectively the same. What differs is the dispersion around it. In motor vehicles the share ranges from 1.7\% to 18.0\%, and at 200 km no region falls below 14.5\%: shocks diffuse widely, but they diffuse in the same way wherever they start. In aerospace the range is 0.7\% to 42.1\%, and at 200 km the least local region retains just 3.1\%. The higher aerospace mean (9.2\% against 7.5\%) is carried by a single commuting zone: a shock to Toulouse keeps two fifths of its upstream response within 100 km, while a shock to most other aerospace regions is transmitted almost entirely elsewhere. Diffusion in motor vehicles is broad and homogeneous; in aerospace it is hub-and-spoke.
...
The comparative-advantage channel changes sign across industries. In motor vehicles, equalising $T$ across areas \emph{lowers} the local share, from 7.4\% to 6.3\% on average, and sends the average euro 65 km further (347 to 412 km): productive suppliers are located where the buyers are, so comparative advantage and proximity reinforce one another. In aerospace, the same operation \emph{raises} the local share, from 8.9\% to 10.5\%, and leaves the average sourcing distance unchanged (353 to 354 km). The mirror image appears when distance is switched off instead: relative to a uniform draw across the sector's cells, comparative advantage alone pulls sourcing 67 km closer to the buyer in motor vehicles but only 14 km closer in aerospace. Aerospace productivity is concentrated in two places, so for the two hubs $T$ is a force of localisation and for every other region a force of dispersion; averaged across regions the two nearly cancel.
