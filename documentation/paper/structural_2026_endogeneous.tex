...
Second, they separate on the \emph{alignment} term rather than on distance. The pure distance component accounts for 2\% of the variance in motor vehicles and 7\% in aerospace --- the ordering $\alpha\hat\theta$ requires --- but the co-location component contributes 11\% in motor vehicles, more than five times the contribution of distance alone, against 9\% in aerospace, only modestly above the 7\% distance already carries. Productive suppliers sit close to buyers in motor vehicles; in aerospace the average alignment is much weaker.\footnote{Restricting the calculation to attraction areas with observed sourcing, as in the estimator's own decomposition of the covariance, gives sector-median correlations of $-0.47$ and $-0.17$ rather than the $-0.34$ and $-0.11$ reported below. The two differ in the cell set and in the weighting, not in what they measure; the numbers quoted in the text are those of the distribution in Figure~\ref{fig:alignment_correlation}, which is the object whose purchase-weighted average is exactly the co-location term of Table~\ref{tab:variance_decomposition}.}

Third --- and this is what a median cannot show --- the two industries differ in \emph{where} that alignment sits, not in how dispersed it is. Figure~\ref{fig:alignment_correlation} reports the distribution of $\rho_{rs}=\operatorname{Corr}_l(\log\hat T_{a(l)s},\log d_{lr})$ across all (sector, buyer) pairs. The two distributions have comparable spread --- both run from roughly $-0.7$ to $+0.7$, and neither is concentrated at any single value --- and they are displaced relative to one another: motor vehicles sits to the left, with a median of $-0.34$ and only 19\% of pairs positive, while aerospace is centred close to zero, with a median of $-0.11$ and 30\% of pairs positive.

The reading this forces is not that capability and proximity are unrelated in aerospace. Almost no buyer there faces a geography in which they are unrelated: individual correlations of $\pm0.4$ and beyond are common, as they are in motor vehicles. What is near zero in aerospace is the \emph{average}, because the alignment points in opposite directions for different buyers and sectors --- other transport equipment, which alone carries 56\% of the industry's modelled intermediate purchases, has a positive sourcing-weighted covariance of $+44$~km against a sector median of $-14$~km. In motor vehicles the displacement is systematic: every sector's sourcing-weighted covariance is negative (between $-6$ and $-116$~km per log point, median $-54$), so an average taken over buyers describes most of them.\footnote{A negative covariance in every sector is consistent with the 19\% of positive \emph{pairs}: the sector-level statistic averages over buyers with their purchase weights, and the buyers for whom the sign flips are small ones.} A weak average alignment and no alignment are therefore different economies, and they predict different counterfactuals: the first cancels in the aggregate while moving individual buyers a great deal, the second moves nobody. Section~\ref{subsec:diffusion} shows the aerospace aggregate is the first of the two.

\begin{figure}[H]
    \centering
\caption{The alignment between comparative advantage and distance, buyer by buyer\label{fig:alignment_correlation}}
    \includegraphics[width = 0.65\linewidth]{figures/alignment_correlation_mu1.pdf}
...
\caption{Amplification of shocks along vertical production linkages\label{fig:amplification}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/amplification_auto_mu1.pdf} & \includegraphics[width = 0.5\linewidth]{figures/amplification_aero_mu1.pdf}
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Amplification of shocks affecting individual commuting zones in the motor vehicles (left panel) and aerospace (right panel) industries. The amplification coefficient $D_{r}$ is the additional sales generated by one unit of demand in the downstream industry, aggregated across downstream firms and their domestic suppliers.}
\end{figure}

\paragraph{Diffusion.} Amplification says how large the upstream response is, not where it lands, and it is the second object that is specific to a model with endogenous linkages. We measure where it lands by the distance the response travels. Writing $X_{r'r}$ for the upstream sales that a demand shock in downstream region $r$ generates in upstream region $r'$, the \emph{average sourcing distance} of a shock originating in $r$ is
\begin{equation}
...
\caption{How far the shock travels, with one force switched off\label{fig:counterfactual_distance}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_distance_auto_mu1.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_distance_aero_mu1.pdf}
    \end{tabular}
...
\caption{How far the euro travels, region by region\label{fig:counterfactual_distance_region}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_distance_region_auto_mu1.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_distance_region_aero_mu1.pdf}
    \end{tabular}
...
\paragraph{What the aerospace two kilometres are made of.} An aggregate this small invites the reading that comparative advantage does not move aerospace sourcing, and that reading is wrong. Two distinct cancellations produce it, and both are measurable.

The first is \emph{composition}, and it is the larger of the two. The counterfactual holds the sectoral split of the euro fixed at its estimated value --- that split is a cost-share object, and neither $T$ nor $\alpha$ enters it --- so the aggregate is a spending-weighted average of sector-level responses. Equalising $\hat T$ raises the average sourcing distance by between $12$ and $59$~km in nine of the ten aerospace sectors, with a median of $32$~km. The tenth is other transport equipment, where it raises it by $2.6$~km --- and that sector alone carries $56$\% of the industry's modelled intermediate spending. Weighting the ten sector responses by their spending shares already brings the median $32$~km down to roughly $17$~km before a single buyer is aggregated.\footnote{Approximately: the per-sector responses are themselves purchase-weighted across buyers, so the spending-weighted average of them is an accounting device rather than an identity. The exact aggregate, computed on the euros a shock actually sends upstream, is the $2.4$~km of Figure~\ref{fig:counterfactual_distance}.} In motor vehicles no sector plays that role, which is why there the sector median and the aggregate coincide ($65$~km against $65$~km).

The second is the \emph{cross-buyer} cancellation already visible in the crossing curves: half of the shocked regions move away from their own location and half move closer, and no quantile moves by more than 18~km. That takes the remaining $17$~km down to $2.4$.

What makes other transport equipment inert is a fact about geography rather than about the strength of comparative advantage there. Its $\hat T$ is as dispersed as any sector's ($\operatorname{sd}_{\rho}(\log\hat T)=1.10$) and its capability is sharply concentrated on Toulouse; but Toulouse sits at roughly the \emph{average} distance from the aerospace buyer distribution, so concentrating sourcing on it costs $313$~km against $316$~km under a uniform allocation. Comparative advantage there redirects the euro without lengthening its journey. This is precisely the configuration \eqref{eq:alignment} says the mean cannot detect: the covariance of $\log T$ with distance is what the mean responds to, and in that sector it is essentially zero --- indeed it is the one aerospace sector whose covariance is \emph{positive} at the estimate ($+44$~km per log point).

Two candidate explanations can be ruled out rather than left open. Granularity is not the mechanism: the counterfactual is closed-form in the sourcing probabilities $\rho_{lrs}$, which contain no variety count, and the realised finite-variety draw differs from that closed form by $1.3$~km. Finite $N_{s}$ changes how many origins one shock reaches --- 62 commuting zones against 262 in the continuum limit --- but leaves the expected distance unchanged. Nor is the result an artefact of the input-output weights being wrong: the sectoral split is read off the estimated economy and reproduces the industry's observed input composition, and the point is not that the weights are mismeasured but that one sector both dominates the spending and happens to be the one where capability and distance are orthogonal.


\paragraph{Does sourcing track the buyer?} The counterfactual says capability sits near demand in motor vehicles. It does not say whether that capability is one mass placed inside the demand distribution or many local pockets, and those are different economies: the first sends every buyer to the same suppliers, the second gives each buyer its own. The distinction is measurable in one number. Let $b_{r}=\sum_{r'}X_{r'r}x_{r'}/\sum_{r'}X_{r'r}$ be the barycentre of the sourcing a shock in $r$ generates, with $x_{r'}$ the location of upstream region $r'$, and regress it on the buyer's own location, $b_{r}=(1-\lambda)c+\lambda x_{r}+\varepsilon_{r}$. Then $\lambda=1$ means every buyer sources around itself, $\lambda=0$ means the allocation is buyer-independent and every shock converges on one national barycentre, and intermediate values measure how much of its own position a buyer's sourcing inherits.\footnote{The barycentre is computed and regressed in a projected metric coordinate system (Lambert-93), and $\lambda$ is the pooled projection over both coordinates at once, so it does not depend on how the map is oriented. Table~\ref{tab:barycentre_shrinkage} reports it with standard errors clustered on the shocked region, and Figure~\ref{fig:sourcing_barycentre} draws the displacements $b_{r}-x_{r}$ themselves.}

The estimates are reported in Table~\ref{tab:barycentre_shrinkage}, and the informative fact about them is that they are \emph{small in both industries}: $\hat\lambda=0.065$ (s.e.\ 0.002) in motor vehicles and $0.080$ (s.e.\ 0.009) in aerospace, against a fit that is tight rather than noisy ($R^{2}=0.96$ and $0.85$). Both are many standard errors from zero, so this is not an imprecise estimate of nothing: buyers agree that their barycentres move with them, and agree that they move by about seven per cent of the way. In neither industry, therefore, does a buyer's sourcing follow the buyer, and the arrows from each origin to its barycentre are correspondingly short. The two industries are also not distinguishable on this margin --- the difference is $-0.015$ with a standard error of $0.009$ (95\% interval $[-0.033,\,0.003]$) --- so although the point estimate has the sign the model predicts, the data do not establish the ranking. That is the paragraph's content: the geography of \emph{where} sourcing sits is common to the two industries, and what separates them is how much of the response each origin retains, which is the next paragraph.

Two things do separate sharply, and both are checks rather than findings. The internal control works: setting $\alpha=0$ removes the only buyer-specific term of the sourcing probability, and $\hat\lambda$ collapses accordingly, by $-0.068$ (t $=-36$) in motor vehicles and $-0.103$ (t $=-9$) in aerospace, to values that are negative but negligible in size. And equalising $\hat T$ \emph{raises} the aerospace shrinkage by half, from $0.080$ to $0.121$, a difference of $+0.041$ with a standard error of $0.016$, while leaving the motor-vehicle one statistically flat ($+0.003$, s.e.\ $0.004$). What holds aerospace sourcing away from each buyer's own neighbourhood is therefore the concentration of capability; nothing but the low trade-cost elasticity holds the motor-vehicle one down.

Read together with the counterfactual, this answers the hub question, and it also disposes of the reading the map most naturally invites. In aerospace every arrow in Figure~\ref{fig:sourcing_barycentre} points south-east, including those from buyers as remote as Brest, which looks like every buyer sourcing from Toulouse. The arrowheads say otherwise: they land in the centre of the country --- Ussel, Tulle and Gu\'eret collect more than half of them --- some 250~km short of Toulouse. A barycentre is a weighted mean of supplier positions, so with four fifths of the euros spread nationally it sits near the national centre of mass however concentrated the remaining fifth is. The map therefore records a \emph{tilt} towards Toulouse, not a destination, and a tilt of that size is exactly what shifts the direction of a buyer's sourcing without shifting its length. That is the same fact as the previous paragraph, seen on a map instead of in kilometres.

The reading that survives is this. Motor-vehicle capability is one broadly central mass sitting inside the demand distribution: buyers do not source locally, they source from a common central pool, which is how a negative covariance can hold at \emph{every} buyer at once without any buyer sourcing around itself. Aerospace capability is concentrated on Toulouse and Île-de-France, away from most of its demand --- but at a distance from the typical buyer that is close to the distance the euro would travel anyway, which is why that concentration governs \emph{where} the response goes without governing how far.
...
\caption{Where the sourcing sits: origin to barycentre\label{fig:sourcing_barycentre}}
    \includegraphics[width = 0.85\linewidth]{figures/sourcing_barycentre_panel_mu1.pdf}
...
be the share of the upstream response accruing within $d$ kilometres of the origin of the shock. We report it at $d=200$~km, the radius at which the reduced-form evidence of Section~\ref{sec:reduced_form_evidence} is measured, and map it in Figure~\ref{fig:local_share_map}.
...
\caption{Where a downstream shock lands\label{fig:local_share_map}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/local_share_map200km_auto_mu1.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/local_share_map200km_aero_mu1.pdf}
    \end{tabular}
...
The medians are again indistinguishable --- the median shocked region keeps 21.3\% of its response within 200~km in motor vehicles and 19.7\% in aerospace --- and again the dispersion behind them is what differs. In motor vehicles the local share ranges from 15.5\% to 43.7\%: shocks diffuse widely, but in much the same way wherever they start, and no region is left without a substantial local response. In aerospace the range runs from 3.1\% to 56.3\%. A shock to Toulouse is retained more than half within 200~km; a shock to most other aerospace regions is transmitted almost entirely elsewhere. Broad and homogeneous against hub-and-spoke, as at 100~km, only more sharply.

Which force produces that retention is the counterfactual again, and it separates the two industries exactly as the distance experiment did --- with one sign reversal that the distance statistic could not show. In motor vehicles the two forces reinforce each other: the local share falls from 23.8\% to 18.1\% when $\hat T$ is equalised and to 19.4\% when $\alpha=0$, so removing either disperses the shock, and comparative advantage is marginally the more important of the two. In aerospace they work against each other. Switching off distance costs 6.7 percentage points (21.4\% to 14.7\%), so proximity is what keeps an aerospace shock local, as it was what kept the euro close; but equalising $\hat T$ \emph{raises} the local share, from 21.4\% to 24.5\%. Comparative advantage in aerospace is not a localising force at all: it takes the response away from the region where the shock originates and sends it to the hubs. That is the reallocation the average distance nearly cancels --- the mass taken off Toulouse and Île-de-France lands partly next door to each buyer, which the local share records, and partly at an intermediate distance, which offsets it in the mean --- and it is why the two statistics are both needed. ``How far'' and ``who keeps it'' are separate questions, and in aerospace comparative advantage answers only the second.
...
\caption{Local incidence with one force switched off\label{fig:counterfactual_local_share}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_local_share200km_auto_mu1.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_local_share200km_aero_mu1.pdf}
    \end{tabular}
...
\caption{The whole diffusion profile under each regime\label{fig:diffusion_profile}}
    \begin{tabular}{cc}
    Motor vehicles&Aerospace\\
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_profile_auto_mu1.pdf} &
        \includegraphics[width = 0.5\linewidth]{figures/counterfactual_profile_aero_mu1.pdf}
    \end{tabular}
