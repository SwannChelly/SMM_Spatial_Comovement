...
\paragraph{Where comparative advantage pulls sourcing.} The comparison above measures the \emph{strength} of comparative advantage relative to geography. It says nothing about its \emph{direction}, and it is the direction that governs the geography of shock propagation: what matters is not how unequal comparative advantage is, but where it is located relative to downstream demand. Dialling comparative advantage down from its estimate towards a common value along $\rho_{lr}(t)\propto T_{a(l)}^{\,t}\,d_{lr}^{-\alpha\theta}$ moves the average sourcing distance at the rate
\begin{equation}
\frac{d}{dt}\,\mathbb{E}_{\rho_{r}(t)}\!\left[\log d_{lr}\right] \;=\; \operatorname{Cov}_{\rho_{r}(t)}\!\left(\log T_{a(l)},\log d_{lr}\right),
\label{eq:alignment}
\end{equation}
an exact property of the Ricardian allocation. What moves sourcing in space is therefore the \emph{covariance} between capability and distance to the buyer, and not the dispersion of capability: a $\hat T$ that varies widely but is uncorrelated with the geography of demand reallocates sourcing across regions without sending it any further away. Dispersion only bounds that covariance, $|\operatorname{Cov}|\le \operatorname{sd}(\log T)\operatorname{sd}(\log d)$.
...
$$
\log \hat T_{a(l)s} \;=\; a_{sr} \;+\; b\,\log d_{lr} \;+\; \varepsilon_{lrs},
\qquad \text{weights } \rho_{lrs},
$$
a weighted least squares with a sector $\times$ downstream-region fixed effect, whose slope is $b = \operatorname{Cov}_{\rho}(\log \hat T,\log d)/\operatorname{Var}_{\rho}(\log d)$. We report the covariance and the correlation rather than $b$, because it is the covariance that enters~\eqref{eq:alignment} and because $\operatorname{sd}_{\rho}(\log d)$ differs across industries. No standard error accompanies this line: $\hat T$ and $d$ are model objects rather than a sample, so the dispersion around it carries no sampling interpretation.

Figure~\ref{fig:spatial_alignment} shows the cloud behind \eqref{eq:alignment} for a representative buyer in each industry: one dot per competing upstream commuting zone, its distance from the buyer against its comparative advantage, with the area of the dot given by the sourcing probability $\rho_{lr}$. The two industries are opposed. In motor vehicles the fitted relationship is downward in every sector, with a median correlation of $-0.37$: the productive suppliers are the ones close to the assembly plants, so comparative advantage and proximity pull in the same direction. In aerospace it is close to flat, $-0.16$ at the median and positive in one sector: capability is concentrated in a small number of hubs --- Toulouse and Île-de-France --- that are not close to most of the demand, and the productivity premium of those hubs is paid for in kilometres by every buyer except the ones next to them. Integrating~\eqref{eq:alignment} to a common $\hat T$ moves the average sourcing distance of the median sector by 65~km in motor vehicles and 32~km in aerospace. The aerospace figure overstates what happens to the industry, and for a reason that is itself the point: the sector carrying 56\% of its intermediate purchases --- other transport equipment --- is exactly the one whose capability is not aligned with its buyers, with a correlation of $+0.12$ and a distance effect of 3~km, so the spend-weighted aggregate of the next section is far below the median sector. In motor vehicles no sector plays that role and the two coincide.\footnote{$\hat T$ is the Sinkhorn image of the observed sourcing shares, so this covariance is model-implied rather than read directly off the data. Writing $\log \hat T = \log \hat\gamma^{\,emp} - \log M(\alpha)$, it splits into the gradient of the observed shares and minus that of market access. In aerospace the two contribute about equally. In motor vehicles the observed shares are, if anything, slightly increasing in distance (a covariance of $+0.07$), and the whole of the negative alignment comes from the market-access term: areas far from a given buyer tend to be centrally located overall, so the sales they are observed to make require less comparative advantage to rationalise.}
...
\caption{Where comparative advantage pulls sourcing\label{fig:spatial_alignment}}
    ...
    \end{tabular}
    \caption*{\footnotesize \emph{Notes}: Each dot is an upstream commuting zone competing to supply one representative downstream buyer --- the buyer whose sourcing is of median reach. The horizontal axis is its distance to that buyer and the vertical axis its estimated comparative advantage $\log \hat T_{a(l)s}$, both measured as deviations from their sourcing-weighted sector mean, since $\hat T$ is identified only up to a per-sector scale. The area of each dot is the sourcing probability $\rho_{lr}$, and the line is the sourcing-weighted fit, whose slope is $\operatorname{Cov}_{\rho}(\log T,\log d)/\operatorname{Var}_{\rho}(\log d)$. The most heavily sourced areas are named. A downward line means the buyer's euro comes from places that are both productive and close, so equalising comparative advantage would push sourcing outward; a flat line means comparative advantage is orthogonal to the buyer's geography and equalising it would move sourcing nowhere.}
\end{figure}
