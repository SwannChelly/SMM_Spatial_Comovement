...
The two forces divide the work in opposite ways, and one number states it. In motor vehicles it is \emph{comparative advantage} that keeps the euro close: equalising $\hat T$ across areas sends it from 348 to 413~km, a 19\% increase, while switching off distance costs only 23~km (348 to 371~km). Removing the dispersion of capability destroys more proximity than removing the trade cost itself does. In aerospace the assignment is reversed: equalising $\hat T$ moves the euro by 2~km (355 to 358~km), essentially nothing, whereas switching off distance sends it 34~km further (355 to 389~km). There, proximity is doing the localising on its own. Appendix Figure~\ref{fig:counterfactual_distance_region} reports the same experiment buyer by buyer, as a percentage deviation from the estimated allocation.
...
\paragraph{Does sourcing track the buyer?} The counterfactual says capability sits near demand in motor vehicles. It does not say whether that capability is one mass placed inside the demand distribution or many local pockets, and those are different economies: the first sends every buyer to the same suppliers, the second gives each buyer its own. The distinction is measurable in one number. Let $b_{r}=\sum_{r'}X_{r'r}x_{r'}/\sum_{r'}X_{r'r}$ be the barycentre of the sourcing a shock in $r$ generates, with $x_{r'}$ the location of upstream region $r'$, and regress it on the buyer's own location, $b_{r}=(1-\lambda)c+\lambda x_{r}+\varepsilon_{r}$. Then $\lambda=1$ means every buyer sources around itself, $\lambda=0$ means the allocation is buyer-independent and every shock converges on one national barycentre, and intermediate values measure how much of its own position a buyer's sourcing inherits.\footnote{The barycentre is computed and regressed in a projected metric coordinate system (Lambert-93), and $\lambda$ is the pooled projection over both coordinates at once, so it does not depend on how the map is oriented. Appendix Table~\ref{tab:barycentre_shrinkage} reports it with standard errors clustered on the shocked region, and Appendix Figure~\ref{fig:sourcing_barycentre} draws the displacements $b_{r}-x_{r}$ themselves.}

The estimates are reported in Appendix Table~\ref{tab:barycentre_shrinkage}, and the informative fact about them is that they are \emph{small in both industries}: $\hat\lambda=0.065$ in motor vehicles and $0.080$ in aerospace, against a fit that is tight rather than noisy ($R^{2}=0.96$ and $0.85$). In neither industry does a buyer's sourcing follow the buyer; the arrows from each origin to its barycentre are correspondingly short, which is the same fact seen on a map rather than a second result. That the two numbers are close, and their difference small against the cross-region dispersion behind each of them, is the point: the geography of \emph{where} sourcing sits is common to the two industries, and what separates them is how much of the response each origin retains, which is the next paragraph. Two internal checks make the small value a finding rather than a failure of the diagnostic. The model predicts the ordering before the number is computed and predicts it against the intuition: the only buyer-specific term in $\rho_{lrs}$ is $d_{lr}^{-\alpha\theta}$, so $\lambda$ is governed by $\alpha\hat\theta$ alone and must be \emph{smaller} in motor vehicles despite its stronger alignment, which it is; and setting $\alpha=0$ removes that term outright, so $\lambda$ must vanish, which it does ($-0.003$ and $-0.023$). The one comparative movement worth recording is that equalising $\hat T$ raises the aerospace shrinkage by half, to $0.121$, while leaving the motor-vehicle one flat ($0.065$ to $0.069$): what holds aerospace sourcing away from each buyer's own neighbourhood is the concentration of capability, and what holds the motor-vehicle one down is nothing but the low trade-cost elasticity.\swann{Les deux comparaisons de ce paragraphe --- l'ecart entre industries et la hausse de $\hat\lambda$ en aerospatiale sous \emph{Distance only} --- doivent etre lues contre les erreurs-types du Tableau~\ref{tab:barycentre_shrinkage}; a confirmer apres le re-run.}

Read together with the counterfactual, this answers the hub question in the only form the data support. Motor-vehicle capability is one broadly central mass sitting inside the demand distribution: buyers do not source locally, they source from a common central pool, which is exactly how a negative covariance can hold at \emph{every} buyer simultaneously without any buyer sourcing around itself. Aerospace capability is concentrated away from most of its demand, and its buyers are pulled towards Toulouse and Île-de-France rather than towards themselves.
