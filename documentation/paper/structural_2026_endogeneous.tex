...
\emph{Across varieties.} A realised network is not the expected one. Each of a sector's $N_{s}$ varieties is won by exactly one region, so a realised incidence vector has at most $N_{s}$ non-zero entries however diffuse the expected network is. Write $\omega_{r'rs}=\sum_{j}v_{jrs}\mathbf{1}\{r'\text{ wins }j\}$, with $v_{jrs}$ the share of buyer $r$'s spending on sector $s$ going to variety $j$. Under Fr\'echet the identity of a variety's winner is independent of the price at which it is won, so expenditure weights and win locations are independent and, for \emph{any} pair of buyers,
\begin{equation}
...
Substituting \eqref{eq:variety_kernel} into \eqref{eq:incidence_identity} splits the expected concentration of a realised shock into four additive terms:
\begin{equation}
