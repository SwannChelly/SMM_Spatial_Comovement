...
whose inverse is the \emph{effective number} of supplier regions the shock reaches in that sector. We take it sector by sector because a buyer's shock hits every one of its input sectors and each sector has its own supplier geography, and we aggregate with the buyer's own purchase shares, $H_{r}(\Omega)=\sum_{s}(X_{rs}/X_{r})H_{rs}(\Omega)$.\footnote{$H_{r}$ is the purchase-weighted average of \emph{within-sector} Herfindahls and not the Herfindahl of the pooled portfolio $\omega_{r}(\Omega)=\sum_{s}(X_{rs}/X_{r})\omega_{rs}(\Omega)$; by convexity the first is the larger of the two, and the gap is the diversification a buyer obtains from the fact that its ten input sectors have different supplier geographies. The decomposition below requires the sector-by-sector object, so that is what we report, but $1/H_{r}$ should be read as an effective number of supplier regions \emph{per sector} rather than as the number of places a buyer's whole input bill reaches. The count of places is \eqref{eq:region_reach} below.}

Unlike the average sourcing distance and the local share, $H$ is a \emph{quadratic} statistic of the network. That is what makes granularity a term of it rather than a caveat on it, and --- as the next two paragraphs show --- what limits how much of it can be read as a statement about the network at all.
...
\end{equation}
the concentration of the expected network plus the dispersion of a realisation around it. The second term is strictly positive whenever $N_{s}$ is finite: in any realisation each variety is won by exactly one region, so $\omega_{rs}(\Omega)$ has at most $N_{s}$ non-zero entries however diffuse $\gamma_{rs}$ is.

Evaluating it takes one object. Writing $\omega_{r'rs}(\Omega)=\sum_{j=1}^{N_{s}}v_{jrs}(\Omega)\,I_{jr's}(\Omega)$, with $v_{jrs}$ the share of buyer $r$'s sector-$s$ spending going to variety $j$ and $I_{jr's}=\mathbf 1\{r'\text{ wins }j\text{ for }r\}$,
\begin{equation}
\mathbb{E}_{\Omega}\big[H_{rs}(\Omega)\big]=\mathcal{V}_{rs}+\big(1-\mathcal{V}_{rs}\big)H(\gamma_{rs}),
\qquad
\mathcal{V}_{rs}=\mathbb{E}_{\Omega}\Big[\textstyle\sum_{j}v_{jrs}^{2}(\Omega)\Big],
\label{eq:granular_buyer}
\end{equation}
with $\mathcal{V}_{rs}$ the \emph{effective number of varieties}, inverted. Under Fr\'echet the identity of a variety's winner is independent of the price at which it is won, so expenditure weights and win locations are independent and \eqref{eq:granular_buyer} is exact rather than an approximation.\footnote{\emph{Appendix :} the mechanism is clearest when varieties carry equal expenditure weights, $v_{jrs}=1/N_{s}$. Then $N_{s}\omega_{r'rs}(\Omega)$ is literally the number of varieties region $r'$ wins, which is $\operatorname{Binomial}(N_{s},\gamma_{r'rs})$, and since $\mathbb{E}[K^{2}]=N\gamma(1-\gamma)+N^{2}\gamma^{2}$ for $K\sim\operatorname{Bin}(N,\gamma)$, summing over $r'$ and using $\sum_{r'}\gamma_{r'rs}=1$ gives
$$\mathbb{E}_{\Omega}\big[H_{rs}(\Omega)\big]=H(\gamma_{rs})+\frac{1}{N_{s}}\big[1-H(\gamma_{rs})\big]:$$
a finite $N_{s}$ makes winner counts random, the variance of a count is mechanically positive, and a variance of shares is exactly what a Herfindahl adds up. Under CES a cheaper winner takes a larger share, so $N_{s}\omega_{r'rs}$ is a \emph{weighted} sum of winner indicators rather than a count and that expression is not literally a binomial second moment; \eqref{eq:granular_buyer} is the general case, with $\mathcal V_{rs}=1/N_{s}$ recovered under equal shares and $\mathcal{V}_{rs}\ge 1/N_{s}$ always by Cauchy--Schwarz, so reading granularity as a count \emph{understates} it. The model predicts $\mathcal{V}_{rs}$ to be the same for every buyer --- the Fr\'echet scale cancels in an expenditure share --- and that is a testable restriction rather than an assumption: measured across buyers its range does not exceed $0.0052$ in motor vehicles and $0.0126$ in aerospace. The closed form for $\mathcal V$ has no content at this calibration and we measure it from the draws instead: the variety weights are Pareto with tail index $\kappa_{s}=\theta/(\nu_{s}-1)$ and $\mathcal V$ would be $\Xi(\kappa_{s})/N_{s}$ with $\Xi(\kappa)=\Gamma(1-2/\kappa)/\Gamma(1-1/\kappa)^{2}$, but $\theta=1$ and $\nu_{s}=1.5$ give $\kappa_{s}=2$ \emph{exactly}, the boundary at which $\Xi$ diverges, so the variance of the variety weights is not finite and a single variety can carry a non-vanishing share of a buyer's sector spending.}

\paragraph{What a realised Herfindahl is counting.}
Equation \eqref{eq:granular_buyer} has two terms and the first of them is not about the network. Measured at the estimate, $\mathcal{V}_{rs}$ carries \textbf{87 per cent} of $\mathbb{E}_{\Omega}[H_{r}(\Omega)]$ in motor vehicles and \textbf{76 per cent} in aerospace: $1/\mathcal{V}_{s}$ is $9.7$ and $6.4$ against realised effective numbers of $8.5$ and $4.8$. \textbf{The reciprocal of a realised Herfindahl is therefore, to first order, an effective number of \emph{varieties} with a region label attached to it.}

This is not a feature of our calibration but of the statistic. A Herfindahl weights regions by the euros they collect, and with finitely many varieties those euros concentrate on whichever few varieties happen to be cheap, whatever the geography behind them. Three consequences run through the rest of this section and we state them once. Comparative advantage and trade costs enter $\mathbb{E}_{\Omega}[H]$ only through $\gamma_{rs}$, so \emph{no counterfactual on the two forces can move a realised Herfindahl by more than the remaining quarter or tenth} --- which is why the counterfactual series of Figure~\ref{fig:buyer_concentration_region} are compressed against the $N_{s}\to\infty$ limit, and the compression is the result rather than a defect of the drawing.\footnote{It is also why that statistic can move in the \emph{opposite} direction from the structural count it is meant to track: switching off comparative advantage moves $1/\mathbb{E}_{\Omega}[H_{r}]$ by $-1.8\%$ in motor vehicles against $+6.3\%$ for $1/H(\gamma_{r})$. The reversal is $\mathcal{V}$, not the network --- $\alpha=0$ lowers $H(\gamma_{rs})$, which should raise the effective number, but it also makes every buyer face the same ranking of varieties and so raises $\mathcal{V}$ ($0.103\to0.106$), and at $87\%$ weight the second channel wins. Any reading of a realised effective number across regimes should be taken from the structural column beside it.} Second, the \emph{realised} column of Table~\ref{tab:incidence} and its two granular cells inherit exactly this contamination, and we read them accordingly below. Third, anything one wants to say about \emph{places} needs a statistic with no expenditure weights in it, which is the next paragraph.

\paragraph{How many regions a shock reaches.}
Region $r'$ loses every one of the $N_{s}$ varieties of sector $s$ with probability $(1-\gamma_{r'rs})^{N_{s}}$, and variety draws are independent across sectors, so the expected number of distinct upstream commuting zones supplying buyer $r$ is
\begin{equation}
...
\end{equation}
exact under the same independence that gives \eqref{eq:granular_buyer} and carrying no expenditure weights at all. Its ceiling is not the geography but the variety count: a buyer cannot source from more zones than it buys varieties, and $\sum_{s}N_{s}$ is \textbf{185} in motor vehicles and \textbf{99} in aerospace. Figure~\ref{fig:buyer_reach_region} reports it for every shocked commuting zone.

\textbf{The level of \eqref{eq:region_reach} is close to a restatement of a targeted moment and we do not read it as a finding.} The median motor-vehicle buyer reaches \textbf{112} zones and the median aerospace buyer \textbf{63}, that is $61\%$ and $63\%$ of their own ceilings. But the estimation targets the share of commuting zones hosting no supplier, $\bar G_{s}(0)=\text{mean}_{l}(1-\hat q_{ls})^{N_{s}}$, and \eqref{eq:region_reach} is the same functional form disaggregated to the buyer; $N_{s}$ is calibrated on precisely that moment. The level therefore carries close to no free information.\footnote{It does carry one useful check: computed instead as the union of origins in a simulated finite-variety economy --- a different route sharing no code --- the same object gives 113 and 62. \emph{Appendix :} the support, i.e.\ the number of zones with $\gamma_{r'rs}>0$ in at least one of the buyer's sectors, is 275 in both industries, so a continuum of varieties would spread the same shock over $2.4$ and $4.4$ times as many places. That comparison should not be read as slack: 275 is unattainable at any finite $N_{s}$, since the binding constraint is the $185$/$99$ ceiling above, and the \emph{Infinite varieties} series of Figure~\ref{fig:buyer_reach_region} is a limit rather than a target.}

What is \emph{not} targeted is how the reach responds to the two forces, and there the answer is lopsided. Both counterfactuals flatten $\gamma_{rs}$ and therefore spread the win probabilities over more zones, so both raise the reach. But equalising $\hat T$ --- that is, removing comparative advantage --- raises it by $14\%$ in motor vehicles and $23\%$ in aerospace, while setting $\alpha=0$ raises it by only $2.0\%$ and $1.1\%$.\footnote{\emph{Appendix :} under $\alpha=0$, and likewise under the unselective benchmark, the reach is \emph{identical} for every buyer to machine precision ($114.32$ and $63.47$). That is the section's internal control and an identity rather than a fitted result: the distance factor $d_{r'r}^{-\alpha\theta}$ is the only term in $\gamma_{r'rs}$ that varies with the identity of the buyer, so with equal distances every buyer faces the same sourcing shares. The same identity drives cells (ii) and (iv) of \eqref{eq:four_cells} to exactly zero and $C^{\text{gran}}$ to exactly one in that regime.}

\paragraph{How far, and how few: the two forces divide the labour.}
That asymmetry is the section's central result and it is the mirror image of Section~\ref{subsec:diffusion}. \textbf{Comparative advantage decides how \emph{few} places a shock reaches; trade costs decide how \emph{far} it goes, and almost nothing about how few.} On the euro-weighted count, equalising $\hat T$ takes the effective number of destinations from $38\%$ of the unselective benchmark to \textbf{98\%} in motor vehicles and from $28\%$ to \textbf{88\%} in aerospace --- essentially all the way to an economy in which nothing selects among regions --- while setting $\alpha=0$ moves it hardly at all ($38\to40\%$ and $28\to29\%$).\footnote{We report these as ratios to the unselective benchmark rather than as levels because the levels ($58$ and $18$ effective commuting zones) are properties of $\gamma$ at $N_{s}=\infty$ and no French firm sources from 58 zones; the realised counts are $8.2$ and $4.9$. \emph{Appendix :} because the sourcing shares form an exponential family, each force moves concentration at a rate given by a covariance under the sourcing probabilities, $\partial H(\gamma_{rs})/\partial t=2\operatorname{Cov}_{\gamma_{\cdot rs}}(\log T_{a(r')s},\gamma_{r'rs})$ and $\partial H(\gamma_{rs})/\partial(\alpha\theta)=-2\operatorname{Cov}_{\gamma_{\cdot rs}}(\log d_{r'r},\gamma_{r'rs})$ --- the same two forces and the same weights as in \eqref{eq:alignment} and \eqref{eq:distance_rate}, now covarying with the shares rather than with the distance. Comparative advantage raises concentration whenever the productive regions already hold large shares; trade costs raise it only when a buyer's largest suppliers are also its nearest, so the second carries no sign restriction. Evaluated at the estimate and weighted by spending, $\partial H/\partial\log T=+0.019$ and $\partial H/\partial(\alpha\theta)=+0.006$ in motor vehicles, $+0.071$ and $+0.007$ in aerospace.} In Section~\ref{subsec:diffusion} the ranking is the other way round: trade costs move the sourcing distance by $6.6\%$ and $9.6\%$ and do so for every buyer, while comparative advantage moves it $19\%$ in motor vehicles and $1\%$ in aerospace.

The asymmetry is not a matter of which force is stronger but of how each discriminates, and it is a direct implication of the estimated $\alpha\hat\theta$ of $0.228$ and $0.458$ rather than a discovery on top of it. Over the whole span of French distances --- a log range of about $3.8$ --- trade costs downweight the most remote supplier by a factor of $2.4$ (motor vehicles) or $5.7$ (aerospace). Spread over the hundred-odd cells competing for a buyer, a gradient that shallow cannot concentrate anything; it can only tilt. Estimated comparative advantage, by contrast, differs by orders of magnitude across attraction areas and can leave a sector's sourcing to a handful of them.

\paragraph{Granularity makes buyers look alike.}
One further reading of Figure~\ref{fig:buyer_concentration_region} is a result in its own right. Across motor-vehicle buyers the realised effective number ranges over $2\%$ of its median and across aerospace buyers over $14\%$, against $16\%$ and $37\%$ for the structural number. \textbf{The cross-buyer heterogeneity the estimated network carries is therefore largely invisible in any one year.} The mechanism is the model's own prediction: $\mathcal{V}_{s}$ is common across buyers, and by the paragraph above it is most of what the realised statistic is made of. This matters beyond this section --- it bounds which cross-sectional facts the reduced-form half of the paper could ever hope to identify from a single cross-section of realised linkages.

\paragraph{Why the buyer-level index is not enough.}
Two buyers can each source narrowly and still describe opposite economies. They may source narrowly from \emph{different} regions, in which case each supplier region holds its own clientele and shocks to different buyers offset one another. Or they may pile onto the \emph{same} regions, in which case those regions hold undiversified clienteles and buyer shocks accumulate. $H_{rs}(\Omega)$ is identical in the two cases, and so is \eqref{eq:region_reach}. Separating them is the second of the two questions --- the supplier's portfolio of customers --- and it requires comparing buyers rather than describing them one at a time.

\paragraph{Aggregating across buyers.}
The concentration faced by the industry as a whole is $\bar H_{s}(\Omega)=\mathbb{E}_{r}[H_{rs}(\Omega)]$, where $\mathbb{E}_{r}$ weights each buyer by its share $X_{rs}/X_{\cdot s}$ of the sector's purchases, with $X_{\cdot s}=\sum_{k}X_{ks}$. The weights are not a smoothing device: a euro of demand is more likely to hit a large buyer than a small one, so weighting by purchases is the statement that the shock is proportional to size, and it is also the only aggregator under which the identity below holds exactly.\footnote{$H$ is averaged before being inverted into an effective number, since $1/H$ is convex; the buyer-level effective numbers of Figure~\ref{fig:buyer_concentration_region} therefore do not average to the aggregate figure. The cross-buyer distributions behind these means are in Appendix~\ref{app:incidence_buyer}.} Fix a realisation $\Omega$ and let $\bar\omega_{s}(\Omega)=\mathbb{E}_{r}[\omega_{rs}(\Omega)]$ be the incidence of a shock hitting the industry as a whole. Then
\begin{equation}
...
\end{equation}
the variance decomposition applied coordinate by coordinate. A shock to a single buyer lands with concentration $\bar H_{s}(\Omega)$ on average; a shock to the whole industry lands with $\|\bar\omega_{s}(\Omega)\|^{2}$, which is smaller exactly to the extent that buyers send their euros to different places. The first term is the part of a buyer's exposure that every other buyer shares, the second the part that is its own. We write $C^{\text{str}}_{s}$ for the common share of the \emph{structural} term and $C^{\text{gran}}_{s}$ for the common share of the \emph{granular} one.

Equation \eqref{eq:incidence_identity} is an average over buyers at a fixed realisation and \eqref{eq:variety_split} an expectation over realisations at a fixed buyer, so the two commute and applying both gives four additive terms, each a squared norm:
\begin{equation}
...
\label{eq:four_cells}
\end{equation}
Each term reads as a variance. Cell (iii) is the variance of the \emph{industry-level} incidence: the part of chance that survives aggregation over buyers, because the same varieties happen to be won for everyone. Cell (iv) is the average variance minus the variance of the average --- the part that diversifies away when the industry is aggregated --- and is non-negative by convexity.\footnote{\emph{Appendix :} both granular terms are evaluated from the variety kernel \eqref{eq:variety_kernel} below. Cell (iii) $=\sum_{r,r''}\tfrac{X_{rs}}{X_{\cdot s}}\tfrac{X_{r''s}}{X_{\cdot s}}\operatorname{Cov}_{\Omega}(\omega_{rs},\omega_{r''s})$, and $\operatorname{Cov}_{\Omega}(\omega_{rs},\omega_{r''s})=\mathcal V_{rr''s}(Q_{rr''s}-\Gamma_{rr''s})$. The total granular term is the same expression restricted to $r''=r$ and averaged, $\mathbb{E}_r[\mathcal V_{rs}(1-H(\gamma_{rs}))]$, using $Q_{rr}=1$, which is \eqref{eq:granular_buyer}; cell (iv) is the difference. Non-negativity of (iv) is $\mathbb{E}_r\|x_r\|^2\ge\|\mathbb{E}_r x_r\|^2$ at $x_r=\omega_{rs}(\Omega)-\gamma_{rs}$, with equality only if every buyer's realised deviation is the same vector.}

\paragraph{It is the hubs that make aerospace sourcing common.}
The buyer-specific cell is the paper's efficiency--stability result, and it has a closed form. Write $\gamma_{r'rs}=\bar\gamma_{r's}(1+e_{r'rs})$ with $e$ mean-zero across buyers. Because $\log\gamma_{r'rs}=\log T_{a(r')s}-\alpha\theta\log d_{r'r}-\log\Phi_{rs}$, comparative advantage --- a property of a \emph{supplier}, identical for every buyer looking at it --- does not enter $e$ at all, while distance --- a property of a \emph{pair} --- enters it multiplied by $\alpha\theta$. Hence
\begin{equation*}
1-C^{\text{str}}_{s}\;\simeq\;(\alpha\theta)^{2}\,\Lambda_{s},
\end{equation*}
with $\Lambda_{s}$ the $\bar\gamma^{2}$-weighted cross-buyer variance of a region's \emph{relative} remoteness: pure geometry, in which $\hat T$ enters only through the weights.

That decomposition separates the two industries cleanly, and in a way that could have come out otherwise. The measured $\Lambda$ is $0.30$ in motor vehicles and $0.22$ in aerospace --- \emph{the same geography} --- so essentially the whole of the threefold gap in $1-C^{\text{str}}$ ($0.015$ against $0.046$) is $(\alpha\theta)^{2}$, $0.052$ against $0.210$. And because $\hat T$ moves this cell only through \emph{where} it puts the weight, the counterfactual is the alignment result of Section~\ref{subsec:diffusion} arriving on a new statistic. Under \emph{Distance only} every buyer sources from its own neighbourhood, the most buyer-specific allocation available, and comparative advantage then redirects part of the euro to regions chosen for their productivity. In aerospace those regions are far from most buyers and therefore roughly \emph{equally} far from all of them, so the redirection makes buyers agree and $\Lambda$ falls from $0.55$ to $0.22$; in motor vehicles comparative advantage already sits near demand at four fifths of the (sector, buyer) pairs, the reweighting is close to a no-op, and $\Lambda$ barely moves, $0.33$ to $0.30$. \textbf{It is the hubs that make aerospace sourcing common; in motor vehicles nothing but the low distance elasticity holds buyer-specificity down.} Geographically the same statement: in aerospace \textbf{Toulouse alone accounts for $30.5\%$} of cell (ii), with Issoudun and \^Ile-de-France at about $10\%$ each, while in motor vehicles the largest contributor, Strasbourg, accounts for $8.1\%$.

Both industries are therefore overwhelmingly common in their structural exposure --- $C^{\text{str}}=0.985$ and $0.954$ --- and they sit in opposite corners of the plane the section opened with: aerospace is concentrated \emph{and} common, the hub-and-spoke case, while motor vehicles is a dispersed common pool. Neither is made of local pockets. On the \emph{structural} network that is a statement about risk on both sides of the link: shocks to different buyers pile onto the same suppliers rather than offsetting, three times more tightly in aerospace, and those suppliers hold correspondingly undiversified clienteles. It does not carry over unchanged to a realised year, where cells (ii) and (iv) together leave $0.075$ and $0.140$ buyer-specific --- three to five times the structural figure --- almost all of it from cell (iv).

\paragraph{Structural against granular.}
\textbf{The level of the granular cells restates a targeted moment, and we state that before the numbers rather than after.} $N_{s}$ is calibrated on the share of commuting zones hosting no supplier, so how much of realised concentration is granular is close to a re-expression of block~6 of the estimation. With that said: in a single year a motor-vehicle shock reaches an effective $8.2$ supplier regions and an aerospace shock $4.9$, against $58$ and $18$ in expectation, so the granular cells carry $\mathbf{86\%}$ and $\mathbf{72\%}$ of realised concentration.\footnote{Two measured inputs set the magnitude. Expenditure is far from evenly spread across varieties --- $\mathcal{V}_{s}N_{s}$ has a median of $1.81$ in motor vehicles and $1.63$ in aerospace, so the effective number of varieties is about $0.6N_{s}$ and the multinomial reading of the equal-share case would understate realised concentration by 40 to 45 per cent. And the $1-H(\gamma_{rs})$ factor of \eqref{eq:granular_buyer} makes granularity and comparative advantage \emph{substitutes} as sources of concentration --- granularity concentrates whatever dispersion the expected network has left --- which the cross-sector spread confirms: in aerospace granularity accounts for $96\%$ of realised concentration in C33Z, whose expected network spreads over an effective hundred regions on five varieties, against $57\%$ in C30C, the sector with the most concentrated comparative advantage. It is also why the granular share rises to $94\%$ and $91\%$ under the unselective benchmark, where the expected network is at its most diffuse.}

What the second dimension of \eqref{eq:four_cells} adds over that two-way split is one comparison, and it is the only part of the four-cell structure that is not either near-degenerate or an identity. \textbf{Cell (iv) is proportionally larger than cell (ii): chance is a more idiosyncratic force than the estimated network.} At $8.5\%$ of the granular term in motor vehicles and $17.6\%$ in aerospace it sits against $1.4\%$ and $4.7\%$ for cell (ii) of the structural term --- so a shock to one buyer does fail to reach another's suppliers more often through chance than through geography, just not nearly often enough to offset what chance adds in common.

\paragraph{Whether chance is shared is a model implication, not a measurement.}
That last clause rests on the split of the granular term between cells (iii) and (iv), which is a statement about \emph{pairs} of buyers. Equation \eqref{eq:granular_buyer} fixes the total, $\mathcal{V}_{s}[1-\mathbb{E}_{r}H(\gamma_{rs})]$; how it divides needs one more object, the probability $Q_{rr''s}$ that a realisation gives two buyers the \emph{same} variety in the same region, against the rate $\Gamma_{rr''s}=\gamma_{rs}\!\cdot\!\gamma_{r''s}$ at which the sourcing shares already put them in the same region through different varieties. The same argument that gave \eqref{eq:granular_buyer} extends to any pair,
\begin{equation}
...
\end{equation}
with $\mathcal{V}_{rr''s}=\mathbb{E}_{\Omega}[\sum_{j}v_{jrs}v_{jr''s}]$; setting $r''=r$ returns \eqref{eq:granular_buyer}. The finite number of varieties therefore enters through two objects and no others: $\mathcal{V}$ governs \emph{how much} a realisation concentrates a shock, and the excess co-sourcing $Q-\Gamma$ governs \emph{whether that concentration is shared}.

Measured on the estimated economy, $Q$ averages $0.90$ in motor vehicles and $0.82$ in aerospace against $\Gamma$ of $0.016$ and $0.030$, and cell (iii) accordingly swamps cell (iv): $C^{\text{gran}}=0.915$ and $0.824$, against a floor of $0.045$ and $0.056$ that independent win events would deliver.\footnote{The floor is not zero because a buyer always co-sources with itself, so the diagonal $Q_{rr}=1$ enters \eqref{eq:four_cells} by construction and $C^{\text{gran}}$ collapses to the granular-weighted Herfindahl of buyer spending. With spending close to equal across the 22 and 18 buyers that floor is close to $1/n$, so it is a benchmark about buyer counts rather than about the network; the quantity with content is $C^{\text{gran}}_{s}$ net of it.} \textbf{We report this as a prediction of the model and not as a measurement}, for three reasons that a reader should weigh before using the number. First, $Q\to1$ as $\alpha\theta\to0$ is an identity --- with equal distances every buyer faces the same ranking of varieties and draws the same winner --- so at the estimated $\alpha\theta$ a $Q$ near one is close to what that identity alone predicts. Second, the entire source of $Q>\Gamma$ is a modelling convention: one productivity draw per (variety, region), \emph{shared across all buyers}. A model in which a supplier can serve one buyer and not another --- capacity, certification, relationship-specific investment, all plausible in aerospace --- would deliver $Q$ far below one while matching every targeted moment identically, and nothing in our data distinguishes the two. Third, no moment in the estimation looks at a pair of buyers at all, so $Q$ is not disciplined anywhere. What the model does deliver, conditional on that convention, is the economics: a variety cheap enough to win one buyer is cheap enough to win the others, so cells (i) and (iii) sum to $92\%$ and $86\%$ of realised concentration and granularity \emph{reinforces} the industry's common exposure rather than diversifying it. Establishing whether that is true of French sourcing requires buyer-supplier links, which we do not observe.
