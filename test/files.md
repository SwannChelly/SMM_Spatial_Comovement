...
| `../gates.ipynb` | **The whole Python suite, run with its output in view.** One cell per gate file, each preceded by a short statement of what it is for, then a summary table; then the objects the gates reason about — the planted economy, the identities `simulate_economy` closes, the three regimes and the channel decomposition — rendered rather than asserted. Runs each file in a SUBPROCESS, because the gates install fixture stubs onto the library modules and two of them in one interpreter would leak. The `.py` files stay the source of truth. |
