...
| `test_cloglog_streaming.jl` | **Correctness gate for the STREAMING cloglog kernel** — the memory-light `_cloglog_irls_cells`, which fits the same model as `_cloglog_irls` without ever materialising the `n_cell × N_rho` design. Three-way comparison on every case: streaming vs the dense kernel vs GLM.jl. Covers all four production design shapes (bin dummies / continuous log-distance × size control on / off), with and without control cells, plus a bin-homogeneous FE group (the worst case for the closed-form within transform's cancellation). Section 3 runs the full `fast_cloglog_regression` on both paths (`CLOGLOG_STREAMING[]` toggled), which also exercises the geography → cell mapping and the `log z = logz_const + logz_resid` decomposition; Section 4 prints the design/allocation/time comparison at production shape. Standalone (defines the geography globals itself — no data or worker pool). Requires GLM.jl. Usage: `julia test/test_cloglog_streaming.jl`. |
