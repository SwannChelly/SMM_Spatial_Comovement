...
if [ -z "$1" ]; then
    ...
    echo "  --draws    : Fréchet draw method for the OPTIMISATION draws: sobol (default) or mc"
...
if [ "$DRAWS" != "mc" ] && [ "$DRAWS" != "sobol" ]; then
    echo "Error: --draws must be sobol or mc (got: $DRAWS)"
    exit 1
fi
