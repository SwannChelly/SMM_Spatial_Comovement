...
| File | Shape | Content |
|---|---|---|
| `Sigma_beta_gamma[_cloglog][_1][_f].npy` | `(N_REG + n_γ)²` | joint bootstrap covariance of β+γ moments, **β block first**. Selected by `sigma_beta_gamma_filename(; smm)` on `REG_METHOD`, `N_REG==1`, and entry point (`_f` = SMM) |
| `w_beta.npy` / `w_gamma.npy` | `(N_REG)²` / `(n_γ)²` | block-diagonal fallback |
| `X_rs.npy` | `(S, R)` | drives `T_MASK` and the Σ reconciliation |
| `filter_N_upstream.npy` | `(S, R)` | status 0 = no firms, 1 = has suppliers, 2 = firms but no supplier (the control group) |
